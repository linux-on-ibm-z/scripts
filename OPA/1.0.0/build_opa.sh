...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/OPA/1.0.0/build_opa.sh
...
GO_VERSION="1.23.1" # see https://github.com/open-policy-agent/opa/blob/v1.0.0/.go-version
SOURCE_ROOT="$(pwd)"
...
function configureAndInstall() {
    ...
    docker build -t golang-wasmtime:"${GO_VERSION}"-bookworm -f ./golang-wasmtime.Dockerfile .
    ...
    #Build OPA
    if [[ $DISTRO == "rhel-9.2" || $DISTRO == "rhel-9.4" || $DISTRO == "rhel-9.5" || $DISTRO == "ubuntu-22.04" || $DISTRO == "ubuntu-24.04" || $DISTRO == "ubuntu-24.10" ]]; then
        make ci-go-ci-build-linux
        make image-s390x
    else
        make ci-go-ci-build-linux-static # due to glibc version
...
function gettingStarted() {
    ...
    printf -- " Binary is available at $SOURCE_ROOT/opa/_release/$PACKAGE_VERSION/opa_linux_s390x \n"
    printf -- '**********************************************************************************************************\n'
}

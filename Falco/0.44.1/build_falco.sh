...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Falco/0.44.1/build_falco.sh
...
GO_VERSION="1.25.0"
PLUGINS_VERSION="0.7.1" # https://github.com/falcosecurity/falco/blob/0.44.1/CMakeLists.txt#L297
...
function cleanup() {
    rm -rf $SOURCE_ROOT/go"${GO_VERSION}".linux-s390x.tar.gz* $SOURCE_ROOT/cmake-3.28.3*
...
function configureAndInstall() {
    ...
    # build kernel driver separately on SLES 15 SP7 as it requires GCC 7.5
    if [[ $DISTRO == "sles-15.7" ]]; then
        cd $SOURCE_ROOT/falco/build/
        make driver \
            CC=/usr/bin/gcc-7 \
...
function printSummary() {
    ...
    printf -- "\n* Getting Started * \n"
    printf -- "\n# Kernel module"
    printf -- "\nSee https://falco.org/docs/developer-guide/source/#load-latest-falco-kernel-module for steps to load the kernel module."
    if [[ "${DISTRO}" =~ ^rhel-8 ]]; then
        printf -- "\nRun sudo LD_PRELOAD=/lib64/libresolv.so.2 falco -o engine.kind=kmod"
    else
        printf -- "\nRun sudo falco -o engine.kind=kmod"
    fi
    printf -- "\n# Modern eBPF probe (Default driver)"
    printf -- "\nRun sudo falco"
...
"rhel-9.6" | "rhel-9.8" | "rhel-10.1" | "rhel-10.2")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
    ...
    if [[ $DISTRO == "rhel-9.6" ]]; then
        ...
        sudo yum install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-10.noarch.rpm |& tee -a "${LOG_FILE}"
        sudo yum install -y clang19 |& tee -a "${LOG_FILE}"
        sudo alternatives --install /usr/bin/clang clang /usr/bin/clang-19 100 --slave /usr/bin/clang++ clang++ /usr/bin/clang++-19 |& tee -a "${LOG_FILE}"
    fi
    configureAndInstall |& tee -a "$LOG_FILE"
...
"sles-15.7")
    ...
    sudo zypper install -y gcc gcc-c++ gcc14 gcc14-c++ git-core cmake patch which automake autoconf libtool libelf-devel gawk tar curl vim wget pkg-config glibc-devel-static "kernel-default-devel=${SLES_KERNEL_PKG_VERSION}" kmod clang17 llvm17 bpftool |& tee -a "${LOG_FILE}"
    sudo update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-14 50 |& tee -a "${LOG_FILE}"
    sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-14 50 |& tee -a "${LOG_FILE}"
    export CC=$(which gcc)
    export CXX=$(which g++)
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
...
"ubuntu-24.04")
    ...
    printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
    sudo apt-get update
    sudo apt-get install -y git cmake libssl-dev build-essential pkg-config autoconf wget curl patch libtool libelf-dev gcc gcc-13 g++-13 rpm linux-headers-$(uname -r) linux-tools-$(uname -r) kmod clang llvm |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Falco/0.43.0/build_falco.sh
...
GO_VERSION="1.25.0"
PLUGINS_VERSION="0.6.1" # https://github.com/falcosecurity/falco/blob/0.43.0/CMakeLists.txt#L270
...
function configureAndInstall() {
    ...
    if [[ "${DISTRO}" =~ ^rhel-9 ]] || [[ "${ID}" == "ubuntu" ]]; then
...
function printSummary() {
    ...
    if [[ "${DISTRO}" =~ ^rhel-9 ]] || [[ "${DISTRO}" == "ubuntu-22.04" ]] || [[ "${DISTRO}" == "ubuntu-24.04" ]]; then
        printf -- "\n# Legacy eBPF probe"

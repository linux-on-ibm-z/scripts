...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Falco/0.34.1/build_falco.sh
...
function configureAndInstall() {
    ...
    cd $SOURCE_ROOT/falco/build
    # insmod will fail in container. Hence, we made non-mandatory.
    sudo insmod driver/falco.ko || true
...
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-22.10")
    ...
    sudo apt-get update
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y git cmake build-essential pkg-config autoconf wget curl patch libtool libelf-dev gcc rpm linux-headers-generic kmod
...
"rhel-8.6" | "rhel-8.7")
    ...
    sudo yum install -y gcc gcc-c++ git make cmake autoconf automake pkg-config patch libtool elfutils-libelf-devel diffutils which createrepo libarchive wget curl rpm-build kmod kernel-devel
...
"rhel-9.0" | "rhel-9.1")
    ...
    sudo yum install --allowerasing -y gcc gcc-c++ git make cmake autoconf automake pkg-config patch perl-FindBin libtool elfutils-libelf-devel diffutils which createrepo libarchive wget curl rpm-build kmod kernel-devel go
    go version

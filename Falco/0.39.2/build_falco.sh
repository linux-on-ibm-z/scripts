...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Falco/0.39.2/build_falco.sh
...
function configureAndInstall() {
    ...
    if [[ "${DISTRO}" =~ ^rhel-8 ]] || [[ "${DISTRO}" == "ubuntu-20.04" ]]; then 
        printf -- '\nApplying patch \n'
    ...
    else # sles-15.6, rhel-9, ubuntu-22.04, ubuntu-24.04, ubuntu-24.10
    ...
    if [[ "${DISTRO}" =~ ^rhel-9 ]] || [[ "${DISTRO}" == "ubuntu-22.04" ]] || [[ "${DISTRO}" == "ubuntu-24.04" ]] || [[ "${DISTRO}" == "ubuntu-24.10" ]] || [[ "${DISTRO}" == "sles-15.6" ]]; then
...
"rhel-9.2" | "rhel-9.4" | "rhel-9.5")
    ...
    sudo yum install --allowerasing -y gcc gcc-c++ git make cmake autoconf automake pkg-config patch perl-IPC-Cmd perl-bignum perl-core perl-FindBin libtool elfutils-libelf-devel diffutils which createrepo libarchive wget curl rpm-build kmod kernel-devel-$(uname -r) go clang llvm bpftool |& tee -a "${LOG_FILE}"
    go version
...
"sles-15.6")
    ...
    sudo zypper install -y gcc gcc-c++ gcc9 gcc9-c++ git-core cmake patch which automake autoconf libtool libelf-devel gawk tar curl vim wget pkg-config glibc-devel-static go1.21 "kernel-default-devel=${SLES_KERNEL_PKG_VERSION}" kmod clang17 llvm17 bpftool |& tee -a "${LOG_FILE}"
    sudo update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-9 50 |& tee -a "${LOG_FILE}"
    export CC=$(which gcc)
    export CXX=$(which g++)
    go version
...
"ubuntu-20.04")
    ...
    sudo apt-get update
    sudo apt-get install -y git cmake build-essential pkg-config autoconf wget curl patch libtool libelf-dev gcc rpm linux-headers-$(uname -r) linux-tools-$(uname -r) kmod |& tee -a "${LOG_FILE}"

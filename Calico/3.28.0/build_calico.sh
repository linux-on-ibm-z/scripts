...
#Description:   The script builds Calico version v3.28.0 on Linux on IBM Z for RHEL (8.8, 8.10, 9.2, 9.4), Ubuntu (20.04, 22.04, 24.04) and SLES (12 SP5, 15 SP5, 15 SP6).
...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.28.0/build_calico.sh
...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.28.0"
ETCD_VERSION="v3.5.6"
GO_VER_GO_BUILD="go1.22.7.linux-s390x.tar.gz"
GO_VER_CALICO="go1.22.5.linux-s390x.tar.gz"
BIRD_VERSION="v0.3.3-208-g1e2ff99d"
GOBUILD_VERSION="v0.91"
FORCE="false"
TESTS="false"
export SOURCE_ROOT=$(pwd)
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.28.0/patch"
GO_INSTALL_URL_GO_BUILD="https://golang.org/dl/${GO_VER_GO_BUILD}"
GO_INSTALL_URL_CALICO="https://golang.org/dl/${GO_VER_CALICO}"
...
function configureAndInstall() {
    ...
    printf -- "\nInstalling Go ... \n" | tee -a "$LOG_FILE"
    wget $GO_INSTALL_URL_GO_BUILD
    sudo tar -C /usr/local -xzf $GO_VER_GO_BUILD
...
EOF
    ...
    if [ $(docker images 'calico/go-build:v0.91' | wc -l) == 2 ]; then
    ...
    export LOG_FILE="$LOGDIR/configuration-$(date +"%F-%T").log"
    printf -- "\nInstalling Go ... \n" | tee -a "$LOG_FILE"
    wget $GO_INSTALL_URL_CALICO
    sudo tar -C /usr/local -xzf $GO_VER_CALICO
...
"rhel-8.8" | "rhel-8.10" | "rhel-9.2" | "rhel-9.4")
    ...
    sudo yum install -y curl git wget tar gcc glibc.s390x docker-ce docker-ce-cli containerd.io make which patch 2>&1 | tee -a "$LOG_FILE"

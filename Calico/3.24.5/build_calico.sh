...
#Description:   The script builds Calico version v3.24.5 on Linux on IBM Z for RHEL (7.8, 7.9, 8.4, 8.6, 8.7, 9.0, 9.1), Ubuntu (18.04, 20.04, 22.04, 22.10) and SLES (12 SP5, 15 SP4).
...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.24.5/build_calico.sh
...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.24.5"
ETCD_VERSION="v3.5.1"
GOLANG_VERSION="go1.19.linux-s390x.tar.gz"
FORCE="false"
TESTS="false"
CURDIR="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.24.5/patch"
...
function cleanup() {
    rm -rf "${CURDIR}/${GOLANG_VERSION}" "${CURDIR}/etcd-v3.5.3-linux-s390x" "${CURDIR}/etcd-v3.5.3-linux-s390x.tar.gz"
...
EOF
    ...
    git clone -b v0.75 https://github.com/projectcalico/go-build $GOPATH/src/github.com/projectcalico/go-build 2>&1 | tee -a "$GOBUILD_LOG"
    ...
    wget $PATCH_URL/Dockerfile.test.s390x -P $GOPATH/src/github.com/projectcalico/calico/felix/fv
    cp $GOPATH/src/github.com/projectcalico/calico/felix/docker-image/Dockerfile.amd64 $GOPATH/src/github.com/projectcalico/calico/felix/docker-image/Dockerfile.s390x
    sed -i 's/amd64/s390x/g' $GOPATH/src/github.com/projectcalico/calico/felix/docker-image/Dockerfile.s390x
    sed -i 's/debian:10-slim/debian:11-slim/g' $GOPATH/src/github.com/projectcalico/calico/felix/docker-image/Dockerfile.s390x
    cp $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.amd64 $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    sed -i 's/amd64/s390x/g' $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    sed -i '18d' $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    sed -i '5c RUN mkdir //code' $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    sed -i '12c RUN microdnf update' $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    sed -i '9d' $GOPATH/src/github.com/projectcalico/calico/apiserver/docker-image/Dockerfile.s390x
    ...
    ARCH=s390x make -C calico bin/helm 2>&1 | tee -a "$CALICO_LOG"
...
EOF
    ...
    ARCH=s390x make fv 2>&1 | tee -a "$TEST_KC_LOG" || true
    ...
    ARCH=s390x make ut 2>&1 | tee -a "$TEST_CNI_LOG" || true

    cd $GOPATH/src/github.com/projectcalico/calico/confd
    ARCH=s390x make ut 2>&1 | tee -a "$TEST_CONFD_LOG" || true
    ...
    ARCH=s390x make ut 2>&1 | tee -a "$TEST_APISERVER_LOG" || true
...
"ubuntu-18.04" | "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-22.10")
    ...
    sudo apt-get update
    sudo apt-get install -y patch git curl tar gcc wget make docker.io clang 2>&1 | tee -a "$LOG_FILE"

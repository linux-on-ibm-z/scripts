...
#               Build logs can be found in $HOME/calico-v3.14.1/logs/ . Test logs can be found at $HOME/calico-v3.14.1/logs/testLog-DATE-TIME.log.
#               By Default, system tests are turned off. To run system tests for Calico, pass argument "-t" to shell script.
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.14.1/build_calico.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.14.1/patch"
...
curl -o "Dockerfile.s390x" $PATCH_URL/Dockerfile.s390x.${GOBUILD_VERSION} 2>&1 | tee -a "$GOBUILD_LOG"
printf -- "\nApplying patch for go-build Makefile ... \n"  | tee -a "$GOBUILD_LOG"
curl -s $PATCH_URL/Makefile.diff.${GOBUILD_VERSION} | git apply - 2>&1 | tee -a "$GOBUILD_LOG"
...
if grep -Fxq "Successfully tagged calico/go-build:${GOBUILD_VERSION}" $GOBUILD_LOG
then
...
cd $GOPATH/src/github.com/projectcalico/typha
ARCH=s390x make GO_BUILD_VER=${GOBUILD_VERSION} image | tee -a "$TYPHA_LOG"
...
#Building Felix
ARCH=s390x make GO_BUILD_VER=${GOBUILD_VERSION} image 2>&1 | tee -a "$FELIX_LOG"
...
ARCH=s390x make GO_BUILD_VER=${GOBUILD_VERSION} image 2>&1 | tee -a "$CNI_LOG"

...
#Description:   The script builds Calico version v3.25.1 on Linux on IBM Z for RHEL (7.8, 7.9, 8.6, 8.7, 9.0, 9.1), Ubuntu (20.04, 22.04, 22.10, 23.04) and SLES (12 SP5, 15 SP4).
...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.25.1/build_calico.sh
...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.25.1"
ETCD_VERSION="v3.5.7"
GOLANG_VERSION="go1.18.linux-s390x.tar.gz"
BIRD_VERSION="v0.3.3-190-g69b6358"
FORCE="false"
TESTS="false"
CURDIR="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.25.1/patch"
...
function cleanup() {
    rm -rf "${CURDIR}/${GOLANG_VERSION}" "${CURDIR}/etcd-v3.5.7-linux-s390x" "${CURDIR}/etcd-v3.5.7-linux-s390x.tar.gz"
...
EOF
    ...
    git clone -b v0.82 https://github.com/projectcalico/go-build $GOPATH/src/github.com/projectcalico/go-build 2>&1 | tee -a "$GOBUILD_LOG"
    ...
    ARCH=s390x VERSION=v0.82 ARCHIMAGE='$(DEFAULTIMAGE)' make image | tee -a "$GOBUILD_LOG"
    docker tag calico/go-build:v0.82 calico/go-build:v0.82-s390x | tee -a "$GOBUILD_LOG"
    if [ $(docker images 'calico/go-build:v0.82' | wc -l) == 2 ]; then
        echo "Successfully built calico/go-build:v0.82" | tee -a "$GOBUILD_LOG"
    else
    ...
    cd projectcalico-bird-69b6358/
    ./build.sh 2>&1 | tee -a "$BIRD_LOG"
    make image -f Makefile.calico 2>&1 | tee -a "$BIRD_LOG"
    docker tag calico/bird:latest-s390x calico/bird:v0.3.3-190-g69b63580-s390x
    cd "$CURDIR"
    sudo rm -rf bird.tar.gz projectcalico-bird-69b6358 2>&1 | tee -a "$BIRD_LOG"

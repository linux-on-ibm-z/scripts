...
#Description:   The script builds Calico version v3.12.0 on Linux on IBM Z for Rhel(7.6, 7.7, 7.8), Ubuntu(16.04, 18.04) and SLES(12SP4, 12SP5, 15 SP1).
...
if [[ "$FORCE" == "true" ]]; then
	...
	printf -- "\nAs part of the installation, Go 1.11.5 will be installed. \n" | tee -a "$CONF_LOG"
	while true; do
...
curl  -o "etcd-dockerfile_release.diff" $PATCH_URL/etcd-dockerfile_release.diff 2>&1 | tee -a "$ETCD_LOG"
printf -- "\nApplying patch to Dockerfile-release . . . \n"  | tee -a "$ETCD_LOG"
patch Dockerfile-release etcd-dockerfile_release.diff 2>&1 | tee -a "$ETCD_LOG"
...
curl  -o "Makefile-typha.diff" $PATCH_URL/Makefile-typha.diff 2>&1 | tee -a "$TYPHA_LOG"
printf -- "\nApplying patch to Makefile . . . \n"  | tee -a "$$TYPHA_LOG"
patch Makefile Makefile-typha.diff 2>&1 | tee -a "$TYPHA_LOG"
rm -rf Makefile-typha.diff
...
curl  -o "Makefile_felix.diff" $PATCH_URL/Makefile_felix.diff 2>&1 | tee -a "$FELIX_LOG"
printf -- "\nApplying patch to Makefile . . . \n"  | tee -a "$FELIX_LOG"
patch Makefile Makefile_felix.diff 2>&1 | tee -a "$FELIX_LOG"
...
curl  -o "Dockerfile.s390x_felix.diff" $PATCH_URL/Dockerfile.s390x_felix.diff 2>&1 | tee -a "$FELIX_LOG"
printf -- "\nApplying patch to Dockerfile.s390x . . . \n"  | tee -a "$FELIX_LOG"
patch Dockerfile.s390x Dockerfile.s390x_felix.diff 2>&1 | tee -a "$FELIX_LOG"
...
# Modify Makefile, patching the same
printf -- "\nDownloading patch for cni-plugin Makefile . . . \n"  | tee -a "$CNI_LOG"
curl  -o "Makefile_cni.diff" $PATCH_URL/Makefile_cni.diff 2>&1 | tee -a "$CNI_LOG"
printf -- "\nApplying patch to Makefile . . . \n"  | tee -a "$CNI_LOG"
patch Makefile Makefile_cni.diff 2>&1 | tee -a "$CNI_LOG"
rm -rf Makefile_cni.diff
...
curl  -o "Makefile_node.diff" $PATCH_URL/Makefile_node.diff 2>&1 | tee -a "$NODE_LOG"
printf -- "\nApplying patch to Makefile . . . \n"  | tee -a "$NODE_LOG"
patch Makefile Makefile_node.diff 2>&1 | tee -a "$NODE_LOG"
rm -rf Makefile_node.diff


# Modify `Dockerfile.s390x`, patching the same
printf -- "\nDownloading patch for node Dockerfile.s390x . . . \n"  | tee -a "$NODE_LOG"
curl  -o "Dockerfile.s390x_node.diff" $PATCH_URL/Dockerfile.s390x_node.diff 2>&1 | tee -a "$NODE_LOG"
printf -- "\nApplying patch to Dockerfile.s390x . . . \n"  | tee -a "$NODE_LOG"
patch Dockerfile.s390x Dockerfile.s390x_node.diff 2>&1 | tee -a "$NODE_LOG"
...
cp $GOPATH/src/github.com/coreos/etcd/bin/etcd-v${ETCD_VERSION}-linux-s390x.tar.gz pkg

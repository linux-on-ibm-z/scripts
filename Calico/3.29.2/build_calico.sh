...
#Description:   The script builds Calico version v3.29.2 on Linux on IBM Z for RHEL (8.8, 8.10, 9.2, 9.4, 9.5), Ubuntu (22.04, 24.04) and SLES (15 SP6).
...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.29.2/build_calico.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.29.2/patch"
GO_INSTALL_URL="https://storage.googleapis.com/golang/${GOLANG_VERSION}"
...
EOF
    ...
    docker tag calico/go-build:${GOBUILD_VERSION}-s390x calico/go-build:${GOBUILD_VERSION} | tee -a "$GOBUILD_LOG"
    docker tag calico/go-build:${GOBUILD_VERSION}-s390x calico/go-build:latest | tee -a "$GOBUILD_LOG"

    if [ $(docker images 'calico/go-build:v0.94' | wc -l) == 2 ]; then
        echo "Successfully built calico/go-build:${GOBUILD_VERSION}" | tee -a "$GOBUILD_LOG"
    else
    ...
    docker tag calico/apiserver:latest-s390x calico/apiserver:${PACKAGE_VERSION}
    docker tag calico/pod2daemon-flexvol:latest-s390x calico/pod2daemon-flexvol:${PACKAGE_VERSION}
    docker tag calico/node-driver-registrar:latest-s390x calico/node-driver-registrar:${PACKAGE_VERSION}
    docker tag calico/csi:latest-s390x calico/csi:${PACKAGE_VERSION}
...
function runTest() {
    ...
    docker build -t bitnami/kubectl:"$K8S_VERSION" . 2>&1 | tee -a "$KUBECTL_LOG"

    if [ $(docker images "bitnami/kubectl:$K8S_VERSION" | wc -l) == 2 ]; then
        echo "Successfully built bitnami/kubectl" | tee -a "$KUBECTL_LOG"
    else
        echo "bitnami/kubectl Build FAILED, Stopping further build !!! Check logs at $KUBECTL_LOG" | tee -a "$KUBECTL_LOG"
        exit 1
    fi
...
calico/pod2daemon-flexvol:${PACKAGE_VERSION}
calico/node-driver-registrar:${PACKAGE_VERSION}
calico/csi:${PACKAGE_VERSION}
...
"rhel-8.8" | "rhel-8.10" | "rhel-9.2" | "rhel-9.4" | "rhel-9.5")
    ...
    sudo yum install -y curl git wget tar gcc glibc.s390x docker-ce docker-ce-cli containerd.io make which patch iproute-devel 2>&1 | tee -a "$LOG_FILE"
    export CC=gcc

...
#Description:   The script builds Calico version v3.29.0 on Linux on IBM Z for RHEL (8.8, 8.10, 9.2, 9.4), Ubuntu (20.04, 22.04, 24.04) and SLES (15 SP5, 15 SP6).
...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.29.0/build_calico.sh
...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.29.0"
ETCD_VERSION="v3.5.6"
GOLANG_VERSION="go1.23.2.linux-s390x.tar.gz"
GOBUILD_VERSION="v0.93"
FORCE="false"
TESTS="false"
export SOURCE_ROOT=$(pwd)
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.29.0/patch"
...
EOF
    ...
    if [ $(docker images 'calico/go-build:v0.93' | wc -l) == 2 ]; then
...
function runTest() {
    ...
    docker build -t bitnami/kubectl:1.29.9 . 2>&1 | tee -a "$KUBECTL_LOG"

    if [ $(docker images 'bitnami/kubectl:1.29.9' | wc -l) == 2 ]; then
        echo "Successfully built calico/dind" | tee -a "$KUBECTL_LOG"
    else
        echo "calico/dind Build FAILED, Stopping further build !!! Check logs at $KUBECTL_LOG" | tee -a "$KUBECTL_LOG"
        exit 1
    fi

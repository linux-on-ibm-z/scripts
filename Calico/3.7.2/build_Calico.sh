...
#Description:   The script builds Calico version v3.7.2 on Linux on IBM Z for Rhel(7.4, 7.5, 7.6), Ubuntu(16.04, 18.04) and SLES(12SP4, 15).
...
#               Build logs can be found in $HOME/Calico_v3.7.2/logs/ . Test logs can be found at $HOME/Calico_v3.7.2/logs/testLog-DATE-TIME.log.
...
NAME_PACKAGE="Calico"
VERSION_PACKAGE="v3.7.2"
CALICO_VERSION="3.7.2"
ETCD_VERSION="3.3.7"
...
# Modify `start-runit`, patching the same
printf -- "\nDownloading patch for node start-runit . . . \n"  | tee -a "$NODE_LOG"
curl  -o "start_runit.diff" $PATCH_URL/start_runit.diff 2>&1 | tee -a "$NODE_LOG"
printf -- "\nApplying patch to start-runit . . . \n"  | tee -a "$NODE_LOG"
patch $GOPATH/src/github.com/projectcalico/node/filesystem/sbin/start_runit start_runit.diff 2>&1 | tee -a "$NODE_LOG"
rm -rf start_runit.diff
...
quay.io/calico/node-s390x:v3.7.2
calico/node:v3.7.2
quay.io/calico/cni-s390x:v3.7.2
calico/cni:latest

...
#
#Download build script :   wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.22.2/build_calico.sh
...
PACKAGE_NAME="calico"
PACKAGE_VERSION="v3.22.2"
ETCD_VERSION="v3.5.3"
GOLANG_VERSION="go1.17.8.linux-s390x.tar.gz"
FORCE="false"
TESTS="false"
CURDIR="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Calico/3.22.2/patch"
...
EOF
    ...
    git clone -b v0.65.1 https://github.com/projectcalico/go-build $GOPATH/src/github.com/projectcalico/go-build 2>&1 | tee -a "$GOBUILD_LOG"
    cd $GOPATH/src/github.com/projectcalico/go-build
    printf -- "\nApplying patch for go-build Makefile ... \n" | tee -a "$GOBUILD_LOG"
    curl -s $PATCH_URL/go-build.patch | patch -p1 2>&1 | tee -a "$GOBUILD_LOG"
    # Then build `calico/go-build-s390x:v0.65.1` image
    ARCH=s390x VERSION=v0.65.1 ARCHIMAGE='$(DEFAULTIMAGE)' make image | tee -a "$GOBUILD_LOG"
    if grep -Fxq "Successfully tagged calico/go-build:v0.65.1" $GOBUILD_LOG; then
        echo "Successfully built calico/go-build:v0.65.1" | tee -a "$GOBUILD_LOG"
    else
    ...
    cp $GOPATH/src/github.com/projectcalico/calico/felix/fv/Dockerfile.wgtool.amd64 $GOPATH/src/github.com/projectcalico/calico/felix/fv/Dockerfile.wgtool.s390x

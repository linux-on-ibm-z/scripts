...
token_request=$(curl -X POST https://iam.cloud.ibm.com/identity/token \
  ...
  -d "grant_type=urn%3Aibm%3Aparams%3Aoauth%3Agrant-type%3Aapikey&apikey=$cos-access-service-id-api-key-z-team")
...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/gzip" -T $1 "https://s3.au-syd.cloud-object-storage.appdomain.cloud/currency-automation-toolci-bucket-s390x/$PACKAGE_NAME/$VERSION/$1")

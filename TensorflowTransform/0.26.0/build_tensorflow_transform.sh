...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/TensorflowTransform/0.26.0/build_tensorflow_transform.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/TensorflowTransform/0.26.0/patch/tft.patch"
PATCH_ARROW_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/TensorflowTransform/0.26.0/patch/arrow.patch"
TENSORFLOW_INSTALL_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Tensorflow/2.3.1/build_tensorflow.sh"
...
function cleanup() {
	if [ -f "$CURDIR/cmake-3.19.2.tar.gz" ]; then
		rm "$CURDIR/cmake-3.19.2.tar.gz"
	fi
...
function configureAndInstall() {
	...
	export PATH=$JAVA_HOME/bin:$PATH:$CURDIR/bazel-s390x/output/
	

	printf -- 'TensorFlow installed successfully \n'

	# Build CMake 3.19.2 for Ubuntu 18.04
	if [[ "$DISTRO" == "ubuntu-18.04" ]]; then
		printf -- 'Installing CMake 3.16.3 ... \n'
		
		# Check if CMake directory exists
		cd "${CURDIR}"
		if [ -d "$CURDIR/cmake-3.19.2" ]; then
			rm -rf $CURDIR/cmake-3.19.2
		fi

		wget -O cmake-3.19.2.tar.gz https://cmake.org/files/v3.19/cmake-3.19.2.tar.gz
	...
	curl -o arrow.patch $PATCH_ARROW_URL
	git apply --ignore-whitespace arrow.patch
	mkdir -p cpp/release
	cd cpp/release
	...
	sudo pip3 install 'avro-python3==1.9.1' 'setuptools>=41.0.0' 'Cython>=0.29' 'httplib2<0.18.0,>=0.8'
	...
	sudo env "PATH=${PATH}" python3 setup.py bdist_wheel
	sudo pip3 install dist/*.whl
...
"ubuntu-18.04")
	...
	sudo apt-get update
	sudo apt-get install -y wget build-essential libffi-dev libjemalloc-dev libboost-dev libboost-filesystem-dev libboost-system-dev libboost-regex-dev autoconf automake libtool make unzip python3 python3-pip flex bison |& tee -a "$LOG_FILE"
...
"ubuntu-20.04")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo apt-get install -y wget build-essential cmake libffi-dev libjemalloc-dev libboost-dev libboost-filesystem-dev libboost-system-dev libboost-regex-dev autoconf automake libtool make unzip python3 python3-pip flex bison |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

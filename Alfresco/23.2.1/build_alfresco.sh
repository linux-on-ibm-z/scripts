...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Alfresco/23.2.1/build_alfresco.sh
...
COMMUNITY_REPO_VERSION="23.2.0.60"
COMMUNITY_SHARE_VERSION="23.2.0.72"
SEARCH_SERVICES_COMMIT="bda494d06127a7a4f6fe92d6bca6288139ba190e"
SEARCH_SERVICES_VERSION="2.0.9.1"
ACS_NGINX_VERSION="3.4.2"
TRANSFORM_CORE_VERSION="5.1.0"
CONTENT_APP_VERSION="4.4.1"
ACS_DOCKER_COMPOSE_VERSION="v8.0.0"
...
function installJava() {
    ...
    if [[ $JAVA_PROVIDED =~ ^Temurin || $JAVA_PROVIDED =~ ^IBMSemeru ]]; then
        ...
        printf "%s is not supported, Please use valid java from {Temurin17, IBMSemeru17, OpenJDK17} only\n" "$JAVA_PROVIDED"
        exit 1
    fi
...
function configureAndInstall() {
    ...
    grep -RiIl 'jre17-rockylinux8-202306121108' | xargs sed -i "s/jre17-rockylinux8-202306121108/jre${jver}-ubi8/g"
    ...
    curl -sSL https://github.com/Alfresco/alfresco-content-app/commit/3e0ce59eec4487e9b51334280260ec83bd7c138d.patch | git apply -
...
function isValidJavaProvided() {
    local jp=$1
    case "$jp" in
    "Temurin17" | "IBMSemeru17" | "OpenJDK17")
        return 0
        ;;
    esac
    return 1

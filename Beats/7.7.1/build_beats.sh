...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/7.7.1/build_beats.sh
...
#PATCH_URL
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/7.7.1/patch"
...
function configureAndInstallPython() {
	...
	if [[ "${ID}" == "sles" ]]
		...
		then
			...
			"8.1" | "8.2")
				sudo /usr/sbin/update-alternatives --install /usr/bin/python3 python3 /usr/local/bin/python3.8 10
				sudo /usr/sbin/update-alternatives --set python3 /usr/local/bin/python3.8
				sudo /usr/sbin/update-alternatives --display python3
				;;
			esac
		fi
...
function configureAndInstall() {
	...
	curl -SLO https://dl.google.com/go/go1.13.9.linux-s390x.tar.gz
	chmod ugo+r go1.13.9.linux-s390x.tar.gz
	sudo tar -C /usr/local -xzf go1.13.9.linux-s390x.tar.gz
  ...
  #Building journalbeat and adding to usr/bin
	...
	#Apply fixes and patches for auditbeat
	pkgPatches
...
function pkgPatches(){
	...
	printf -- 'change files with patch <patch_diff>\n'
	wget $PATCH_URL/audit.patch -O - | sudo patch

	sudo chmod a+w+x rule/
	cd rule/
	printf -- 'change files with patch <patch_diff>\n'
	wget $PATCH_URL/binary.patch -O - | sudo patch
	
	#Creating endian.go
	printf -- 'Create endian.go\n'
	sudo wget $PATCH_URL/endian.go -P $GOPATH/pkg/mod/github.com/elastic/go-libaudit@v0.4.0

	##Clean and compile auditbeat again for auditbeat binary to reflect changes
	cd $GOPATH/src/github.com/elastic/beats/auditbeat
  make clean	
	make auditbeat
	make update
...
"ubuntu-16.04" | "ubuntu-18.04")
	...
	sudo apt-get install -y git curl make wget tar gcc libcap-dev libpcap0.8-dev openssl libssh-dev acl rsync tzdata patch fdclone libsystemd-dev libjpeg-dev python3-venv|& tee -a "${LOG_FILE}"
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo zypper install -y git curl gawk make wget tar gcc libpcap1 libpcap-devel git libopenssl-devel libffi48-devel acl patch systemd-devel libjpeg62-devel  |& tee -a "${LOG_FILE}"
	#Installing Python 3.8.3

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/8.11.1/build_beats.sh
...
function gettingStarted() {
    ...
    printf -- '\nFor more information visit https://www.elastic.co/guide/en/beats/libbeat/8.7/getting-started.html \n'
    printf -- '*************************************************************************************************\n'
    printf -- '\n'
}
...
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y git curl make wget tar devtoolset-7-gcc-c++ devtoolset-7-gcc libpcap libpcap-devel which acl zlib-devel patch systemd-devel libjpeg-develbzip2-devel gcc gcc-c++ gdbm-devel libdb-devel libffi-devel libuuid-devel ncurses-devel readline-devel sqlite-devel tk-devel xz xz-devel |& tee -a "${LOG_FILE}"

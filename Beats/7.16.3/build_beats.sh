...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/7.16.3/build_beats.sh
...
function configureAndInstall() {
        ...
        wget --no-check-certificate https://go.dev/dl/go1.17.6.linux-s390x.tar.gz
        chmod ugo+r go1.17.6.linux-s390x.tar.gz
        sudo tar -C /usr/local -xzf go1.17.6.linux-s390x.tar.gz
...
function fileChanges(){
        ...
        wget --no-check-certificate $PATCH_URL/beats.patch -O - | git apply
...
"rhel-8.2" | "rhel-8.4" | "rhel-8.5" )
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y git curl make wget tar gcc libpcap-devel openssl openssl-devel which acl zlib-devel patch  systemd-devel libjpeg-devel python38-devel |& tee -a "${LOG_FILE}"
        python3 -V
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/9.5.4/build_beats.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/9.5.4/patch"
GO_VERSION="1.26.5"
...
function cleanup() {
    # Remove artifacts
    cd "$CURDIR"
    sudo rm -rf Python-3.9.13* data go1* rustup-init.sh
...
function configureAndInstallPython() {
    ...
    ./configure --prefix=/opt/python3.9 --enable-optimizations --with-lto
    make -j$(nproc)
    sudo make install
...
function configureAndInstall() {
    ...
    if [[ "${ID}" != "ubuntu" ]]; then
        sudo update-alternatives --install /usr/bin/s390x-linux-gnu-gcc s390x-linux-gnu-gcc /usr/bin/gcc 10
...
"rhel-8.10" | "rhel-9.6" | "rhel-9.7" | "rhel-9.8" | "rhel-10.0" | "rhel-10.1" | "rhel-10.2")
    ...
    sudo yum install -y git curl make wget tar gcc gcc-c++ libpcap-devel openssl openssl-devel which acl zlib-devel patch systemd-devel libjpeg-devel python3-devel bzip2-devel gdbm-devel libdb-devel libffi-devel libuuid-devel ncurses-devel readline-devel sqlite-devel tk-devel xz xz-devel  |& tee -a "${LOG_FILE}"

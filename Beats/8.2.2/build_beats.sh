...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Beats/8.2.2/build_beats.sh
...
function configureAndInstallPython() {
    if [[ "${DISTRO}" == "ubuntu-18.04" ]] || [[ "${DISTRO}" == "rhel-7."* ]] || [[ "${DISTRO}" == "sles-12.5" ]]; then
...
function configureAndInstall() {
    ...
    sudo tar -C /usr/local -xzf go1.17.9.linux-s390x.tar.gz
    ...
    if [[ "${DISTRO}" == "ubuntu-20.04" ]] || [[ "${DISTRO}" == "sles-12.5" ]]; then
    ...
    # heartbeat is not supported on ubuntu20.04 or RHEL 7.x
    if [[ "${DISTRO}" != "ubuntu-20.04" ]] && [[ "${DISTRO}" != "rhel-7."* ]]; then
...
function gettingStarted() {
        ...
        printf -- '\nFor more information visit https://www.elastic.co/guide/en/beats/libbeat/8.2/getting-started.html \n'
        printf -- '*************************************************************************************************\n'
        printf -- '\n'
}
...
"ubuntu-20.04")
        ...
        printf -- "Installing dependencies... it may take some time.\n"
        sudo apt-get update
        sudo DEBIAN_FRONTEND=noninteractive TZ=America/Toronto apt-get install -y git curl make wget tar gcc g++ libcap-dev libpcap0.8-dev openssl libssh-dev acl rsync tzdata patch fdclone libsystemd-dev libjpeg-dev libffi-dev libpython3-dev python3.8 python3.8-dev python3.8-venv python3.8-distutils python3-lib2to3 python3-testresources |& tee -a "${LOG_FILE}"
...
"sles-15.3" )
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y git curl gawk make wget tar gcc gcc-c++ libpcap libpcap-devel acl patch libsystemd0 systemd-devel libjpeg62-devel openssl libopenssl-devel zlib-devel python39 python39-devel |& tee -a "${LOG_FILE}"

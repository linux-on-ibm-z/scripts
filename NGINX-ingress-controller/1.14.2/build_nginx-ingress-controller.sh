...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX-ingress-controller/1.14.2/build_nginx-ingress-controller.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX-ingress-controller/${PACKAGE_VERSION}/patch"
...
EOF
    ...
    printf -- "Building test-runner image.\n"
    ...
    printf -- "Building nginx-ingress-controller image.\n"
    cd "$GOPATH/src/k8s.io/ingress-nginx/"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd "$GOPATH/src/k8s.io/ingress-nginx/"
        make E2E_IMAGE="localhost:5000/e2e-test-runner:v${NGINX_VERSION}" test
        printf -- 'Running lua tests \n\n'
        make E2E_IMAGE="localhost:5000/e2e-test-runner:v${NGINX_VERSION}" lua-test
    fi

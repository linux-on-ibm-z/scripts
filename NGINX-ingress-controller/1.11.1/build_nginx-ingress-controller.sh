...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX-ingress-controller/1.11.1/build_nginx-ingress-controller.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX-ingress-controller/1.11.1/patch"
...
function configureAndInstall() {
    ...
    git clone -b controller-v$PACKAGE_VERSION https://github.com/kubernetes/ingress-nginx.git --depth 1
    cd ingress-nginx/
    ...
    make build
    docker tag local/e2e-test-runner:"v$(date +%Y%m%d)-7b37df8" gcr.io/ingress-nginx/e2e-test-runner:v$PACKAGE_VERSION
...
"sles-12.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl git make which wget libcurl-devel nghttp2-devel |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"sles-15.5" | "sles-15.6")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"

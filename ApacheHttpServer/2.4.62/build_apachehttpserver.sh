...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheHttpServer/2.4.62/build_apachehttpserver.sh
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-8.8" ]] || [[ "$ID-$VERSION_ID" == "rhel-8.10" ]]; then
         sudo alternatives --set python /usr/bin/python2
	fi
	
	#Creating daemon user for sles 15.x
	if [[ "$ID-$VERSION_ID" == "sles-15.5" || "$ID-$VERSION_ID" == "sles-15.6" ]] ; then

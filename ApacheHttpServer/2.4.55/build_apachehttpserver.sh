...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheHttpServer/2.4.55/build_apachehttpserver.sh
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-8.4" ]] || [[ "$ID-$VERSION_ID" == "rhel-8.6" ]] || [[ "$ID-$VERSION_ID" == "rhel-8.7" ]]; then
         sudo alternatives --set python /usr/bin/python2
	fi
	
	if [[ "$ID-$VERSION_ID" == "sles-15.4" ]]; then
         sudo ln -s /usr/bin/python3 /usr/bin/python
	fi
	
	#Creating daemon user for sles 15 SP4
	if [[ "$ID-$VERSION_ID" == "sles-15.4" ]]; then
...
"ubuntu-22.04" | "ubuntu-22.10")
	...
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
"rhel-7.8" | "rhel-7.9" | "rhel-9.0" | "rhel-9.1")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for HTTP server from repository \n' |& tee -a "$LOG_FILE"
	sudo yum install -y git openssl openssl-devel python gcc libtool autoconf make pcre pcre-devel libxml2 libxml2-devel expat-devel file which wget tar |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheHttpServer/2.4.68/build_apachehttpserver.sh
# Execute build script: bash build_apachehttpserver.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="apachehttpserver"
PACKAGE_VERSION="2.4.68"
APR_VERSION="1.7.6"
...
"ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.10")
    ...
    sudo update-alternatives --install /usr/bin/python python /usr/bin/python3 1
    sudo update-alternatives --install /usr/bin/autoconf autoconf /usr/bin/autoconf2.69 1
	sudo update-alternatives --set autoconf /usr/bin/autoconf2.69
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

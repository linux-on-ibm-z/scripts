...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheHttpServer/2.4.48/build_apachehttpserver.sh
# Execute build script: bash build_apachehttpserver.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="apachehttpserver"
PACKAGE_VERSION="2.4.48"
APR_VERSION="1.6.5"
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-8.2" ]] || [[ "$ID-$VERSION_ID" == "rhel-8.3" ]] || [[ "$ID-$VERSION_ID" == "rhel-8.4" ]]; then

...
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Keystone/29.1.0/build_keystone.sh
# Execute build script: bash build_keystone.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="keystone"
PACKAGE_VERSION="29.1.0"
...
"rhel-8.10")
	...
	sudo dnf install -y python3.12 python3.12-pip python3.12-devel gcc gcc-c++ make rust cargo openssl-devel libffi-devel cmake git wget gawk
	configureLibexpat
	python3.12 -m pip install -U pip setuptools wheel
	python3.12 -m pip install "cryptography<44.0.0"

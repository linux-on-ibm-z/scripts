...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/6.4.0/build_bazel.sh
...
NETTY_TCNATIVE_PREVIOUS_VERSION="2.0.50" # version required by netty
...
function buildNetty() {
    ...
    if [[ $DISTRO == ubuntu-22.04 || $DISTRO == ubuntu-23.04 || $DISTRO == ubuntu-23.10 ]]; then
        curl -sSL $PATCH_URL/netty-tcnative-gcc_$NETTY_TCNATIVE_VERSION.patch | patch -p1 || error "Patch netty tcnative gcc"
    fi
    mvn install -DskipTests

    # Install netty-tcnative 2.0.50 - required by netty 4.1.75 (below)
...
function configureAndInstall() {
    ...
    cd rules_java
    curl -sSL $PATCH_URL/rules_java_5.5.1.patch | git apply || error "Patch rules_java v5.5.1"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/7.2.1/build_bazel.sh
...
function configureAndInstall() {
    ...
    cd rules_java
    curl -sSL $PATCH_URL/rules_java_7.6.1.patch | git apply || error "Patch rules_java v7.6.1"
    
    # Download and patch rules_graalvm v0.11.1
    printf -- '\nDownload and patch rules_graalvm v0.11.1... \n'
    cd $SOURCE_ROOT
    git clone -b v0.11.1 https://github.com/sgammon/rules_graalvm
    cd rules_graalvm
    curl -sSL $PATCH_URL/rules_graalvm_0.11.1.patch | git apply || error "Patch rules_graalvm v0.11.1"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd $SOURCE_ROOT/bazel
        ./output/bazel --host_jvm_args=-Xmx2g test -c opt --build_tests_only --flaky_test_attempts=1 --test_timeout=3600 --show_progress_rate_limit=5 --terminal_columns=143 --show_timestamps --verbose_failures \
...
"ubuntu-20.04")
    ...
    sudo apt-get install -y --no-install-recommends \
        ...
        lcov less libssl-dev lsb-release netcat-openbsd openjdk-11-jdk-headless openjdk-21-jdk-headless zip zlib1g-dev unzip wget python2 \
...
"ubuntu-22.04")
    ...
    sudo apt-get install -y --no-install-recommends \
        bind9-host build-essential coreutils curl dnsutils ed expect file git gnupg2 iproute2 iputils-ping mkisofs \
        lcov less libssl-dev lsb-release netcat-openbsd openjdk-11-jdk-headless openjdk-21-jdk-headless zip zlib1g-dev unzip wget python2 python3 \

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/8.4.2/build_bazel.sh
...
SOURCE_ROOT="$(pwd)"
USER="$(whoami)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/8.4.2/patch"
...
function buildNetty() {
    ...
    if [[ $DISTRO =~ ^ubuntu-(24|25)\.04$ ]]; then
...
"ubuntu-24.04" | "ubuntu-25.04")
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        ...
        less libssl-dev lsb-release netcat-openbsd openjdk-21-jdk-headless zip zlib1g-dev unzip wget python3 gcc-13 g++-13 cmake |& tee -a "${LOG_FILE}"
    sudo ln -sf /usr/bin/python3 /usr/bin/python

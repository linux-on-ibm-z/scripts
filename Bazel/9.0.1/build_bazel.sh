...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/9.0.1/build_bazel.sh
...
SOURCE_ROOT="$(pwd)"
USER="$(whoami)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Bazel/9.0.1/patch"
...
function configureAndInstall() {
	...
	curl -sSL "${PATCH_URL}/protobuf.patch" | git apply --ignore-whitespace -
	cd third_party
	wget "${PATCH_URL}/rules_fuzzing.patch"
	...
	${SOURCE_ROOT}/dist/bazel/output/bazel build --java_runtime_version=additionaljdk --tool_java_runtime_version=additionaljdk --stamp --embed_label "9.0.1" //src:bazel
    mkdir -p output
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
     ...
     ./output/bazel --host_jvm_args=-Xmx2g test --lockfile_mode=update -c opt --build_tests_only --flaky_test_attempts=1 --test_timeout=3600 --show_progress_rate_limit=5 --terminal_columns=143 --show_timestamps --verbose_failures --java_runtime_version=additionaljdk --tool_java_runtime_version=additionaljdk \
-- //scripts/... //src/java_tools/... //src/main/starlark/tests/builtins_bzl/... //src/test/... //src/tools/execlog/... //src/tools/one_version/... //src/tools/singlejar/... //src/tools/workspacelog/... //third_party/ijar/... //tools/aquery_differ/... //tools/python/...
...
"ubuntu-22.04")
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        ...
        lcov less libssl-dev lsb-release netcat-openbsd openjdk-21-jdk-headless zip zlib1g-dev unzip wget python3 gcc-11 g++-11 openjdk-21-source openjdk-25-jdk clang clang-tools-14|& tee -a "${LOG_FILE}"
...
"ubuntu-24.04" | "ubuntu-25.10")
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
        ...
        less libssl-dev lsb-release netcat-openbsd openjdk-21-jdk-headless zip zlib1g-dev unzip wget python3 gcc-13 g++-13 cmake openjdk-21-source openjdk-25-jdk clang clang-tools-18|& tee -a "${LOG_FILE}"
    sudo ln -sf /usr/bin/python3 /usr/bin/python

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/24.3.3/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="24.3.3"
CURDIR="$(pwd)"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "IBM_Semeru_8" && "$JAVA_PROVIDED" != "IBM_Semeru_11" && "$JAVA_PROVIDED" != "Eclipse_Adoptium_Temurin_11" && "$JAVA_PROVIDED" != "OpenJDK11" ]];
    then
        printf --  "$JAVA_PROVIDED is not supported, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, Eclipse_Adoptium_Temurin_11, OpenJDK11 } only." |& tee -a "$LOG_FILE"
        exit 1
    fi
}
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "IBM_Semeru_11" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.14.1%2B1_openj9-0.30.1/ibm-semeru-open-jdk_s390x_linux_11.0.14.1_1_openj9-0.30.1.tar.gz
        sudo tar -C /opt/java -xzf ibm-semeru-open-jdk_s390x_linux_11.0.14.1_1_openj9-0.30.1.tar.gz --strip 1
        ...
        cd "$SOURCE_ROOT"
        sudo wget https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u322-b06_openj9-0.30.0/ibm-semeru-open-jdk_s390x_linux_8u322b06_openj9-0.30.0.tar.gz
        sudo tar -C /opt/java -xzf ibm-semeru-open-jdk_s390x_linux_8u322b06_openj9-0.30.0.tar.gz --strip 1
        export JAVA_HOME=/opt/java
        ...
        cd "$SOURCE_ROOT"
        sudo wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.14.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.14.1_1.tar.gz
        sudo tar -C /opt/java -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.14.1_1.tar.gz --strip 1
        export JAVA_HOME=/opt/java
        ...
        printf --  '$JAVA_PROVIDED is not supported, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, Eclipse_Adoptium_Temurin_11, OpenJDK11} only' >> "$LOG_FILE"
        exit 1
    fi

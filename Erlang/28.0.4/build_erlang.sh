...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/28.0.4/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="28.0.4"
CURDIR="$(pwd)"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "IBM_Semeru_11" ]]; then
        ...
        if [[ "$DISTRO" == "ubuntu-25.04" ]]; then
            printf -- 'IBM Semeru runtime is not supported on Ubuntu 25.04, Please use OpenJDK or Eclipse Adoptium Temurin runtime instead.\n' >> "$LOG_FILE"
            exit 1
        fi

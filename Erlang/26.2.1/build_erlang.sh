...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/26.2.1/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="26.2.1"
CURDIR="$(pwd)"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "IBM_Semeru_8" && "$JAVA_PROVIDED" != "IBM_Semeru_11"  && "$JAVA_PROVIDED" != "IBM_Semeru_17" && "$JAVA_PROVIDED" != "Eclipse_Adoptium_Temurin_11" && "$JAVA_PROVIDED" != "Eclipse_Adoptium_Temurin_17" && "$JAVA_PROVIDED" != "OpenJDK11"  && "$JAVA_PROVIDED" != "OpenJDK17" && "$JAVA_PROVIDED" != "OpenJDK8" ]];
    then
        printf --  "$JAVA_PROVIDED is not supported, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, OpenJDK8, OpenJDK11, OpenJDK17 } only." |& tee -a "$LOG_FILE"
        exit 1
    fi
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK17" ]]; then
      if [[ "$DISTRO" == "rhel-7."* || "$DISTRO" == "sles-12.5" ]]; then
        printf --  "$DISTRO does not have support for OpenJDK17, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, OpenJDK8, OpenJDK11 } only. " |& tee -a "$LOG_FILE"
        exit 1
      fi
    fi
}
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "IBM_Semeru_11" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru11.tar.gz https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.20.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_11.0.20.1_1_openj9-0.40.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru17.tar.gz https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.8.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_17.0.8.1_1_openj9-0.40.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru8.tar.gz https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u382-b05_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_8u382b05_openj9-0.40.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O temurin11.tar.gz https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.20.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.20.1_1.tar.gz
        ...
        sudo mkdir -p /opt/java
        cd "$SOURCE_ROOT"
        sudo wget -O temurin17.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.8.1%2B1/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.8.1_1.tar.gz
        ...
        printf --  '$JAVA_PROVIDED is not supported, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, OpenJDK8, OpenJDK11, OpenJDK17} only' >> "$LOG_FILE"
        exit 1
    fi
...
function printHelp() {
    echo
    echo "Usage: "
    echo "bash build_erlang.sh  [-d debug] [-y install-without-confirmation] [-t install and run tests] [-j Java to use from {IBM_Semeru_8, OpenJDK8, IBM_Semeru_11, Eclipse_Adoptium_Temurin_11, OpenJDK11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_17, OpenJDK17}]"

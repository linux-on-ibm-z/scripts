...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/26.2.3/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="26.2.3"
CURDIR="$(pwd)"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK17" ]]; then
      if [[ "$DISTRO" == "rhel-7."* || "$DISTRO" == "sles-12.5" ]]; then
        printf --  "$DISTRO does not have support for OpenJDK17, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, IBM_Semeru_21, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, Eclipse_Adoptium_Temurin_21, OpenJDK8, OpenJDK11 } only. " |& tee -a "$LOG_FILE"
        exit 1
      fi
    elif [[ "$JAVA_PROVIDED" == "OpenJDK21" ]]; then
      if [[ "$DISTRO" == "rhel-7."* || "$DISTRO" == "sles-12.5" ]]; then
        printf --  "$DISTRO does not have support for OpenJDK21, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, IBM_Semeru_21, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, Eclipse_Adoptium_Temurin_21, OpenJDK8, OpenJDK11 } only. " |& tee -a "$LOG_FILE"
        exit 1
      elif [[ "$DISTRO" == "sles-15.5" ]]; then
        printf --  "$DISTRO does not have support for OpenJDK21, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, IBM_Semeru_21, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, Eclipse_Adoptium_Temurin_21, OpenJDK8, OpenJDK11, OpenIDK17 } only. " |& tee -a "$LOG_FILE"
        exit 1
      fi
    fi
}
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "IBM_Semeru_11" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru11.tar.gz https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.22%2B7_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_11.0.22_7_openj9-0.43.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru17.tar.gz https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.10%2B7_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_17.0.10_7_openj9-0.43.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru8.tar.gz https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u402-b06_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_8u402b06_openj9-0.43.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O semeru21.tar.gz https://github.com/ibmruntimes/semeru21-binaries/releases/download/jdk-21.0.2%2B13_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_21.0.2_13_openj9-0.43.0.tar.gz
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O temurin11.tar.gz https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.22%2B7/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.22_7.tar.gz
        ...
        sudo mkdir -p /opt/java
        cd "$SOURCE_ROOT"
        sudo wget -O temurin17.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.10%2B7/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.10_7.tar.gz
...
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.10" | "ubuntu-24.04")
    ...
    ;;
"rhel-7.8" | "rhel-7.9" | "rhel-8.6" | "rhel-8.8" | "rhel-8.9" | "rhel-8.10" | "rhel-9.0"| "rhel-9.2" | "rhel-9.3" | "rhel-9.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/26.0.2/build_erlang.sh
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK17" ]]; then
      if [[ "$DISTRO" == "rhel-7."* || "$DISTRO" == "sles-12.5" ]]; then
        printf --  "$DISTRO does not have support for OpenJDK17, Please use valid java from {IBM_Semeru_8, IBM_Semeru_11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_11, Eclipse_Adoptium_Temurin_17, OpenJDK11 } only. " |& tee -a "$LOG_FILE"
        exit 1
      fi
    fi
}
...
function printHelp() {
    echo
    echo "Usage: "
    echo "bash build_erlang.sh  [-d debug] [-y install-without-confirmation] [-t install and run tests] [-j Java to use from {IBM_Semeru_8, IBM_Semeru_11, Eclipse_Adoptium_Temurin_11, OpenJDK11, IBM_Semeru_17, Eclipse_Adoptium_Temurin_17, OpenJDK17}]"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/28.1.1/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="28.1.1"
CURDIR="$(pwd)"
    ...
    if [[ "$JAVA_PROVIDED" == "IBM_Semeru_11" ]]; then
        ...
        if [[ "$ID" == "rhel" ]]; then
            if [[ "$DISTRO" == "rhel-10.0" ]]; then
                printf -- 'OpenJDK11 is not supported on rhel 10.0, Please use IBM Semeru runtime or Eclipse Adoptium Temurin runtime instead.\n' >> "$LOG_FILE"
                exit 1
	    fi
        ...
        if [[ "$ID" == "rhel" ]]; then
            if [[ "$DISTRO" == "rhel-10.0" ]]; then
                printf -- 'OpenJDK17 is not supported on rhel 10.0, Please use IBM Semeru runtime or Eclipse Adoptium Temurin runtime instead.\n' >> "$LOG_FILE"
                exit 1
	    fi
        ...
        if [[ "$ID" == "rhel" ]]; then
            if [[ "$DISTRO" == "rhel-10.0" ]]; then
                printf -- 'OpenJDK8 is not supported on rhel 10.0, Please use IBM Semeru runtime or Eclipse Adoptium Temurin runtime instead.\n' >> "$LOG_FILE"
                exit 1
	    fi

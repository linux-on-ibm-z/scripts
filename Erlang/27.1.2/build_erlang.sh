...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/27.1.2/build_erlang.sh
# Execute build script: bash build_erlang.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="erlang"
PACKAGE_VERSION="27.1.2"
CURDIR="$(pwd)"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK21" ]]; then
      if [[ "$DISTRO" == "sles-15.5" ]]; then

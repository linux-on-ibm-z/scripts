...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/XMLSec/1.3.6/build_xmlsec.sh
# Execute build script: bash build_xmlsec.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="xmlsec"
PACKAGE_VERSION="xmlsec_1_3_6"
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "ubuntu-20.04" || "$DISTRO" == "ubuntu-22.04" || "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "ubuntu-24.10" ]]; then
        ./autogen.sh
    else
...
case "$DISTRO" in
"ubuntu-20.04" | "ubuntu-24.04" | "ubuntu-24.10")

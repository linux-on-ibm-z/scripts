...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/XMLSec/1.3.0/build_xmlsec.sh
# Execute build script: bash build_xmlsec.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="xmlsec"
PACKAGE_VERSION="xmlsec_1_3_0"
...
function cleanup() {
    # Remove artifacts
	cd $SOURCE_ROOT
	rm -rf  openssl-1.1.1h.tar.gz  openssl-1.1.1h  libxml2 automake 
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "ubuntu-20.04" || "$DISTRO" == "ubuntu-22.04" || "$DISTRO" == "ubuntu-22.10" || "$DISTRO" == "ubuntu-23.04" ]]; then
        ./autogen.sh 
    else
...
case "$DISTRO" in
"ubuntu-20.04" | "ubuntu-22.10" | "ubuntu-23.04")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update
    sudo apt-get install -y git make libtool libxslt1-dev autoconf libssl-dev libtool-bin pkg-config |& tee -a "$LOG_FILE"
...
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y git make libtool libxslt-devel libtool-ltdl-devel diffutils devtoolset-7 texinfo gettext python2-devel wget tar |& tee -a "$LOG_FILE"
...
"sles-12.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y wget libxslt-tools libxml2-tools libxml2-devel git-core make libtool libxslt-devel libopenssl-devel gawk gcc7 gcc7-c++ texinfo gettext transfig |& tee -a "$LOG_FILE" 
...
"sles-15.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y git-core gcc make libtool libxslt-devel libopenssl-devel gawk |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    	;;
*)

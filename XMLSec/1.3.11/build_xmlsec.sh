...
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/XMLSec/1.3.11/build_xmlsec.sh
# Execute build script: bash build_xmlsec.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="xmlsec"
PACKAGE_VERSION="xmlsec_1_3_11"
...
"sles-16.0")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y diffutils git-core gcc make libtool libxslt-devel libopenssl-devel gawk libxmlsec1-openssl1 |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    export PATH=/usr/local/bin:$PATH
    export LD_LIBRARY_PATH=/usr/local/lib
	;;
...
"ubuntu-25.10")
    ...
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update
    sudo apt-get install -y git make libtool libxslt1-dev autoconf libssl-dev libtool-bin pkg-config libxmlsec1-openssl1 |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
        ;;
*)

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/XMLSec/1.3.2/build_xmlsec.sh
# Execute build script: bash build_xmlsec.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="xmlsec"
PACKAGE_VERSION="xmlsec_1_3_2"
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "ubuntu-20.04" || "$DISTRO" == "ubuntu-22.04" || "$DISTRO" == "ubuntu-23.04" || "$DISTRO" == "ubuntu-23.10" ]]; then
        ./autogen.sh 
    else
...
case "$DISTRO" in
"ubuntu-20.04" | "ubuntu-23.04" | "ubuntu-23.10")
...
"sles-12.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y wget libxslt-tools libxmlsec1-openssl1 libxml2-tools libxml2-devel git-core make libtool libxslt-devel libopenssl-devel gawk gcc7 gcc7-c++ texinfo gettext transfig |& tee -a "$LOG_FILE" 

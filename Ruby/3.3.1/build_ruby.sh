...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Ruby/3.3.1/build_ruby.sh
...
#Download the source code
  ...
  cd "${CURDIR}"
  wget http://cache.ruby-lang.org/pub/ruby/3.3/ruby-${PACKAGE_VERSION}.tar.gz
...
"rhel-7.8" | "rhel-7.9")
	...
	configureAndInstall |& tee -a "${LOG_FILE}"
;;
"rhel-8.6" | "rhel-8.8" | "rhel-8.9" | "rhel-8.10" | "rhel-9.0" | "rhel-9.2" | "rhel-9.3" | "rhel-9.4")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Ruby/3.1.3/build_ruby.sh
...
function configureAndInstall()
{
  ...
  cd "${CURDIR}"
  wget http://cache.ruby-lang.org/pub/ruby/3.1/ruby-${PACKAGE_VERSION}.tar.gz
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y devtoolset-7-gcc-c++ devtoolset-7-gcc bison flex openssl-devel readline-devel gdbm-devel make wget tar |& tee -a "${LOG_FILE}"
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	sudo zypper install -y bison flex libopenssl-devel readline-devel gdbm-devel gcc7 make wget tar gawk gzip |& tee -a "${LOG_FILE}"
...
"sles-15.3" | "sles-15.4")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	sudo zypper install -y bison flex libopenssl-devel readline-devel gdbm-devel gcc make wget tar gawk gzip |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
  ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kong/3.9.3/build_kong.sh
...
GO_VERSION=1.24.0
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kong/3.9.3/patch"
...
function configureAndInstall() {
    ...
    # Remove any user-level ~/.bazelrc left over from compiling Bazel from source.
    rm -f "$HOME/.bazelrc"
    bazel --version
...
EOF
    ...
    # Increase Bazel fetch timeouts to avoid git submodule fetch failures in Docker
    cat >> .bazelrc <<'EOF'
startup --host_jvm_args=-Dsun.net.client.defaultConnectTimeout=120000
startup --host_jvm_args=-Dsun.net.client.defaultReadTimeout=120000
common --http_timeout_scaling=5.0
EOF
    make build-venv
    make build-kong

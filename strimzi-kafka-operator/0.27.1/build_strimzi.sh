...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/strimzi-kafka-operator/0.27.1/build_strimzi.sh
...
PRE_PACKAGE_VERSION="0.26.0"
PRE2_PACKAGE_VERSION="0.24.0"
BRIDGE_VERSION="0.21.3"
PRE_BRIDGE_VERSION="0.20.3"
...
function configureAndInstall() {
        ...
        cd $CURDIR
        wget --no-check-certificate $PATCH_URL/rocksdb_v6.19.3.diff
        mv rocksdb rocksdb-6 && cd rocksdb-6/
        git checkout v6.19.3
        cat $CURDIR/rocksdb_v6.19.3.diff | patch -p1 || error "rocksdb_v6.19.3.diff"
        ...
        # Copy locally built rocksdbjni-6.19.3.jar into local maven repo
        ...
        if [[ "$JAVA_PROVIDED" == "IBMSemeru11" ]]; then
                ...
                curl -SL -o adoptjdk.tar.gz https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.13%2B8_openj9-0.29.0/ibm-semeru-open-jdk_s390x_linux_11.0.13_8_openj9-0.29.0.tar.gz
                ...
                curl -SL -o adoptjdk.tar.gz https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.13%2B8/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.13_8.tar.gz
                # Everytime new jdk is downloaded, Ensure that --strip valueis correct
                sudo tar -zxvf adoptjdk.tar.gz -C /opt/adopt/java --strip-components 1
        ...
        curl -LO https://github.com/kubernetes/minikube/releases/download/v1.24.0/minikube-linux-s390x && sudo install minikube-linux-s390x /usr/bin/minikube && rm -rf minikube-linux-s390x
        ...
        if [[ "$PUSH_TO_REGISTRY" == "Y" ]]; then
            ...
            make MVN_ARGS='-DskipTests -DskipITs' java_install
        ...
        if [[ "$PUSH_TO_REGISTRY" == "Y" ]]; then
            make all
            export BRIDGE_IMAGE=$DOCKER_REGISTRY/$DOCKER_ORG/kafka-bridge:$BRIDGE_VERSION
            printf -- "export BRIDGE_IMAGE=$BRIDGE_IMAGE\n" >> "$BUILD_ENV"
        else
        ...
        if [[ $BUILD_SYSTEMTESTS_DEPS == "Y" ]]; then
            ...
            #Build and publish strimzi-kafka-operator v0.26.0 images
            printf -- "Building and pushing Strimzi-kafka-operator v0.26.0 images\n"
            cd $CURDIR
            ...
            wget --no-check-certificate $PATCH_URL/operator_log4j_${PRE_PACKAGE_VERSION}.diff
            if [[ "$JAVA_PROVIDED" == "IBMSemeru11" ]]; then
            ...
            patch -p1 < ../operator_log4j_${PRE_PACKAGE_VERSION}.diff
            if [[ "${DISTRO}" == "sles-12.5" ]]; then
                sed -i 's/"$code" == "404"/"$code" == "404" || "$code" == "000"/g' docker-images/build.sh
            fi
            if [[ "$JAVA_PROVIDED" == "IBMSemeru11" ]]; then
            ...
            #Build and publish strimzi-kafka-bridge v0.20.3 image
            printf -- "Building and pushing Strimzi-kafka-bridge v0.20.3 image\n"
            cd $CURDIR/system_test/${PRE_PACKAGE_VERSION}
            wget --no-check-certificate $PATCH_URL/bridge_${PRE_BRIDGE_VERSION}.diff
            wget --no-check-certificate $PATCH_URL/bridge_log4j_${PRE_BRIDGE_VERSION}.diff
            ...
            patch -p1 < ../bridge_log4j_${PRE_BRIDGE_VERSION}.diff
            make all

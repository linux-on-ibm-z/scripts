...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Protobuf/3.10.0/build_protobuf.sh
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-6.x" || "$ID-$VERSION_ID" == "rhel-8.0" || "$ID-$VERSION_ID" == "sles-15" || "$ID-$VERSION_ID" == "sles-15.1" || "$ID-$VERSION_ID" == "ubuntu-19.04" ]]; then

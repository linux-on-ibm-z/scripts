...
function cleanup() {
	...
	if [ -f "$BUILD_DIR/gcc-4.8.2.tar.bz2" ]; then
		sudo rm "$BUILD_DIR/gcc-4.8.2.tar.bz2"
	fi
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-6.x" ]]; then
		...
		if [ -d "$BUILD_DIR/gcc-4.8.2" ]; then
			sudo rm -rf "$BUILD_DIR/gcc-4.8.2"
		fi

		sudo wget http://ftp.gnu.org/gnu/gcc/gcc-4.8.2/gcc-4.8.2.tar.bz2

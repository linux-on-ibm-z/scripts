...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Protobuf/3.12.4/build_protobuf.sh
...
function configureAndInstall() {
	...
	if [[ "$ID-$VERSION_ID" == "rhel-8.1" || "$ID-$VERSION_ID" == "rhel-8.2" || "$ID-$VERSION_ID" == "sles-15.1" || "$ID-$VERSION_ID" == "ubuntu-20.04" || "$ID-$VERSION_ID" == "sles-15.2" ]]; then
        cd "$CURDIR"

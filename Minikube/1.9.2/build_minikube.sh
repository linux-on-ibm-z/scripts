...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Minikube/1.9.2/build_minikube.sh
...
function configureAndInstall() {
    ...
    MINIKUBE_BINARY_URL=https://storage.googleapis.com/minikube/releases/latest/minikube-linux-s390x
    KUBECTL_BINARY_URL=https://storage.googleapis.com/kubernetes-release/release/$KUBERNETES_VERSION/bin/linux/s390x/kubectl

    cd "$CURDIR"

    # Fix the iptables-legacy missing problem in kubeproxy image.
    echo -e "FROM k8s.gcr.io/kube-proxy:$KUBERNETES_VERSION\nRUN apt-get update && apt-get install -y iptables" | docker build -t k8s.gcr.io/kube-proxy:$KUBERNETES_VERSION -
    ...
    curl -Lo kubectl $KUBECTL_BINARY_URL && chmod +x kubectl && sudo cp kubectl /usr/local/bin/ && rm kubectl
...
"ubuntu-16.04" | "ubuntu-18.04" )
    ...
    sudo apt-get update
    sudo apt-get install -y wget curl conntrack |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"rhel-7.6" | "rhel-7.7" | "rhel-7.8" | "rhel-8.1" )
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y wget curl conntrack |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"sles-12.4" | "sles-12.5" | "sles-15.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y wget curl conntrack-tools
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

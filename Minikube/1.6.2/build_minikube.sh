...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Minikube/1.6.2/build_minikube.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Minikube/1.6.2/patch"
...
function configureAndInstall() {
    ...
    printf -- 'Updated images_test.go : success\n'
    
    curl -o "images.go.diff" "$PATCH_URL/images.go.diff"
    git apply --ignore-space-change --ignore-whitespace  images.go.diff
    printf -- 'Updated images.go : success\n'
    
    curl -o "kube_kubeadm_test.go.diff" "$PATCH_URL/kube_kubeadm_test.go.diff"
    git apply --ignore-space-change --ignore-whitespace  kube_kubeadm_test.go.diff
    printf -- 'Updated kube_kubeadm_test.go : success\n'
    
    curl -o "kubeadm_test.go.diff" "$PATCH_URL/kubeadm_test.go.diff"
    git apply --ignore-space-change --ignore-whitespace  kubeadm_test.go.diff
    printf -- 'Updated kubeadm_test.go : success\n'

    curl -o "kubeadm.go.diff" "$PATCH_URL/kubeadm.go.diff"
    git apply --ignore-space-change --ignore-whitespace  kubeadm.go.diff
    printf -- 'Updated kubeadm.go : success\n'


    cd "$GOPATH/src/k8s.io/minikube"
    make 
    #Add minikube to /usr/bin
    sudo cp ./out/minikube /usr/bin/minikube
...
function gettingStarted() {
    ...
    printf -- "curl -Lo kubectl https://storage.googleapis.com/kubernetes-release/release/v1.17.0/bin/linux/s390x/kubectl && chmod +x kubectl && sudo cp kubectl /usr/local/bin/ && rm kubectl \n"
    ...
    printf -- "sudo -E minikube start --vm-driver=none --kubernetes-version=1.17.0\n"
...
"sles-12.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y tar wget gcc git-core make python curl patch libvirt libvirt-devel libtasn1-devel which
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"sles-15.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"

...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "adoptjdk" && "$JAVA_PROVIDED" != "openjdk" ]]
    then
        err "$JAVA_PROVIDED is not supported, Supported are {adoptjdk, openjdk} only"
        exit 1
    fi

    if [[ "$JAVA_PROVIDED" == "adoptjdk" && "$ID" == "sles" ]]
    then
        err "$ID work with openjdk, provided =$JAVA_PROVIDED"
        exit 1
    fi
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "rhel-8.0" || "$DISTRO" == "rhel-8.1" ]];
    then
...
function printHelp() {
    ...
    echo "       default: -j=adopjdk(for all except sles) or openjdk ( for sles) "
    echo
}

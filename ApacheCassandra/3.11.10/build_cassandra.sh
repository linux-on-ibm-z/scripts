...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/3.11.10/build_cassandra.sh
...
function configureAndInstall() {
    ...
    # Install Ant, required for rhel8 as it does not ship ant, junit, ant-junit
    cd "$CURDIR"
    DISTRO="$ID-$VERSION_ID"
    if [[ "$DISTRO" == "rhel-8.3" || "$DISTRO" == "rhel-8.2" || "$DISTRO" == "rhel-8.1" ]]; then
    ...
    if [[ "$DISTRO" == "ubuntu-20.10" || "$DISTRO" == "ubuntu-20.04" || "$ID" == "sles" ]]; then
        cd "$CURDIR"
        ...
        make
        sudo make install
        if [[ "$DISTRO" == "ubuntu-20.10" || "$DISTRO" == "ubuntu-20.04" ]]; then
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        sed -i 's/timeout =.*/timeout = 100000)/' ${CURDIR}/cassandra/test/unit/org/apache/cassandra/io/sstable/IndexSummaryManagerTest.java
        ant test
...
"rhel-8.3" | "rhel-8.2" | "rhel-8.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y langpacks-en_GB.noarch curl git which gcc-c++ make automake autoconf libtool libstdc++ tar wget patch words libXt-devel libX11-devel unzip python2 |& tee -a "$LOG_FILE"
    sudo ln -sf /usr/bin/python2 /usr/bin/python |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
...
"sles-12.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install --no-recommends -y ant ant-junit junit curl git which make wget tar zip unzip words gcc-c++ patch libtool automake autoconf ccache xorg-x11-proto-devel xorg-x11-devel alsa-devel cups-devel libffi48-devel libstdc++6-locale glibc-locale libstdc++-devel libXt-devel libX11-devel texinfo python |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
...
"sles-15.2")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install --no-recommends -y ant junit ant-junit gzip curl git which make wget tar zip unzip gcc-c++ patch libtool automake autoconf ccache xorg-x11-proto-devel xorg-x11-devel alsa-devel cups-devel libffi-devel libstdc++6-locale glibc-locale libstdc++-devel libXt-devel libX11-devel texinfo python |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

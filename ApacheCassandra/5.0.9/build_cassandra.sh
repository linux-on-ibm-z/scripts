...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/5.0.9/build_cassandra.sh
# Execute build script: bash build_ApacheCassandra.sh    (provide -h for help)
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "sles-"* ]]; then
        mv $CURDIR/amazon-corretto-crypto-provider/build/lib/AmazonCorrettoCryptoProvider.jar \
           $CURDIR/amazon-corretto-crypto-provider/build/lib/AmazonCorrettoCryptoProvider-2.4.1-linux-s390_64.jar
    ...
    if [[ "$DISTRO" != "ubuntu-22.04" && "$DISTRO" != "rhel-9.8" ]]; then
...
function printHelp() {
    echo
    echo "Usage: "
    echo " bash build_ApacheCassandra.sh  [-d debug] [-y install-without-confirmation] [-t install-with-tests] [-j java to use [OpenJDK11, Temurin11, OpenJDK17, Temurin17]]"

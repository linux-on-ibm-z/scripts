...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.70/build_netty.sh
...
function configureAndInstall() {
	...
	if [[ "${ID}" == "ubuntu" ]]; then
        ...
        # Fix APR configuration for Ubuntu 24.04 and SLES 16.0 where apr-1-config is resolved into
        # a malformed include argument during netty-tcnative native compilation.
        if [[ "$DISTRO" == "sles-16.0" ]]; then
                export APR_CONFIG=/usr/bin/apr-1-config
                export APR_INCLUDE_DIR=/usr/include/apr-1
                export APR_LIB_DIR=/usr/lib64
        elif [[ "$DISTRO" == "ubuntu-24.04" ]]; then
                export APR_CONFIG=/usr/bin/apr-1-config
                export APR_INCLUDE_DIR=/usr/include/apr-1.0
                export APR_LIB_DIR=/usr/lib/s390x-linux-gnu
        fi
...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                if [[ "${ID}" == "ubuntu" ]]; then
                        if [[ "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "ubuntu-24.10"  || "$DISTRO" == "ubuntu-25.04" || "$DISTRO" == "ubuntu-25.10" ]]; then
                                cd $SOURCE_ROOT/
        ...
        # Force maven's hawtjni plugin to resolve apr-1-config using /usr prefix to prevent
        # SLES-16.0's path resolution from picking up /bin/apr-1-config and triggering the cross-compile bug.
        if [[ "$DISTRO" == "sles-16.0" ]]; then
        ...
        # Skip javadoc on distros where netty-tcnative fails only during javadoc generation;
        # this does not affect the native build output.
        if [[ "$DISTRO" == "sles-16.0" ]]; then
            export APR_CONFIG=/usr/bin/apr-1-config
            export APR_INCLUDE_DIR=/usr/include/apr-1
            export APR_LIB_DIR=/usr/lib64
...
case "$DISTRO" in
"rhel-8.10" | "rhel-9.4" | "rhel-9.6" | "rhel-9.7" | "rhel-9.8")

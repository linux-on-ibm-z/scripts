...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Semeru11" && "$JAVA_PROVIDED" != "Temurin11" && "$JAVA_PROVIDED" != "OpenJDK11" && "$JAVA_PROVIDED" != "AdoptOpenJDK8" && "$JAVA_PROVIDED" != "OpenJDK8" ]]; then
        printf "$JAVA_PROVIDED is not supported, Please use valid java from {Semeru11, Temurin11, OpenJDK11, AdoptOpenJDK8 and OpenJDK8} only"
        exit 1
    fi
...
function configureAndInstall() {
    ...
    if [[ "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-22.10" || "${DISTRO}" == "ubuntu-23.04" || "${DISTRO}" == "rhel-9"* ]]; then
...
"rhel-7.8" | "rhel-7.9")
    ...
    configureAndInstall |& tee -a "${LOG_FILE}"
    ;;
"rhel-8.6" | "rhel-8.7" | "rhel-8.8" | "rhel-9"*)
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"

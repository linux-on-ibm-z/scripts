...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.1.1/build_cassandra.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.1.1/patch"
...
function java_setup() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin11" ]]; then
        ...
        if [[ "$ID" == "rhel" ]]; then
            ...
            sudo DEBIAN_FRONTEND=noninteractive apt-get install -y openjdk-8-jre openjdk-8-jdk
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "AdoptOpenJDK8" ]]; then
        case "$DISTRO" in
        "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-22.10" | "ubuntu-23.04" | "sles-12.5" | "sles-15.4" | "rhel-9.0" | "rhel-9.1" | "rhel-9.2")

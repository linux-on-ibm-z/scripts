...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.0.6/build_cassandra.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.0.6/patch"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Temurin11" && "$JAVA_PROVIDED" != "OpenJDK11" ]]; then
        err "$JAVA_PROVIDED is not supported, Supported are {Temurin11, OpenJDK11} only"
        exit 1
    fi
...
function printHelp() {
    echo
    echo "Usage: "
    echo " build_cassandra.sh  [-d debug] [-y install-without-confirmation] [-t install-with-tests] [-j java to use [Temurin11, OpenJDK11]]"

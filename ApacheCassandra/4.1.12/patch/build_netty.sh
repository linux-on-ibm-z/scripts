...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK11" && "$ID" == "rhel" && ( "$VERSION_ID" == "10.0" || "$VERSION_ID" == "10.1" ) ]]; then
        printf -- "OpenJDK11 is not supported on RHEL 10.0 and RHEL 10.1. Please use Temurin11.\n"
        printf -- "Example: bash build_netty.sh -j Temurin11\n"
        exit 1
    fi
...
function configureAndInstall() {
    ...
    if [[ "${DISTRO}" == "rhel-9"* || "${DISTRO}" == "rhel-10"* ||"${DISTRO}" == "sles-15.7" || "${DISTRO}" == "sles-16.0" || "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-24"* || "${DISTRO}" == "ubuntu-25"* || "${DISTRO}" == "debian-12" || "${DISTRO}" == "debian-13" ]]; then
    ...
    sed -i 's#https://www.openssl.org/source/old/#https://openssl-library.org/source/old/#g' openssl-static/pom.xml
    ./mvnw clean install

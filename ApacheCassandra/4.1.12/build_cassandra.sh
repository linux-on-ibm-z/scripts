...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.1.12/build_cassandra.sh
...
FORCE="false"
TESTS="false"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/4.1.12/patch"
...
function prepare() {
	...
	if [[ "$JAVA_PROVIDED" == "OpenJDK11" && ( ( "$ID" == "rhel" && "$VERSION_ID" == 10.* ) || ( "$ID" == "sles" && "$VERSION_ID" == "16.0" ) ) ]]; then

		err "OpenJDK11 is not supported on RHEL 10.x and SLES 16.0. Please use Temurin11."
		err "Example: bash build_cassandra.sh -j Temurin11"
		exit 1
	fi
...
function build_python() {
    ...
    if [[ "$ID" == "ubuntu" ]]; then
        sudo apt-get update
        sudo apt-get install -y gcc g++ libbz2-dev libdb-dev libffi-dev libgdbm-dev \
            liblzma-dev libncurses-dev libreadline-dev libsqlite3-dev libssl-dev \
            make tar tk-dev uuid-dev wget xz-utils zlib1g-dev

    elif [[ "$ID" == "rhel" && "$VERSION_ID" == 10.* ]]; then
        sudo yum install -y gcc gcc-c++ make wget tar \
            openssl-devel bzip2-devel libffi-devel zlib-devel \
            xz-devel readline-devel sqlite-devel ncurses-devel

    elif [[ "$ID" == "sles" && "$VERSION_ID" == "16.0" ]]; then
        sudo zypper install -y \
            gcc gcc-c++ make wget tar \
            libopenssl-devel libbz2-devel libffi-devel \
            xz-devel readline-devel sqlite3-devel ncurses-devel
    fi
...
function java_setup() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin11" ]]; then
        ...
        export JAVA_HOME=$CURDIR/jdk-11.0.27+6
...
function configureAndInstall() {
	...
	if [[ "$DISTRO" == "rhel-10."* ]]; then
		sed -i 's/egrep -c/grep -E -c/g' conf/cassandra-env.sh
	fi
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        ANT_OPTS="-Xms4G -Xmx4G" ant test | tee -a "$LOG_FILE"
...
"rhel-10.0" | "rhel-10.1" | "rhel-10.2")
	...
	export CQLSH_PYTHON=/usr/local/bin/python3.11
	echo 'export CQLSH_PYTHON=/usr/local/bin/python3.11' >> "$BUILD_ENV"
...
"sles-15.7")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    sudo zypper install -y gzip curl git which make wget tar zip unzip cmake gcc-c++ patch procps libtool automake autoconf ccache libX11-devel libXext-devel libXrender-devel libXt-devel alsa-devel cups-devel glibc-locale-base libstdc++-devel texinfo python3-devel maven
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

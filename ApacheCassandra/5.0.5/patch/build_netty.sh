...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                if [[ "${ID}" == "ubuntu" ]]; then
                        if [[ "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "ubuntu-24.10"  || "$DISTRO" == "ubuntu-25.04" ]]; then
                                cd $SOURCE_ROOT/
...
"sles-15.6")
    ...
    configureAndInstall |& tee -a "${LOG_FILE}"
    ;;
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.04" )
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo apt-get update -y
        sudo apt-get install -y ninja-build cmake perl golang-1.23 libssl-dev libapr1-dev autoconf automake libtool make tar git wget curl libtool-bin xz-utils gzip python3 |& tee -a "${LOG_FILE}"
	sudo update-alternatives --install /usr/bin/go go /usr/lib/go-1.23/bin/go 100
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;
*)

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheCassandra/5.0.6/build_cassandra.sh
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "rhel-"* || "$DISTRO" == "sles-"* ]]; then
...
"sles-15.7")
    sudo zypper install -y gzip curl git which make wget tar zip unzip gcc14 gcc14-c++ patch libtool automake autoconf ccache xorg-x11-devel alsa-devel cups-devel glibc-locale libstdc++-devel libXt-devel libX11-devel texinfo python311-devel maven cmake go

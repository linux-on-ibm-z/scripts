...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB/10.4.11/build_mariadb.sh
# Execute build script: bash build_mariadb.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="mariadb"
PACKAGE_VERSION="10.4.11"
CURDIR="$(pwd)"
...
function buildGCC() {
    ...
    # Some pkgs like galera expects g++ as c++ compiler
    sudo mv /usr/bin/g++ /usr/bin/g++.bkup
    sudo ln -s /opt/gcc/bin/g++ /usr/bin/g++
...
{
    printf -- "Build python 3.x for rhel-6.x or SLES 12-x\n" >>"$LOG_FILE"
...
"rhel-6.x" | "rhel-7.5" | "rhel-7.6" |"rhel-7.7")
    ...
    printf -- "Installing dependencies... it may take some time.\n"
    if [[ "$DISTRO" == "rhel-6.x" ]]; then
        sudo yum install -y git gcc gcc-c++ xz make which curl wget tar cmake openssl-devel ncurses-devel bison boost-devel check-devel perl-Test-Simple perl-Time-HiRes openssl libpcre3-dev pam-devel patch |& tee -a "$LOG_FILE"
        # python relied on gcc availablity for it's compilation, Don't change the call order
        buildGCC |& tee -a "$LOG_FILE"
        build_python3 |& tee -a "$LOG_FILE"
    else
        sudo yum install -y git gcc gcc-c++ make wget curl tar cmake openssl-devel ncurses-devel bison python3 boost-devel check-devel perl-Test-Simple perl-Time-HiRes openssl libpcre3-dev pam-devel patch |& tee -a "$LOG_FILE"
    fi
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
...
"sles-12.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y git gcc gcc-c++ make which wget tar gzip cmake libopenssl-devel ncurses-devel bison glibc-locale python3 boost-devel check-devel gawk pam-devel patch |& tee -a "$LOG_FILE"
...
"sles-15.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y git gcc gcc-c++ make wget tar gzip cmake libopenssl-devel ncurses-devel bison glibc-locale python3 boost-devel libboost_program_options-devel check-devel gawk pam-devel patch |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

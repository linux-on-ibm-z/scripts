...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB/10.11.6/build_mariadb.sh
# Execute build script: bash build_mariadb.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="mariadb"
PACKAGE_VERSION="10.11.6"
CURDIR="$(pwd)"
...
"ubuntu-23.10")
    ...
    sudo apt-get update
    sudo apt-get install -y make wget curl tar cmake openssl bison git gcc g++ libssl-dev libncurses-dev bison libboost-dev libboost-program-options-dev check libpam0g-dev zlib1g-dev python3 python3-pip |& tee -a "$LOG_FILE"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB/10.5.8/build_mariadb.sh
...
function configureAndInstall() {
    ...
    sudo scripts/mysql_install_db $MARIADB_NO_DEFAULTS --user=mysql
...
function gettingStarted() {
    ...
    printf -- "sudo /usr/local/mysql/bin/mysqld_safe $MARIADB_NO_DEFAULTS --user=mysql & \n"
...
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    MARIADB_NO_DEFAULTS="--no-defaults"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SaltStack/3007.7/build_salt.sh
# Execute build script: bash build_salt.sh    (provide -t for test)
#
set -e -o pipefail
PACKAGE_NAME="salt"
PACKAGE_VERSION="3007.7"
PYTHON_VERSION="3.10.18"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SaltStack/3007.7/patch/"
CURDIR="$PWD"
...
function configureAndInstall()
{
  printf -- 'Configuration and Installation started \n'
	...
	if [[ "$ID-$VERSION_ID" == "sles-15.6" ]]; then
	  ...
	  cd openssl-3.2.2
	  ./Configure --prefix=/usr/local/openssl-3.2.2 --openssldir=/usr/local/openssl-3.2.2 shared zlib
	  make
      sudo make install
	  export PATH=/usr/local/openssl-3.2.2/bin:$PATH
	  export LD_LIBRARY_PATH=/usr/local/openssl-3.2.2/lib64:$LD_LIBRARY_PATH
	fi

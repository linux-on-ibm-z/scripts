...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.11.2/build_kibana.sh
# Execute build script: bash build_kibana.sh    (provide -h for help)
#
set -e -o pipefail

PACKAGE_NAME="kibana"
PACKAGE_VERSION="7.11.2"
...
function cleanup() {
	sudo rm -rf "${CURDIR}/node-v14.16.0-linux-s390x.tar.gz" "${CURDIR}/gcc-4.9.4.tar.gz" "${CURDIR}/kibana"
...
"ubuntu-18.04" | "ubuntu-20.04")
	...
	sudo apt-get update
	sudo apt-get install -y curl git g++ gzip libssl-dev make python tar wget patch pkg-config libpixman-1-dev libcairo2-dev libpangocairo-1.0-0 libpango1.0-dev libjpeg-dev libre2-dev xz-utils |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

"rhel-7.8" | "rhel-7.9")
	printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo yum install -y autoconf automake bzip2 gcc-c++ git gzip libtool make openssl-devel python tar wget zlib-devel curl pixman-devel.s390x cairo cairo-devel cairomm-devel libjpeg-turbo-devel pango pango-devel pangomm pangomm-devel giflib-devel xz |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;
"rhel-8.1" | "rhel-8.2"| "rhel-8.3")
	printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo yum install -y autoconf automake bzip2 gcc-c++ git gzip libtool make openssl-devel python2 tar wget zlib-devel curl pkgconf-pkg-config pixman-devel.s390x cairo cairo-devel cairomm-devel libjpeg-turbo-devel pango pango-devel pangomm pangomm-devel giflib-devel xz |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

"sles-15.2")
	printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo zypper install -y autoconf automake bzip2 gcc-c++ git gzip libtool make openssl-devel python2 tar wget zlib-devel curl libjpeg8-devel libpixman-1-0-devel cairomm1_0-devel pangomm1_4-devel pkg-config giflib-devel xz |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;
*)

declare -a configs=(
    ...
    "src/platform/packages/private/kbn-data-streams/jest.integration.config.js"
    ...
    "src/platform/plugins/shared/workflows_execution_engine/integration_tests/jest.integration.config.js"
    "x-pack/platform/packages/shared/kbn-langgraph-checkpoint-saver/jest.integration.config.js"
    ...
    "x-pack/platform/plugins/shared/automatic_import_v2/jest.integration.config.js"
    ...
    "x-pack/solutions/security/plugins/security_solution/public/detection_engine/rule_creation_ui/components/step_define_rule/__integration_tests__/jest.integration.config.js"
    ...
    "x-pack/solutions/security/plugins/security_solution/public/jest.integration.config.js"
    "x-pack/solutions/security/plugins/security_solution/server/jest.integration.config.js"
)
...
do
    echo "=== ${config}"
    FORCE_COLOR=1 NODE_OPTIONS="--max-old-space-size=12288 --trace-warnings --no-experimental-require-module" node scripts/jest --config "${config}" --coverage=false --passWithNoTests --runInBand --forceExit
    exitCode=$?
    if [[ $exitCode != 0 ]]; then
        failedConfigs+=("${config}")
        echo "--- FAILED ${config}"
    fi
    # https://github.com/elastic/kibana/blob/v9.3.0/x-pack/solutions/security/plugins/security_solution/scripts/run_cypress/parallel.ts#L161-L162
    pkill -f 9220 || true
    pkill -f 5620 || true
    sleep 5
    ps aux

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/9.3.0/build_kibana.sh
...
ENV_VARS=$SOURCE_ROOT/setenv_kibana.sh
...
function buildAndInstallMoonRepo() {
    printf -- "Building and installing MoonRepo support\n"
    cd $SOURCE_ROOT
...
function configureAndInstall() {
    ...
    echo "export RUST_MIN_STACK=$((32 * 1024 * 1024))" >>$ENV_VARS
    export MOON_DISABLE_NATIVE=1
    echo "export MOON_DISABLE_NATIVE=1" >>$ENV_VARS
    export MOON_BINARY_PATH=/usr/local/bin/moon
    echo "export MOON_BINARY_PATH=/usr/local/bin/moon" >>$ENV_VARS
    ...
    if [[ $DISTRO == "rhel-"* || $DISTRO == "ubuntu-22.04" ]]; then
        sed -i '95s/0.34.4/0.34.3/;2043s/0.34.4/0.34.3/' package.json
    fi

    # Bootstrap Kibana
    buildAndInstallMoonRepo
    cd $SOURCE_ROOT/kibana
...
function buildAndInstallSharp() {
    printf -- "Building and installing Sharp support\n"
...
function buildAndInstallParcelWatcher() {
    printf -- "Building and installing ParcelWatcher support\n"
    cd $SOURCE_ROOT
    source $ENV_VARS
...
function runTest() {
    ...
    if [[ $HAVE_ES_DIST == "true" ]]; then
        export KBN_ES_SNAPSHOT_URL="$ES_DIST"
        echo "export KBN_ES_SNAPSHOT_URL=$KBN_ES_SNAPSHOT_URL" >>$ENV_VARS
    fi
...
"rhel-8.10" | "rhel-9.4" | "rhel-9.6" | "rhel-9.7")
    ...
    sudo dnf install -y --allowerasing curl git gcc-c++ gzip make python${PYVER} java-21-openjdk-devel unzip zip tar wget \
    patch xz pkg-config expat-devel glib2-devel meson ninja-build brotli cmake clang clang-devel llvm-devel perl coreutils \
    ed expect file gnupg2 iproute iputils less openssl-devel python${PYVER}-devel python${PYVER}-pip python${PYVER}-requests \
    python${PYVER}-setuptools python${PYVER}-six python${PYVER}-wheel python${PYVER}-pyyaml zlib-devel |& tee -a "${LOG_FILE}"
...
"sles-15.7")
    printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
    SLES_GCC_VERSION="14"
    sudo zypper install -y curl git gcc-c++ gcc${SLES_GCC_VERSION}-c++ gzip make python311 java-21-openjdk-devel unzip zip tar \
    wget patch xz which gawk pkg-config glib2-devel libexpat-devel cmake meson ninja gobject-introspection-devel procps coreutils \
    ed expect file iproute2 iputils less libopenssl-devel python311-devel python311-pip python311-requests cmake clang llvm-clang-devel \
    python311-setuptools python311-six python311-wheel unzip zlib-devel python311-python-gnupg python311-PyYAML |& tee -a "${LOG_FILE}"
...
"ubuntu-22.04" | "ubuntu-24.04")
    printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
    sudo apt-get update
    sudo apt-get install -y curl git gzip make python3 python-is-python3 unzip zip tar wget patch xz-utils cmake libclang-dev clang \

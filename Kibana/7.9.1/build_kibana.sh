...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.9.1/build_kibana.sh
...
function cleanup() {
	sudo rm -rf "${CURDIR}/node-v10.22.0-linux-s390x.tar.gz" "${CURDIR}/gcc-4.9.4.tar.gz" "${CURDIR}/kibana"
...
function runTest() {
	...
	yarn test --force 2>&1 | tee -a ${CURDIR}/test_logs
		...
		if [[ 'grep "Test Suites: 16 failed"  ${CURDIR}/test_logs' ]]; then
			printf -- '**********************************************************************************************************\n'

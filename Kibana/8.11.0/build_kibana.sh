...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/8.11.0/build_kibana.sh
...
function cleanup() {
        ...
        sudo rm -rf "${CURDIR}/libvips-8.14.2-linux-s390x"
        sudo rm -rf "${CURDIR}/v8.14.2.tar.gz"
        sudo rm -rf "${CURDIR}/libvips-8.14.5-linux-s390x"
        sudo rm -rf "${CURDIR}/v8.14.5.tar.gz"
...
function installLibvips() {
        ...
        meson compile
        meson install
        export LD_LIBRARY_PATH="${CURDIR}"/libvips-$LIBVIPS_VER/libvips_install/LIB_DIR:$LD_LIBRARY_PATH
        echo "export LD_LIBRARY_PATH=$LD_LIBRARY_PATH" >> $ENV_VARS

        mkdir "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x && cd "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x
        ...
        cp "${CURDIR}"/libvips-$LIBVIPS_VER/libvips_install/$LIB_DIR/libvips-cpp.so.42 "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x/lib/
        sed -i 's/linux-x64/linux-s390x/g' "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x/platform.json
        cd "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x
        tar -cf libvips-$LIBVIPS_VER-linux-s390x.tar *
        cd "${CURDIR}"
        mkdir v$LIBVIPS_VER && cd v$LIBVIPS_VER
        mv "${CURDIR}"/libvips-$LIBVIPS_VER-linux-s390x/libvips-$LIBVIPS_VER-linux-s390x.tar .
...
function configureAndInstall() {
        ...
        if [[ $DISTRO == "ubuntu-20.04" ]]; then
                ...
                cd meson-0.55.3
                ln -s meson.py meson
                export PATH=$CURDIR/meson-0.55.3:$PATH
                echo "export PATH=$PATH" >> $ENV_VARS
        fi
        ...
        export npm_config_sharp_libvips_local_prebuilds="${CURDIR}"
...
"rhel-9.0")
        ...
        sudo yum install -y --allowerasing python3-bind bind-chroot coreutils-single ed expect file gnupg2 iproute iproute-devel iputils lcov less openssl-devel netcat python3-devel python3-pip python3-requests python3-setuptools python3-six python3-wheel python3-pyyaml zlib-devel |& tee -a "${LOG_FILE}"
        sudo ln -sf /usr/bin/python3 /usr/bin/python
        export JAVA_HOME=/usr/lib/jvm/java-11-openjdk
        export PATH=$JAVA_HOME/bin:$PATH
        configureAndInstall |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/8.16.1/build_kibana.sh
# Execute build script: bash build_kibana.sh    (provide -h for help)
#
set -e -o pipefail

PACKAGE_NAME="kibana"
PACKAGE_VERSION="8.16.1"
...
"ubuntu-24.04")
        printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        sudo apt-get update
        sudo apt-get install -y curl git g++ gzip make python3 openjdk-11-jdk unzip zip tar wget patch xz-utils |& tee -a "${LOG_FILE}"
        sudo apt-get install -y build-essential pkg-config libglib2.0-dev libexpat1-dev meson ninja-build brotli libgirepository1.0-dev  |& tee -a "${LOG_FILE}"
        export JAVA_HOME=/usr/lib/jvm/java-11-openjdk-s390x
        export PATH=$JAVA_HOME/bin:$PATH
        configureAndInstall |& tee -a "${LOG_FILE}"

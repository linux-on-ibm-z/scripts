...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.0.1/build_kibana.sh
...
function cleanup() {
	sudo rm -rf "${WORKDIR}/kibana-${PACKAGE_VERSION}-linux-x86_64.tar.gz" "${WORKDIR}/node-v10.15.2-linux-s390x.tar.gz" "${WORKDIR}/gcc-8.3.0.tar.gz" "${WORKDIR}/gcc_build"
...
function buildGCC() {
	printf -- 'Building GCC \n'
	cd "${CURDIR}"
	wget ftp://gcc.gnu.org/pub/gcc/releases/gcc-8.3.0/gcc-8.3.0.tar.gz
	...
	mkdir -p gcc_build
	cd gcc_build/
	../gcc-8.3.0/configure --prefix="/opt/gcc" --enable-checking=release --enable-languages=c,c++ --disable-multilib
	make
	sudo make install
	export PATH=/opt/gcc/bin:$PATH
	export LD_LIBRARY_PATH='/opt/gcc/lib64'
	sudo ln -sf /opt/gcc/bin/gcc /usr/bin/gcc
	export C_INCLUDE_PATH=/opt/gcc/lib/gcc/s390x-ibm-linux-gnu/8.3.0/include
	export CPLUS_INCLUDE_PATH=/opt/gcc/lib/gcc/s390x-ibm-linux-gnu/8.3.0/include

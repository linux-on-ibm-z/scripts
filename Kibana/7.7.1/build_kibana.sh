...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.7.1/build_kibana.sh
...
function cleanup() {
	sudo rm -rf "${CURDIR}/node-v10.19.0-linux-s390x.tar.gz" "${CURDIR}/gcc-4.9.4.tar.gz" "${CURDIR}/kibana"
...
function configureAndInstall() {
    ...
    wget https://nodejs.org/dist/v10.19.0/node-v10.19.0-linux-s390x.tar.gz
    sudo tar xzvf node-v10.19.0-linux-s390x.tar.gz -C /usr/local/lib/nodejs
    sudo ln -s /usr/local/lib/nodejs/node-v10.19.0-linux-s390x/bin/* /usr/bin/
    node -v  >> "${LOG_FILE}"
...
function runTest() {
	...
	if [[ 'grep "9 failed"  ${CURDIR}/test_logs' ]]; then
	 printf -- '**********************************************************************************************************\n'
         printf -- '\nCompleted test execution !! Some test case failures can be ignored as they are seen on x86 also \n'
         printf -- '**********************************************************************************************************\n'
	else
	 printf -- '**********************************************************************************************************\n'

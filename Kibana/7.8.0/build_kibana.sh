...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Kibana/7.8.0/build_kibana.sh
...
function cleanup() {
	sudo rm -rf "${CURDIR}/node-v10.21.0-linux-s390x.tar.gz" "${CURDIR}/gcc-4.9.4.tar.gz" "${CURDIR}/kibana"
...
function configureAndInstall() {
	...
	wget https://nodejs.org/dist/v12.18.2/node-v12.18.2-linux-s390x.tar.gz
        sudo tar xzvf node-v12.18.2-linux-s390x.tar.gz -C /usr/local/lib/nodejs
        sudo ln -s /usr/local/lib/nodejs/node-v12.18.2-linux-s390x/bin/* /usr/bin/
    else
	wget https://nodejs.org/dist/v10.21.0/node-v10.21.0-linux-s390x.tar.gz
        sudo tar xzvf node-v10.21.0-linux-s390x.tar.gz -C /usr/local/lib/nodejs
        sudo ln -s /usr/local/lib/nodejs/node-v10.21.0-linux-s390x/bin/* /usr/bin/
    fi
...
function runTest() {
	...
	if [[ "${DISTRO}" == "rhel-7.6" ]] || [[ "${DISTRO}" == "rhel-7.7" ]]; then
		if [[ 'grep "Test Suites: 22 failed"  ${CURDIR}/test_logs' ]]; then
			printf -- '**********************************************************************************************************\n'
		...
		if [[ 'grep "Test Suites: 8 failed"  ${CURDIR}/test_logs' ]] || [[ 'grep "Test Suites: 9 failed"  ${CURDIR}/test_logs' ]]; then
			printf -- '**********************************************************************************************************\n'
			printf -- '\nCompleted test execution !! Test case failures can be ignored as they are seen on x86 also \n'
			printf -- '**********************************************************************************************************\n'
		else
...
"rhel-7.6" | "rhel-7.7" | "rhel-7.8")
	printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo yum install -y autoconf automake bzip2 gcc-c++ git gzip libtool make openssl-devel python tar wget zlib-devel curl |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;
"rhel-8.1" | "rhel-8.2")
	printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo yum install -y autoconf automake bzip2 gcc-c++ git gzip libtool make openssl-devel python2 tar wget zlib-devel curl |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;
*)

...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/1.18.0/build_consul.sh
...
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y curl gcc rh-git227-git.s390x make wget diffutils procps-ng unzip tar python3 python3-pip |& tee -a "$LOG_FILE"
...
"sles-12.5" | "sles-15.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl libnghttp2-devel which gcc git-core make wget awk unzip tar python3 python3-pip |& tee -a "$LOG_FILE"
    if [[ $DISTRO != "sles-12.5" ]]
    then

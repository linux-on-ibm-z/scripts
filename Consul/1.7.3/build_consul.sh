...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/1.7.3/build_consul.sh
...
function configureAndInstall() {
    ...
    #Download Go
    wget $GO_INSTALL_URL && bash build_go.sh -y -v 1.13.7
    go version
...
"sles-12.4" | "sles-12.5"|"sles-15.1")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl gcc git make wget awk |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

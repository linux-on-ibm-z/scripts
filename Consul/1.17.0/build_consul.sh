...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/1.17.0/build_consul.sh
...
function cleanup() {
    # Remove artifacts
    rm -rf "$SOURCE_ROOT/go1.20.10.linux-s390x.tar.gz"
...
function configureAndInstall() {
    ...
    cd ${GOPATH}
    wget https://go.dev/dl/go1.20.10.linux-s390x.tar.gz
    tar -xzf go1.20.10.linux-s390x.tar.gz
...
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.04" | "ubuntu-23.10")
    ...
    sudo apt-get update
    sudo apt-get install -y curl gcc git make wget unzip tar |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y curl gcc rh-git227-git.s390x make wget diffutils procps-ng unzip tar |& tee -a "$LOG_FILE"
...
"sles-12.5" | "sles-15.4" | "sles-15.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl libnghttp2-devel which gcc git-core make wget awk unzip tar |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

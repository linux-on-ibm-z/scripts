...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/2.0.4/build_consul.sh
...
function configureAndInstall() {
	...
	export INSTALL_DIR="${SOURCE_ROOT}/${GO_VERSION}"
    export GOROOT=${INSTALL_DIR}
	export GO_INSTALL_URL="https://go.dev/dl/go"$GO_VERSION".linux-s390x.tar.gz"
    sudo mkdir -p "${INSTALL_DIR}"
    wget -q $GO_INSTALL_URL
    chmod ugo+r go"$GO_VERSION".linux-s390x.tar.gz
    sudo tar -C "${INSTALL_DIR}" --strip-components=1 -xzf go"$GO_VERSION".linux-s390x.tar.gz
...
"rhel-8.10")
    ...
    sudo yum install -y curl gcc git make wget diffutils procps-ng tar python3 python3-pip |& tee -a "$LOG_FILE"

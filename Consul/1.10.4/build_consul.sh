...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/1.10.4/build_consul.sh
...
function cleanup() {
    # Remove artifacts
    rm -rf "$SOURCE_ROOT/go1.16.7.linux-s390x.tar.gz"
...
function configureAndInstall() {
    ...
    cd ${GOPATH}
    wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Go/${GO_PACKAGE_VERSION}/build_go.sh
    bash build_go.sh -v 1.16.7 -y
    go version
...
"rhel-7.8" | "rhel-7.9" | "rhel-8.2" | "rhel-8.4" | "rhel-8.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y  curl gcc git make wget diffutils procps-ng |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

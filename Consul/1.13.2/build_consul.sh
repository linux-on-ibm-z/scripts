...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Consul/1.13.2/build_consul.sh
...
function cleanup() {
    # Remove artifacts
    rm -rf "$SOURCE_ROOT/go1.18.2.linux-s390x.tar.gz"
...
"sles-12.5" | "sles-15.3" | "sles-15.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl which gcc git-core make wget awk unzip|& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

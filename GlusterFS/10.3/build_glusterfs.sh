...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/10.3/build_glusterfs.sh
...
SOURCE_ROOT="$(pwd)"
PREFIX="/usr/local"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/10.3/patch"
...
function configureAndInstall() {
	...
	# Install liburing-devel for rhel-9.x
	if [[ "${DISTRO}" == "rhel-9.0" ]] || [[ "${DISTRO}" == "rhel-9.1" ]]; then
		cd $SOURCE_ROOT
  ...
  # Download and configure GlusterFS
	...
	if [[ "${DISTRO}" == "rhel-7.8" ]] || [[ "${DISTRO}" == "rhel-7.9" ]]; then
		...
		./configure --enable-gnfs --disable-linux-io_uring --disable-events
	else
...
function runTest() {
	set +e
		...
		if [[ "$TESTS" == "true" ]]; then
			...
			"sles-12.5")
				zypper install -y acl attr bc bind-utils gdb libxml2-tools net-tools nfs-utils psmisc vim xfsprogs python3-xattr libselinux-devel selinux-tools popt-devel python3-pip
				pip3 install prettytable
				;;
			...
			# Install thin-provisioning-tools (RHEL and SLES12SP5 only)
			...
			# Apply run-tests.sh patch for SLES12SP5

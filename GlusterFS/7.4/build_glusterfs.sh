...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/7.4/build_glusterfs.sh
...
CURDIR="$(pwd)"
BUILD_DIR="/usr/local"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/7.4/patch"
...
function configureAndInstall() {
	...
	if [[ "${ID}" == "rhel" ]] || [[ "${DISTRO}" == "sles-12.4" ]] || [[ "${DISTRO}" == "sles-12.5" ]]; then
	...
	if [[ "${DISTRO}" == "rhel-7.6" ]] || [[ "${DISTRO}" == "rhel-7.7" ]] || [[ "${DISTRO}" == "rhel-7.8" ]] || [[ "${DISTRO}" == "sles-12.4" ]] || [[ "${DISTRO}" == "sles-12.5" ]] || [[ "${DISTRO}" == "ubuntu-16.04" ]]; then
		cd $SOURCE_ROOT
	...
	./autogen.sh
	if [[ "${DISTRO}" == "sles-12.4" ]] || [[ "${DISTRO}" == "sles-12.5" ]]; then
		./configure --enable-gnfs --disable-events # For SLES 12 SP4 and SLES 12 SP5
	else
  ...
  # Only for Ubuntu, SLES, and RHEL 8
	...
	cd "${CURDIR}"
	curl -o patch_geo-rep.diff $PATCH_URL/patch_geo-rep.diff
...
function runTest() {
	...
	if [[ "$TESTS" == "true" ]]; then
        ...
        "sles-12.4")
        	zypper install -y acl attr bc bind-utils boost-devel gcc-c++ gdb libexpat-devel libxml2-tools net-tools nfs-utils psmisc vim xfsprogs python3 python3-pip python3-devel python3-cffi 
		pip3 install xattr
        	;;
	...
	"sles-12.5")
		...
		if [[ "${DISTRO}" == "sles-12.4" ]] || [[ "$DISTRO" == "sles-12.5" ]] || [[ "${DISTRO}" == "ubuntu-16.04" ]]; then

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/8.0/build_glusterfs.sh
...
SOURCE_ROOT="$(pwd)"
BUILD_DIR="/usr/local"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/8.0/patch"
...
function configureAndInstall() {
	...
	if [[ "${DISTRO}" == "ubuntu-20.04" ]] || [[ ${DISTRO} == rhel-8\.[1-2] ]]; then
...
"ubuntu-20.04")
	...
	printf -- '\nInstalling dependencies \n' | tee -a "$LOG_FILE"
	apt-get update
	apt-get install -y autoconf automake bison curl flex gcc g++ git libacl1-dev libaio-dev libfuse-dev libglib2.0-dev libibverbs-dev librdmacm-dev libreadline-dev libssl-dev libtool liburcu-dev libxml2-dev lvm2 make openssl pkg-config python3 uuid-dev zlib1g-dev patch wget
...
"rhel-7.8" )
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
	printf -- '\nInstalling dependencies \n' | tee -a "$LOG_FILE"
	yum install -y autoconf automake bison bzip2 curl flex fuse-devel gcc git glib2-devel libacl-devel libaio-devel libibverbs-devel librdmacm-devel libtool libxml2-devel libuuid-devel lvm2 make pkgconfig python python3 readline-devel wget zlib-devel patcg which
	configureAndInstall | tee -a "$LOG_FILE"
	;;

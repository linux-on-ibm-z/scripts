...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/10.1/build_glusterfs.sh
...
SOURCE_ROOT="$(pwd)"
PREFIX="/usr/local"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/GlusterFS/10.1/patch"
	...
	set +e
		...
		if [[ "$TESTS" == "true" ]]; then
			...
			git apply test-patch.diff
                        ...
                        if [[ "${ID}" == "sles" ]]; then
                            wget --no-check-certificate $PATCH_URL/test-patch_sles.diff 
                            git apply test-patch_sles.diff
                            printf -- "Patch test-patch_sles.diff success\n" 
                        fi

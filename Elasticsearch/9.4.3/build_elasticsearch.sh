...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Elasticsearch/9.4.3/build_elasticsearch.sh
...
function configureMlControllerLibraryPath() {
    local controller_dir="/usr/share/elasticsearch/modules/x-pack-ml/platform/linux-s390x/bin"
    local controller="$controller_dir/controller"
    local controller_bin="$controller_dir/controller.bin"

    if [[ ! -f "$controller" && ! -f "$controller_bin" ]]; then
        return 0
    fi

    if [[ ! -f "$controller_bin" ]]; then
        sudo mv "$controller" "$controller_bin"
    fi

    sudo tee "$controller" >/dev/null <<'EOF'
#!/bin/bash
DIR="$(cd "$(dirname "$0")" && pwd)"
export LD_LIBRARY_PATH="$DIR/../lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
exec -a controller "$DIR/controller.bin" "$@"
EOF
    sudo chmod 0755 "$controller" "$controller_bin"
}
...
function configureAndInstall() {
    ...
    sed -i -e 's|6cd91991323dd7b2fb28ca93d7ac12af5a86a2f53279e2b35827b30313fd0b9f|'"${sha256}"'|g' \
        -e 's|616491ff68183f90e145b68874627742d796572dbfe4d8dfe64bac22a27864cb|'"${sha256}"'|g' \
        "${SOURCE_ROOT}/elasticsearch/gradle/verification-metadata.xml"
    ...
    docker build ${DOCKER_BUILD_NETWORK_OPTS} -f Dockerfile -t docker.elastic.co/elasticsearch-dev/openldap-fixture:1.0 .
    ...
    docker build ${DOCKER_BUILD_NETWORK_OPTS} -f Dockerfile -t docker.elastic.co/elasticsearch-dev/es-smb-fixture:1.0 .
    ...
    sed -i '/ENV CGO_ENABLED=0/a ENV GODEBUG=http2client=0' Dockerfile
    docker build ${DOCKER_BUILD_NETWORK_OPTS} --build-arg TARGETARCH=s390x --build-arg RELEASE="RELEASE.2025-09-07T16-13-09Z" --build-arg MC_RELEASE="RELEASE.2025-08-13T08-35-41Z" -t  minio/minio:RELEASE.2025-09-07T16-13-09Z --platform=linux/s390x .
    ...
    export LOCAL_VEC_BINARY_OS="${LOCAL_VEC_BINARY_OS:-linux-s390x}"
    ./gradlew --stop >/dev/null 2>&1 || true
    configureEs9GradleOpts
    printf -- "export GRADLE_USER_HOME=$GRADLE_USER_HOME\n" >> "$BUILD_ENV"
    printf -- "export LOCAL_VEC_BINARY_OS=$LOCAL_VEC_BINARY_OS\n" >> "$BUILD_ENV"
    ...
    configureMlControllerLibraryPath
...
function runTest() {
    if [[ "$TESTS" == "true" ]]; then
        ...
        printf -- '*****************************************************************************************************\n'
        printf -- 'Some X-Pack test cases may fail as not all X-Pack plugins are not supported on s390x.\n\n'
...
function printSummary() {
    ...
    printf -- '\n\nStart Elasticsearch using: elasticsearch &\n'
    printf -- 'Set ELASTIC_PASSWORD from the startup output, or run: elasticsearch-reset-password -b -u elastic\n'
    printf -- 'Then verify with: curl --cacert /usr/share/elasticsearch/config/certs/http_ca.crt -u elastic:$ELASTIC_PASSWORD https://localhost:9200\n'
    printf -- '\n*****************************************************************************************************\n'
}
...
"rhel-8.10" | "rhel-9.6" | "rhel-9.7" | "rhel-9.8" | "rhel-10.0" | "rhel-10.1" | "rhel-10.2")
    sudo yum install -y curl git gzip tar wget patch make gcc gcc-c++ bzip2 zip unzip zlib-devel libxml2-devel python3 python3-devel python3-pip cmake ninja-build |& tee -a "$LOG_FILE"
    sudo yum install -y gcc-toolset-13-gcc gcc-toolset-13-gcc-c++ |& tee -a "$LOG_FILE" || true

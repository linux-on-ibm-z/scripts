...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Elasticsearch/8.11.1/build_elasticsearch.sh
...
function isValidJavaProvided() {
    ...
    "OpenJDK17")
        ...
        ;;
    "OpenJDK21")
        # OpenJDK 21 can not currently compile elasticsearch due to 'this-escape' errors.
        # This may be fixed in 8.12.0. eg. https://github.com/elastic/elasticsearch/pull/101519
        return 1
        ;;
    esac
    return 1
...
function configureAndInstall() {
        ...
        printf -- "export JAVA17_HOME=$JAVA17_HOME\n" >> "$BUILD_ENV"
...
function runTest() {
        if [[ "$TESTS" == "true" ]]; then
                ...
                ./gradlew --continue test internalClusterTest -Dtests.haltonfailure=false -Dtests.jvm.argline="-Xss2m" |& tee -a ${CURDIR}/logs/test_results.log
                printf -- '*****************************************************************************************************\n'
...
while getopts "h?dytj:" opt; do
        ...
        j)
                ...
                if ! isValidJavaProvided "$JAVA_PROVIDED"; then
                        printf "%s is not supported, Please use valid java from {Temurin17, OpenJDK17} only" "$JAVA_PROVIDED"
...
"rhel-7.8" | "rhel-7.9")
        sudo yum install -y curl git gzip tar wget patch make gcc gcc-c++ rh-git227 fontconfig urw-fonts |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;

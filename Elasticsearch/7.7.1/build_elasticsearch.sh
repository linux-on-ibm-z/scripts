...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Elasticsearch/7.7.1/build_elasticsearch.sh
...
function configureAndInstall() {
	...
	wget $PATCH_URL/oss_docker_build_context_build.gradle -P ${CURDIR}/elasticsearch/distribution/docker/oss-docker-s390x-build-context
        ...
        wget -O - $PATCH_URL/patch_gradle.diff | git apply
...
function runTest() {
	...
	set +e
        ...
        ./gradlew --continue test -Dtests.haltonfailure=false -Dtests.jvm.argline="-Xss4m" |& tee -a ${CURDIR}/logs/test_results.log

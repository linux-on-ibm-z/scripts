...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Elasticsearch/7.2.0/build_elasticsearch.sh
...
function cleanup() {
        ...
        rm -rf "${CURDIR}/OpenJDK12U-jdk_s390x_linux_hotspot_12.0.1_12.tar.gz"
...
function configureAndInstall() {
        ...
        #git apply patch_gradle.diff
		
		sed -i "176s/linux-x86_64/linux-s390x/" "${CURDIR}/elasticsearch/distribution/archives/build.gradle"

        curl -o BuildPlugin.diff $REPO_URL/BuildPlugin.diff
        git apply BuildPlugin.diff
...
"ubuntu-16.04" | "ubuntu-18.04")
        ...
        sudo apt-get update
        sudo apt-get install -y curl git gzip tar wget |& tee -a "$LOG_FILE"
...
"rhel-7.4" | "rhel-7.5" | "rhel-7.6")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y curl git gzip tar wget |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        startService |& tee -a "$LOG_FILE"
        installClient |& tee -a "$LOG_FILE"
        ;;
...
"sles-12.4" | "sles-15")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y curl git gzip tar wget | tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        startService |& tee -a "$LOG_FILE"
        installClient |& tee -a "$LOG_FILE"
        ;;

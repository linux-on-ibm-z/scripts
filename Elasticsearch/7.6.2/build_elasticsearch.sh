...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Elasticsearch/7.6.2/build_elasticsearch.sh
...
function cleanup() {
        rm -rf "${CURDIR}/OpenJDK13U-jdk_s390x_linux_hotspot_13.0.2_8.tar.gz"
...
function configureAndInstall() {
        ...
        printf -- 'Downloading OpenJDK 13 with HotSpot. \n'

        wget https://github.com/AdoptOpenJDK/openjdk13-binaries/releases/download/jdk-13.0.2%2B8/OpenJDK13U-jdk_s390x_linux_hotspot_13.0.2_8.tar.gz
        sudo tar -C /usr/local -xzf OpenJDK13U-jdk_s390x_linux_hotspot_13.0.2_8.tar.gz
        export PATH=/usr/local/jdk-13.0.2+8/bin:$PATH
        java -version |& tee -a "$LOG_FILE"
        printf -- 'OpenJDK 13 with HotSpot installed\n'
        ...
        if [[ "$(grep -q JAVA_HOME .bashrc)" ]]; then
                printf -- '\nChanging JAVA_HOME\n'
                sed -n 's/^.*\bJAVA_HOME\b.*$/export JAVA_HOME=\/usr\/local\/jdk-13.0.2+8\//p' ~/.bashrc
        else
                echo "export JAVA_HOME=/usr/local/jdk-13.0.2+8/" >>.bashrc
        fi
...
function installClient() {
        ...
        if [[ "${ID}" == "sles" ]]; then
          sudo -H env PATH=$PATH:/usr/local/bin pip install elasticsearch-curator
        else

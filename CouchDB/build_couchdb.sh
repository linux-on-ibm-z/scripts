...
function configureAndInstall() {
	...
	#Install SpiderMonkey 1.8.5 (Only for RHEL 6.10, SLES and Ubuntu 18.04)
	if [ "${ID}" == "sles" ] || [ ${VERSION_ID} == "18.04" ] || [ ${VERSION_ID} == "6.10" ]; then
		...
		# For RHEL 6.10 and SLES 12 SP3
		if [ "${VERSION_ID}" == "6.10" ] || [ ${VERSION_ID} == "12.3" ]; then
			autoconf-2.13 
		fi 

		#For SLES 15 and Ubuntu 18.04
		if [ "${VERSION_ID}" == "15" ] || [ ${VERSION_ID} == "18.04" ]; then
			autoconf2.13 
		fi 
...
function printSummary() {
	...
	printf -- '\nFor more help visit http://docs.couchdb.org/en/2.3.0/index.html \n' |& tee -a "$LOG_FILE"
}

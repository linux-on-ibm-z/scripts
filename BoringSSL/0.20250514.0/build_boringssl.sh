...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/BoringSSL/0.20250514.0/build_boringssl.sh
...
BORINGSSL_BRANCH="0.20250514.0-s390x"
GO_VERSION="1.24.6"
TESTS="false"
FORCE="false"
...
"rhel-9.4" | "rhel-9.6" | "rhel-9.7" | "rhel-10.0" | "rhel-10.1")
	...
	;;
"sles-15.7")
    sudo zypper install -y wget git tar gzip cmake ninja zlib-devel gcc14 gcc14-c++ curl |& tee -a "$LOG_FILE"
    ...
    ;;
"sles-16.0")
    sudo zypper install -y wget git tar gzip cmake ninja zlib-devel gcc gcc-c++ curl |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

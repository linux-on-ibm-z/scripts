...
#
#Instructions
#Get Build script : wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Python3/3.13.12/build_python3.sh
#Execute build script: bash build_python3.sh

set -e -o pipefail

PACKAGE_NAME="python"
PACKAGE_VERSION="3.13.12"
TESTS="false"
FORCE="false"
CURDIR="$(pwd)"
...
"sles-15.7" | "sles-16.0")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        sudo zypper ref -s
        sudo zypper install -y gawk gcc gcc-c++ gdbm-devel libbz2-devel libffi-devel libopenssl-devel libuuid-devel make ncurses-devel tar tk-devel wget xz-devel zlib-devel gzip libtirpc-devel
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;

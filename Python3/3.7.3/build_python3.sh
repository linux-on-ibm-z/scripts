...
function printHelp() {
	...
	echo "This script supports Python version 3.7.3"
	echo
}
...
"rhel-7.4" | "rhel-7.5" | "rhel-7.6" | "rhel-8.0")
	...
	sudo yum install -y bzip2-devel gcc gcc-c++ gdbm-devel libdb libffi-devel libuuid make ncurses openssl readline sqlite tar tk wget xz xz zlib-devel glibc-langpack-en |& tee -a "$LOG_FILE"
	else
	sudo yum install -y bzip2-devel gcc gcc-c++ gdbm-devel libdb-devel libffi-devel libuuid-devel make ncurses-devel openssl-devel readline-devel sqlite-devel tar tk-devel wget xz xz-devel zlib-devel |& tee -a "$LOG_FILE"
	fi
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

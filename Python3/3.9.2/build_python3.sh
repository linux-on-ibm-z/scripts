...
#
#Instructions
#Get Build script : wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Python3/3.9.2/build_python3.sh
...
function printHelp() {
        ...
        echo "This script supports Python version 3.9.2"
        echo
}
...
"rhel-7.8" | "rhel-7.9" | "rhel-8.1" | "rhel-8.2" | "rhel-8.3")
        ...
        if [[ "$ID" == "rhel" && "$VERSION_ID" == "8.1" ]] || [[ "$ID" == "rhel" && "$VERSION_ID" == "8.2" ]] || [[ "$ID" == "rhel" && "$VERSION_ID" == "8.3" ]]; then
                sudo yum install -y bzip2-devel gcc gcc-c++ gdbm-devel libdb libffi-devel libuuid make ncurses openssl readline sqlite tar tk wget xz xz zlib-devel glibc-langpack-en diffutils xz-devel
        else

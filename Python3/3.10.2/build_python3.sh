...
#
#Instructions
#Get Build script : wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Python3/3.10.2/build_python3.sh
...
function printHelp() {
        ...
        echo "This script supports Python version 3.10.2"
        echo
}
...
"rhel-7.8" | "rhel-7.9" | "rhel-8.2" | "rhel-8.4" | "rhel-8.5" | "rhel-8.6" | "rhel-8.8")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        if [[ "$VERSION_ID" == "8.2" ]] || [[ "$VERSION_ID" == "8.4" ]] || [[ "$VERSION_ID" == "8.5" ]] || [[ "$VERSION_ID" == "8.6" ]] || [[ "$VERSION_ID" == "8.8" ]]; then

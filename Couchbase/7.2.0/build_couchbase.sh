...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Couchbase/7.2.0/build_couchbase.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Couchbase/7.2.0/patch"
...
function buildGrpc() {
    ...
    OPENSSL_BUILD_DIR=$SOURCE_ROOT/openssl-1.1.1t/_build
    (
...
function buildPrometheus() {
    ...
    echo "export PATH=$SOURCE_ROOT/node-v16.20.1-linux-s390x/bin:$PATH" >> $PRESERVE_ENVARS
    npm install -g yarn
...
solutions = [
  {
    "url": "https://chromium.googlesource.com/v8/v8.git@10.7.21",
    "managed": False,
    "name": "v8",
    "deps_file": "DEPS",
  },
...
function buildCouchbase() {
    ...
    if [[ $DISTRO == ubuntu* ]]; then
        sed -i "s/ubuntu20.04/$DISTRO/g" $SOURCE_ROOT/couchbase/tlm/deps/manifest.cmake
    fi

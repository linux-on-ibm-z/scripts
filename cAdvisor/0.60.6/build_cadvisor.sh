...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/cAdvisor/0.60.6/build_cadvisor.sh
...
function prepare() {
    ...
    if [[ "$FORCE" == "true" ]]; then
        ...
        DISTRO="$ID"
        printf -- "\nAs part of the installation , Go 1.25.11 will be installed, \n"
        while true; do
...
function configureAndInstall() {
    ...
    wget https://go.dev/dl/go1.25.11.linux-s390x.tar.gz
    tar -xzf go1.25.11.linux-s390x.tar.gz
    sudo rm -rf /usr/local/go /usr/bin/go
    sudo ln -sf $CURDIR/go/bin/go /usr/bin/ 
    sudo ln -sf $CURDIR/go/bin/gofmt /usr/bin/
...
"sles-15.7" | "sles-16.0")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper  install -y git wget tar curl gcc patch make findutils |& tee -a "${LOG_FILE}"
    configureAndInstall |& tee -a "${LOG_FILE}"
    ;;
...
"ubuntu-22.04" | "ubuntu-24.04")
    ...
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update 
    sudo apt-get install -y wget git curl patch make gcc libc6-dev |& tee -a "${LOG_FILE}"
    configureAndInstall |& tee -a "${LOG_FILE}"
    ;;

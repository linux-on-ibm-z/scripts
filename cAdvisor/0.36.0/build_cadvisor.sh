...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/cAdvisor/0.36.0/build_cadvisor.sh
...
GO_INSTALL_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Go/1.14.4/build_go.sh"
...
"ubuntu-18.04")
	...
	sudo apt-get update 
	sudo apt-get install -y wget git curl patch |& tee -a "${LOG_FILE}"
...
"ubuntu-20.04")
	...
	printf -- "Installing dependencies... it may take some time.\n"
	sudo apt-get update 
	sudo apt-get install -y wget make git curl patch golang-1.14 |& tee -a "${LOG_FILE}"

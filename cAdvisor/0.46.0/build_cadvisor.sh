...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/cAdvisor/0.46.0/build_cadvisor.sh
...
function prepare() {
    ...
    if [[ "$FORCE" == "true" ]]; then
        ...
        DISTRO="$ID"
        printf -- "\nAs part of the installation , Go 1.18.x will be installed, \n"
        while true; do
...
function configureAndInstall() {
    ...
    wget -q https://storage.googleapis.com/golang/go1.18.4.linux-s390x.tar.gz
    chmod ugo+r go1.18.4.linux-s390x.tar.gz
    sudo tar -C /usr/local -xzf go1.18.4.linux-s390x.tar.gz

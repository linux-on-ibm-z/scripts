...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX/1.20.2/build_nginx.sh
...
function printHelp() {
    echo
    echo "Usage: "
    echo "bash build_nginx.sh  [-d debug] [-y install-without-confirmation]"
    echo
}
...
case "$DISTRO" in
"ubuntu-18.04" | "ubuntu-21.04" | "ubuntu-21.10")
...
"sles-12.5" | "sles-15.3")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y pcre-devel wget tar xz gcc make zlib-devel diffutils gzip |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

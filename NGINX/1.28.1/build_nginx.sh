...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/NGINX/1.28.1/build_nginx.sh
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd nginx-tests
        TEST_NGINX_BINARY=$SOURCE_ROOT/nginx-1.28.1/objs/nginx prove .
...
function install_http_request_ascgi() {
    curl -LO "https://download.opensuse.org/repositories/openSUSE:/Factory/standard/noarch/perl-HTTP-Request-AsCGI-1.200.0-1.2.noarch.rpm"
    sudo zypper --no-gpg-checks --non-interactive install --force-resolution ./perl-HTTP-Request-AsCGI-1.200.0-1.2.noarch.rpm
    perl -MHTTP::Request::AsCGI -e 'print "Module loaded successfully on Perl $^V\n"'
    rm -rf perl-HTTP-Request-AsCGI-1.200.0-1.2.noarch.rpm
}
...
"rhel-10.0" | "rhel-10.1")
 printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y pcre2-devel wget tar xz gcc make zlib-devel diffutils |& tee -a "$LOG_FILE"
    if [[ "$TESTS" == "true" ]]; then
        sudo yum install -y libedit-devel gd-devel perl perl-devel openssl-devel libxml2-devel libxslt-devel perl-Cache-Memcached perl-CryptX perl-GD perl-IO-Socket-SSL perl-Test-Harness perl-HTTP-Request-AsCGI perl-FCGI uwsgi uwsgi-plugin-python3 |& tee -a "$LOG_FILE"
    fi
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"sles-15.7")

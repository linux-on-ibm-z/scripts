...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/7.0.0/build_logstash.sh
...
function cleanup() {
	sudo rm -rf "${WORKDIR}/ibm-java-s390x-sdk-8.0-5.30.bin" "${WORKDIR}/installer.properties"
...
function configureAndInstall() {
	...
	cd "${WORKDIR}"
	sudo wget -q http://public.dhe.ibm.com/ibmdl/export/pub/systems/cloud/runtimes/java/8.0.5.30/linux/s390x/ibm-java-s390x-sdk-8.0-5.30.bin
	...
	sudo chmod +x ibm-java-s390x-sdk-8.0-5.30.bin
	sudo ./ibm-java-s390x-sdk-8.0-5.30.bin -r installer.properties
	...
	cd ${WORKDIR}/logstash-${PACKAGE_VERSION}/logstash-core/lib/jars
        sudo unzip -d jruby-complete-9.2.6.0.jar-dir jruby-complete-9.2.6.0.jar
	cd ${WORKDIR}/logstash-${PACKAGE_VERSION}/logstash-core/lib/jars/jruby-complete-9.2.6.0.jar-dir/META-INF/jruby.home/lib/ruby/stdlib/ffi/platform/s390x-linux
        sudo cp -n types.conf platform.conf
        cd ${WORKDIR}/logstash-${PACKAGE_VERSION}/logstash-core/lib/jars/jruby-complete-9.2.6.0.jar-dir
        sudo zip -r jruby-complete-9.2.6.0.jar *
        sudo mv -f jruby-complete-9.2.6.0.jar ..
        cd ${WORKDIR}/logstash-${PACKAGE_VERSION}/logstash-core/lib/jars
        sudo rm -rf jruby-complete-9.2.6.0.jar-dir
...
"rhel-6.x")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	sudo yum install -y wget zip unzip tar gcc make |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

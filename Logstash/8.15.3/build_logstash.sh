...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/8.15.3/build_logstash.sh
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin11" ]]; then
        ...
        if [[ "${ID}" == "ubuntu" ]]; then
            ...
            if [[ "${DISTRO}" == "sles-15.5" ]]; then
                printf "$JAVA_PROVIDED is not available on SLES 15 SP5.  Please use use valid variant from {Temurin11, Temurin17, Temurin21, OpenJDK11, OpenJDK17}.\n"
                exit 1
            else

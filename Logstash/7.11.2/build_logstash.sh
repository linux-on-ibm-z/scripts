...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/7.11.2/build_logstash.sh
...
function cleanup() {
        sudo rm -rf "${CURDIR}/jffi-${JFFI_VERSION}.tar.gz" "${CURDIR}/logstash-oss-${PACKAGE_VERSION}.tar.gz" "${CURDIR}/adoptjdk.tar.gz"
...
function configureAndInstall() {
	...
	if [[ "$ID" == "rhel" || "$ID" == "sles" ]]; then
		...
		tar -xzvf jffi-${JFFI_VERSION}.tar.gz
		cd jffi-jffi-${JFFI_VERSION}
		ant
		export LD_LIBRARY_PATH=${CURDIR}/jffi-jffi-${JFFI_VERSION}/build/jni/:${CURDIR}/jffi-jffi-${JFFI_VERSION}/build/jni/libffi-s390x-linux/.libs:$LD_LIBRARY_PATH
                printf -- "export LD_LIBRARY_PATH=${CURDIR}/jffi-jffi-${JFFI_VERSION}/build/jni:${CURDIR}/jffi-jffi-${JFFI_VERSION}/build/jni/libffi-s390x-linux/.libs:\$LD_LIBRARY_PATH\n" >> "$BUILD_ENV"
	fi
...
"ubuntu-18.04" | "ubuntu-20.04")
        ...
        sudo apt-get update
        sudo apt-get install -y make tar curl wget libjffi-jni gzip |& tee -a "${LOG_FILE}"
...
"rhel-7.8" | "rhel-7.9" | "rhel-8.1" | "rhel-8.2" | "rhel-8.3" )
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        sudo yum install -y curl ant gcc java-1.8.0-openjdk make tar wget |& tee -a "${LOG_FILE}"
...
"sles-12.5")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        sudo zypper install -y curl ant gawk gcc java-1_8_0-openjdk-devel make tar wget |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/9.2.0/build_logstash.sh
...
function prepare() {
        ...
        if [[ "$JAVA_PROVIDED" != "Temurin17" && "$JAVA_PROVIDED" != "Temurin21" && "$JAVA_PROVIDED" != "OpenJDK17" && "$JAVA_PROVIDED" != "OpenJDK21" ]]; then
                printf "$JAVA_PROVIDED is not supported, Please use valid java from {Temurin11, Temurin17, Temurin21, OpenJDK17, OpenJDK21} only"
                exit 1
        fi
...
function cleanup() {
        sudo rm -rf "${CURDIR}/logstash-oss-${PACKAGE_VERSION}-linux-aarch64.tar.gz" "${CURDIR}/temurin17.tar.gz" "${CURDIR}/temurin21.tar.gz"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin17" ]]; then
        ...
        cd $SOURCE_ROOT
        wget -O temurin17.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.16%2B8/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.16_8.tar.gz
        ...
        cd $SOURCE_ROOT
        wget -O temurin21.tar.gz https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.9%2B10/OpenJDK21U-jdk_s390x_linux_hotspot_21.0.9_10.tar.gz
        ...
        printf "$JAVA_PROVIDED is not supported, Please use valid variant from {Temurin17, Temurin21, OpenJDK17, OpenJDK21} only"
        exit 1
    fi
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  bash build_logstash.sh  [-d debug] [-y install-without-confirmation] [-j Java to use from {Temurin17, Temurin21, OpenJDK17, OpenJDK21}]"
        echo "  default: If no -j specified, OpenJDK 17 will be installed"
        echo
}

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/8.10.4/build_logstash.sh
...
function prepare() {
        ...
        if [[ "$JAVA_PROVIDED" != "Temurin11" && "$JAVA_PROVIDED" != "Temurin17" && "$JAVA_PROVIDED" != "OpenJDK11" && "$JAVA_PROVIDED" != "OpenJDK17" ]]; then
...
function cleanup() {
        sudo rm -rf "${CURDIR}/logstash-oss-${PACKAGE_VERSION}-linux-aarch64.tar.gz" "${CURDIR}/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.9_9.tar.gz" "${CURDIR}/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.21_9.tar.gz"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin11" ]]; then
        ...
        cd $SOURCE_ROOT
        wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.21%2B9/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.21_9.tar.gz
        tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.21_9.tar.gz
        export LS_JAVA_HOME=$PWD/jdk-11.0.21+9
        ...
        cd $SOURCE_ROOT
        wget https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.9%2B9/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.9_9.tar.gz
        tar -xzf OpenJDK17U-jdk_s390x_linux_hotspot_17.0.9_9.tar.gz
        export LS_JAVA_HOME=$PWD/jdk-17.0.9+9
        ...
        if [[ "${ID}" == "ubuntu" ]]; then
            ...
            if [[ "${DISTRO}" == "rhel-7."* ]]; then
                printf "$JAVA_PROVIDED is not available on RHEL 7.  Please use use valid variant from {Temurin11, Temurin17, OpenJDK11}.\n"
                exit 1
            else
            ...
            if [[ "${DISTRO}" == "sles-12.5" ]]; then
                printf "$JAVA_PROVIDED is not available on SLES 12 SP5.  Please use use valid variant from {Temurin11, Temurin17, OpenJDK11}.\n"
                exit 1
            else
        ...
        printf "$JAVA_PROVIDED is not supported, Please use valid variant from {Temurin11, Temurin17, OpenJDK11, OpenJDK17} only"
        exit 1
    fi
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  bash build_logstash.sh  [-d debug] [-y install-without-confirmation] [-j Java to use from {Temurin11, Temurin17, OpenJDK11, OpenJDK17}]"

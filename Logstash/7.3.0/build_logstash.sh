...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Logstash/7.3.0/build_logstash.sh
...
function cleanup() {
        sudo rm -rf "${CURDIR}/jffi-1.2.18.tar.gz" "${CURDIR}/logstash-oss-${PACKAGE_VERSION}.tar.gz"
...
function configureAndInstall() {
        ...
        sudo wget https://github.com/jnr/jffi/archive/jffi-1.2.18.tar.gz
        sudo tar -xzvf jffi-1.2.18.tar.gz -C /usr/local/jffi --strip-components 1
        sudo chown "$(whoami)" -R /usr/local/jffi
        cd /usr/local/jffi
        ant
        export LD_LIBRARY_PATH=$CURDIR/jffi-jffi-1.2.18/build/jni/:$CURDIR/jffi-jffi-1.2.18/build/jni/libffi-s390x-linux/.libs:$LD_LIBRARY_PATH

        # Downloading and installing Logstash
        printf -- 'Downloading and installing Logstash.\n'
        cd "${CURDIR}"
        wget https://artifacts.elastic.co/downloads/logstash/logstash-oss-"$PACKAGE_VERSION".tar.gz

        sudo mkdir /usr/share/logstash
        sudo tar -xzf logstash-oss-"$PACKAGE_VERSION".tar.gz  -C /usr/share/logstash --strip-components 1
...
"ubuntu-16.04" | "ubuntu-18.04")
        ...
        sudo apt-get update
        sudo apt-get install -y ant gcc gzip openjdk-8-jdk make tar unzip wget zip |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/8.0.35/build_mysql.sh
# Execute build script: bash build_mysql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="mysql"
PACKAGE_VERSION="8.0.35"
SOURCE_ROOT="$(pwd)"
...
function configureAndInstall() {
    ...
    if [[ "$DISTRO" =~ rhel-7\.[8-9] ]]; then
        #curl -sSL "${PATCH_URL}/CMakelist.patch" | git apply -

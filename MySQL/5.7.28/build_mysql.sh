...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/5.7.28/build_mysql.sh
# Execute build script: bash build_mysql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="mysql-server"
PACKAGE_VERSION="5.7.28"
SOURCE_ROOT="$(pwd)"
...
"ubuntu-16.04" | "ubuntu-18.04")
        ...
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y bison cmake gcc g++ git libncurses-dev libssl-dev hostname openssl make pkg-config |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
    	;;
*)

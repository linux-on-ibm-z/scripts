...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/8.0.34/build_mysql.sh
...
"rhel-7.8" | "rhel-7.9")
    ...
    sudo yum install -y make curl wget bison bzip2 gcc gcc-c++ git hostname ncurses-devel pkgconfig tar wget procps zlib-devel doxygen devtoolset-11-gcc devtoolset-11-gcc-c++ devtoolset-11-binutils net-tools |& tee -a "$LOG_FILE"
...
"sles-12.5")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
	sudo zypper install -y curl wget make cmake git ncurses-devel pkg-config gawk m4 procps doxygen tar gcc11 gcc11-c++ libtirpc-devel libnghttp2-devel net-tools timezone |& tee -a "$LOG_FILE"
...
"sles-15.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl make cmake bison gcc11 gcc11-c++ git hostname ncurses-devel openssl procps openssl-devel pkg-config gawk doxygen libtirpc-devel rpcgen tar wget net-tools-deprecated xz |& tee -a "$LOG_FILE"
	sudo update-alternatives --install /usr/bin/cc cc /usr/bin/gcc-11 11
	sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-11 11
	sudo update-alternatives --install /usr/bin/g++ g++ /usr/bin/g++-11 11
	sudo update-alternatives --install /usr/bin/c++ c++ /usr/bin/g++-11 11

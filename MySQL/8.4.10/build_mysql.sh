...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/8.4.10/build_mysql.sh
# Execute build script: bash build_mysql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="mysql"
PACKAGE_VERSION="8.4.10"
SOURCE_ROOT="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/8.4.10/patch"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        if [[ "$ID" != "ubuntu" ]]; then
            LIBPROTOBUF_LITE=$(find $SOURCE_ROOT/mysql-server/build/library_output_directory -name "libprotobuf-lite.so.*.*.*" | head -n 1)
            export LD_PRELOAD=${LIBPROTOBUF_LITE}${LD_PRELOAD:+:${LD_PRELOAD}}
        ...
        # Run the test suite from the build tree so mtr uses binaries from the
        # build directory and does not need write access to the install dirs.
        mkdir -p $SOURCE_ROOT/mysql-test/var/tmp
        cd $SOURCE_ROOT/mysql-server/build/mysql-test
        ./mysql-test-run.pl --vardir=${SOURCE_ROOT}/mysql-test/var --tmpdir=${SOURCE_ROOT}/mysql-test/var/tmp --suite=main --force
...
function gettingStarted() {
    ...
    printf -- " MySQL %s installed successfully.       \n" "$PACKAGE_VERSION"
...
"rhel-9.6" | "rhel-9.7" | "rhel-9.8")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y curl wget bison bzip2 bzip2-devel gcc gcc-c++ git xz xz-devel hostname ncurses ncurses-devel openssl procps openssl-devel \

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MySQL/8.1.0/build_mysql.sh
...
function configureAndInstall() {
    ...
    curl -sSL "${PATCH_URL}/my-timer-cycles.patch" | git apply -
...
function gettingStarted() {
    ...
    printf -- " MySQL 8.1.0 installed successfully.       \n"
    printf -- " Information regarding the post-installation steps can be found here : https://dev.mysql.com/doc/refman/8.1/en/postinstallation.html\n"
...
"rhel-7.8" | "rhel-7.9")
    ...
    sudo yum install -y make curl wget bison bzip2 gcc gcc-c++ git hostname ncurses-devel pkgconfig tar wget procps \
        zlib-devel doxygen devtoolset-11-gcc devtoolset-11-gcc-c++ devtoolset-11-binutils net-tools \
        cpan perl-JSON perl-Memoize psmisc zip unzip |& tee -a "$LOG_FILE"
...
"sles-15.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y curl make cmake bison gcc11 gcc11-c++ git hostname ncurses-devel openssl procps openssl-devel pkg-config gawk doxygen \
        libtirpc-devel rpcgen tar wget net-tools-deprecated xz perl-JSON perl-Expect psmisc zip unzip gzip |& tee -a "$LOG_FILE"
...
"ubuntu-20.04" | "ubuntu-22.04")
    ...
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y curl wget tar bison cmake gcc g++ git hostname \

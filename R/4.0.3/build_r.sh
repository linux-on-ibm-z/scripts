...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/R/4.0.3/build_r.sh
  ...
  if [[ "$JAVA_PROVIDED" != "AdoptJDK-OpenJ9" && "$JAVA_PROVIDED" != "OpenJDK" && "$JAVA_PROVIDED" != "AdoptJDK-HotSpot" ]]
   ...
   echo "Java provided by user $JAVA_PROVIDED" >> "$LOG_FILE"
    if [[ "$JAVA_PROVIDED" == "AdoptJDK-OpenJ9" ]]; then
        ...
        curl -SL -o adoptjdk.tar.gz https://github.com/AdoptOpenJDK/openjdk11-binaries/releases/download/jdk-11.0.9%2B11_openj9-0.23.0/OpenJDK11U-jdk_s390x_linux_openj9_linuxXL_11.0.9_11_openj9-0.23.0.tar.gz

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/R/4.2.0/build_r.sh
  ...
  printf -- "Building R %s \n,$PACKAGE_VERSION"
   ...
   echo "Java provided by user $JAVA_PROVIDED" >> "$LOG_FILE"
    if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
		...
		printf -- "export JAVA_HOME=$CURDIR/jdk-11.0.14.1+1\n" >> "$BUILD_ENV"

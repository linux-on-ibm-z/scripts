...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/R/4.2.2/build_r.sh
   ...
   echo "Java provided by user $JAVA_PROVIDED" >> "$LOG_FILE"
    if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
        ...
        wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.15%2B10_openj9-0.32.0/ibm-semeru-open-jdk_s390x_linux_11.0.15_10_openj9-0.32.0.tar.gz
		    tar -xf ibm-semeru-open-jdk_s390x_linux_11.0.15_10_openj9-0.32.0.tar.gz
        export JAVA_HOME=$CURDIR/jdk-11.0.15+10
        printf -- "export JAVA_HOME=$CURDIR/jdk-11.0.15+10\n" >> "$BUILD_ENV"
        ...
        tar -xf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.15_10.tar.gz
        export JAVA_HOME=$CURDIR/jdk-11.0.15+10
        printf -- "export JAVA_HOME=$CURDIR/jdk-11.0.15+10\n" >> "$BUILD_ENV"
...
"ubuntu-18.04" | "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-22.10")
  ...
  sudo apt install -y apt-utils |& tee -a "$LOG_FILE" 
  sudo apt-get install -y tzdata |& tee -a "$LOG_FILE" 

...
#
# Instructions:
# Download build script: curl -sSLO https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CloudStack/4.22.0.0/build_cloudstack.sh
# Execute build script: bash build_cloudstack.sh    (provide -h for help)

set -e   -o pipefail

PACKAGE_NAME="cloudstack"
PACKAGE_VERSION="4.22.0.0"
...
function configureAndInstall() {
    ...
    sed -i '/cd ui && npm install && npm run build && cd ../c\
cd ui && sudo rm -rf ${SOURCE_ROOT}/cloudstack/dist/rpmbuild/BUILD/cloudstack-${PACKAGE_VERSION}/ui/node_modules && npm install --force && NODE_OPTIONS="--max-old-space-size=4096" npm run build && cd ..' \
centos8/cloud.spec
    ...
    # Install rpm
    cd ../dist/rpmbuild/RPMS/noarch/
    sudo rpm --nodeps -i cloudstack-common-${PACKAGE_VERSION}-1.noarch.rpm cloudstack-management-${PACKAGE_VERSION}-1.noarch.rpm cloudstack-ui-${PACKAGE_VERSION}-1.noarch.rpm cloudstack-baremetal-agent-${PACKAGE_VERSION}-1.noarch.rpm cloudstack-usage-${PACKAGE_VERSION}-1.noarch.rpm cloudstack-agent-${PACKAGE_VERSION}-1.noarch.rpm
...
function configureAndInstallUb() {
    ...
    sed -i 's# nodejs (>= 12), lsb-release, dh-systemd | debhelper (>= 13)# lsb-release, dh-systemd | debhelper (>= 13)#' debian/control
    sed -i 's# python (>= 2.7) | python2 (>= 2.7), python3 (>= 3), python-setuptools, python3-setuptools,# python3 (>= 3), python3-setuptools,#' debian/control
    sed -i 's#Depends: ${misc:Depends}, ${python3:Depends}, genisoimage, nfs-common, python3-pip, python3-distutils | python3-distutils-extra, python3-netaddr, uuid-runtime#Depends: ${misc:Depends}, ${python3:Depends}, genisoimage, nfs-common, python3-netaddr#' debian/control
...
function printHelp() {
    ...
    echo "  default: OpenJDK 17 will be installed"
    echo
}
...
setupNodejs() {
    ...
    if [[ $DISTRO == ubuntu* ||  $DISTRO == sles* || $DISTRO == rhel-8.10 ]]; then
        nvm install 10
        nvm use 10
        ...
        nvm use default
        printf -- "\n Installation of Node.js 14 was successful \n"
    fi
    node -v
}
...
setupPython() {
    ...
    if [[ $DISTRO == ubuntu-24.* ]]; then
        ...
        sudo apt-get install -y python-is-python3 python3-pip |& tee -a "$LOG_FILE"
...
case "$DISTRO" in
    ...
    "rhel-8.10" | "rhel-9.6" | "rhel-9.7" | "rhel-10.0")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        if [[ $DISTRO != rhel-10.0 ]]; then
            sudo yum groupinstall -y "Development Tools" |& tee -a "$LOG_FILE"
        fi
        sudo yum install -y git wget python3 python3-pip maven genisoimage createrepo nodejs npm \
        nfs-utils qemu-img ipmitool python3-devel python3-libvirt libvirt perl qemu-kvm rng-tools httpd \
        syslinux-tftpboot tftp-server libffi-devel ant curl chkconfig jq openssl-devel bzip2-devel zlib-devel |& tee -a "$LOG_FILE"
        if [[ $DISTRO == rhel-10.* ]]; then
            sudo yum install -y mariadb mariadb-server jna rpm-build rpmdevtools
        else
            sudo yum install -y mysql mysql-server dhcp-server
        fi
        setupJava 2>&1 | tee -a "$LOG_FILE"
    ...
    "sles-15.7")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        for pkg in sudo wget git java-17-openjdk java-17-openjdk-devel ant ant-junit python3-libvirt-python libvirt selinux-tools dhcp-server qemu-img qemu-kvm dhcp \
        python3 python2 ipmitool python3-pip unzip cryptsetup ethtool ipset python3-setuptools mkisofs tftp mariadb mysql httpd qemu-tools timezone-java nfs-utils libffi-devel libopenssl-devel rpm-build python3-devel jq; do
   ...
   "ubuntu-22.04" | "ubuntu-24.04")
        ...
        printf -- "Installing dependencies... it may take some time.\n"
        sudo apt update -y
        sudo apt-get install -y curl git wget dpkg-dev debhelper openjdk-17-jdk genisoimage build-essential python3 python3-setuptools python3-mysql.connector \
        ...
        ebtables vlan ipset python3-libvirt ethtool iptables cpu-checker libffi-dev rustc cargo jq |& tee -a "$LOG_FILE"

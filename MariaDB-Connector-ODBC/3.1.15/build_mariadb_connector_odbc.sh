...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB-Connector-ODBC/3.1.15/build_mariadb_connector_odbc.sh
...
function configureAndInstall() {
    ...
    case "$DISTRO" in
        ...
        "sles"* | "rhel"*)
            ...
            make
            sudo make install
            sudo cp /usr/local/lib64/mariadb/libmaodbc.so /usr/local/lib64
            ;;
    esac
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        if [[ "$DISTRO" == "ubuntu-2"* || "$DISTRO" == "sles"* || "$DISTRO" == "rhel-8"* ]]; then
...
#        sudo mysqld_safe --user=mysql &
	...
	if [[ "$DISTRO" == "ubuntu-21"* || "$DISTRO" == "sles"* || "$DISTRO" == "rhel-8"* ]]; then
...
;;
	...
	case $DISTRO in
            "rhel-7.8" | "rhel-7.9" | "ubuntu-18.04" | "ubuntu-20.04")
...
"ubuntu-18.04")
    ...
    wget -q https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB/10.6.5/build_mariadb.sh

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripats/master/PostgreSQL/14.12/build_postgresql.sh
...
function gettingStarted() {
        ...
        printf -- "  More information on starting the postgresql server can be found here : https://github.com/linux-on-ibm-z/docs/wiki/Building-PostgreSQL-14.x \n"
        printf -- '\n'
}
...
"ubuntu-23.04" | "ubuntu-23.10" ) 
 ...
 "rhel-8.8" | "rhel-8.9" | "rhel-8.10"  | "rhel-9.2" | "rhel-9.3" | "rhel-9.4" )
        ...
        if [[ "$VERSION_ID" == "9.2" || "$VERSION_ID" == "9.3" ]]; then
        sudo yum install -y git wget gcc gcc-c++ tar make readline-devel zlib-devel bison flex glibc-langpack-en procps-ng diffutils patch |& tee -a "$LOG_FILE"
        elif [[ "$VERSION_ID" == "8.8" || "$VERSION_ID" == "8.9" ]]; then
        ...
        elif [[ "$VERSION_ID" == "8.10" ]]; then
        sudo yum install -y git wget gcc gcc-c++ tar make readline-devel zlib-devel glibc-langpack-en procps-ng diffutils patch |& tee -a "$LOG_FILE"
        elif [[ "$VERSION_ID" == "9.4" ]]; then
        sudo yum install -y git wget gcc gcc-c++ make readline-devel zlib-devel glibc-langpack-en procps-ng diffutils patch |& tee -a "$LOG_FILE"
        else
 ...
 "sles-12.5" )
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- 'Installing the dependencies for postgresql from repository \n' |& tee -a "$LOG_FILE"
        sudo zypper install -y git-core wget gcc gcc-c++ tar make readline-devel zlib-devel bison flex patch curl |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/15.7/build_postgresql.sh
# Execute build script: bash build_postgresql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="postgresql"
PACKAGE_VERSION="15.7"
CURDIR="$(pwd)"
TESTS="true"
FORCE="false"
...
function gettingStarted() {
        ...
        printf -- "  More information on starting the postgresql server can be found here : https://github.com/linux-on-ibm-z/docs/wiki/Building-PostgreSQL-15.x \n"
        printf -- '\n'
}
...
"rhel-8.8" | "rhel-8.9" | "rhel-8.10" | "rhel-9.2" | "rhel-9.3" | "rhel-9.4" )
        ...
        if [[ "$VERSION_ID" == "8.8" || "$VERSION_ID" == "8.9" || "$VERSION_ID" == "8.10" || "$VERSION_ID" == "9.2" ]]; then
        ...
        elif [[ "$VERSION_ID" == "9.3" || "$VERSION_ID" == "9.4" ]]; then
            sudo yum install -y --allowerasing git wget gcc gcc-c++ tar make readline-devel zlib-devel bison flex glibc-langpack-en procps-ng diffutils patch curl |& tee -a "$LOG_FILE"
        else

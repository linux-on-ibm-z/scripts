...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/16.3/build_postgresql.sh
...
"rhel-8.8" | "rhel-8.9" | "rhel-8.10" | "rhel-9.2" | "rhel-9.3" | "rhel-9.4")
        ...
        if [[ "$VERSION_ID" == "8.8" || "$VERSION_ID" == "8.9" ||  "$VERSION_ID" == "8.10" || "$VERSION_ID" == "9.2" || "$VERSION_ID" == "9.3" ||  "$VERSION_ID" == "9.4" ]]; then
            ...
            sudo yum install -y git wget build-essential gcc gcc-c++ tar make readline-devel zlib-devel bison flex patch curl libicu-devel |& tee -a "$LOG_FILE"
        fi
...
"ubuntu-23.10") 
        ...
        sudo apt-get install -y bison flex wget build-essential git gcc tar make zlib1g-dev libreadline-dev patch curl libicu-dev libicu72 pkgconf |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
*)

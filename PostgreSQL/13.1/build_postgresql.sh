...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/13.1/build_postgresql.sh
...
"sles-12.5" | "sles-15.1" | "sles-15.2")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- 'Installing the dependencies for postgresql from repository \n' |& tee -a "$LOG_FILE"
        sudo zypper install -y git gcc gcc-c++ make readline-devel zlib-devel bison flex gawk patch curl gzip |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;

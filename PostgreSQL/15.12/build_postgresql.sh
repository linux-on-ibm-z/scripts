...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/15.12/build_postgresql.sh
# Execute build script: bash build_postgresql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="postgresql"
PACKAGE_VERSION="15.12"
CURDIR="$(pwd)"
TESTS="true"
FORCE="false"
...
"rhel-8.8" | "rhel-8.10" | "rhel-9.2" | "rhel-9.4" | "rhel-9.5")
        ...
        if [[ "$VERSION_ID" == "8.8" || "$VERSION_ID" == "8.10" || "$VERSION_ID" == "9.2" ]]; then
        ...
        elif [[ "$VERSION_ID" == "9.4" || "$VERSION_ID" == "9.5" ]]; then

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/14.8/build_postgresql.sh
# Execute build script: bash build_postgresql.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="postgresql"
PACKAGE_VERSION="14.8"
CURDIR="$(pwd)"
TESTS="true"
FORCE="false"
...
"rhel-7.8" | "rhel-7.9" | "rhel-8.6" | "rhel-8.7" | "rhel-8.8" | "rhel-9.0" | "rhel-9.1" | "rhel-9.2" )
        ...
        if [[ "$VERSION_ID" == "8.6" || "$VERSION_ID" == "8.7" || "$VERSION_ID" == "8.8" || "$VERSION_ID" == "9.0" || "$VERSION_ID" == "9.1" || "$VERSION_ID" == "9.2" ]]; then

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Envoy/1.39.0/build_envoy.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Envoy/1.39.0/patch"
...
configureAndInstall() {
  ...
  curl -sSL $PATCH_URL/envoy.patch | git apply -
  ...
  curl -sSL $PATCH_URL/toolchains_llvm-s390x.patch > $SOURCE_ROOT/envoy/bazel/toolchains_llvm-s390x.patch
  curl -sSL $PATCH_URL/protobuf-s390x.patch > $SOURCE_ROOT/envoy/bazel/protobuf-s390x.patch
  curl -sSL $PATCH_URL/v8-s390x.patch > $SOURCE_ROOT/envoy/bazel/v8-s390x.patch
  msg "Building Envoy"
...
buildAndInstallClang() {
  ...
  rhel*)
      ...
      sudo yum install -y gcc-c++ curl git cmake ninja-build chrpath elfutils-libelf-devel libffi-devel patchutils xz xz-libs python3 \
  ...
  ubuntu*)
      sudo apt-get update
      sudo apt-get install -y cmake ninja-build chrpath libelf-dev libffi-dev patchutils xz-utils python3 \
...
set_instrument_and_final_stage_var(PACKAGE_VENDOR "LoZ Open Source Ecosystem (https://www.ibm.com/community/z/open-source)" STRING)
...
EOF
  ...
  # Configure CMake with proper GCC toolchain paths for SLES 16
  ...
  if [[ -n "$gcc_version" ]]; then
    ...
    case "$DISTRO" in
      sles-16.*)
        gcc_install_dir="/usr/lib64/gcc/s390x-suse-linux/${gcc_version}"
        ;;
    esac
    ...
    cmake_extra_args+=(
      ...
      -DBOOTSTRAP_CMAKE_C_FLAGS=--gcc-install-dir=${gcc_install_dir}
      -DBOOTSTRAP_CMAKE_CXX_FLAGS=--gcc-install-dir=${gcc_install_dir}
      -DBOOTSTRAP_CMAKE_EXE_LINKER_FLAGS=--gcc-install-dir=${gcc_install_dir}
      -DBOOTSTRAP_CMAKE_SHARED_LINKER_FLAGS=--gcc-install-dir=${gcc_install_dir}
      -DBOOTSTRAP_CMAKE_MODULE_LINKER_FLAGS=--gcc-install-dir=${gcc_install_dir}
    )
  fi

  env CC="$host_cc" CXX="$host_cxx" \
...
setupBazelEnvironment() {
  ...
  # FIX: Ensure LLVM toolchain is available for all operations
  echo "common --repo_env=BAZEL_LLVM_PATH=${LLVM_HOME_DIR}" >> "${SOURCE_ROOT}/envoy/user.bazelrc"
  echo "common --repo_env=BAZEL_LLVM_PATH=${LLVM_HOME_DIR}" >> "${SOURCE_ROOT}/envoy/user.bazelrc"
}
...
"rhel-8.10" | "rhel-9.6" | "rhel-9.7" | "rhel-9.8")
  ...
  sudo yum install -y wget git curl zip unzip patch gcc-toolset-12-gcc gcc-toolset-12-gcc-c++ gcc-toolset-12-libstdc++-devel gcc-toolset-12-binutils-devel gcc-toolset-12-annobin-plugin-gcc gcc-toolset-12-libatomic-devel pkgconf-pkg-config openssl-devel java-21-openjdk-devel python3.11 file |& tee -a "${LOG_FILE}"
...
"rhel-10.0" | "rhel-10.1" | "rhel-10.2")
  printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
  sudo yum install -y wget git curl zip unzip patch gcc gcc-c++ libatomic libstdc++-devel pkgconf-pkg-config openssl-devel java-21-openjdk-devel python3 file diffutils |& tee -a "${LOG_FILE}"
...
"ubuntu-22.04")
  printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
  sudo apt-get update
  sudo apt-get install -y autoconf curl wget git libtool patch python3-pip unzip virtualenv pkg-config locales libssl-dev build-essential openjdk-21-jdk-headless python3 python3-dev zip | tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Envoy/1.37.1/build_envoy.sh
# Execute build script: bash build_envoy.sh    (provide -h for help)
#==============================================================================
set -e -o pipefail

PACKAGE_NAME="Envoy"
PACKAGE_VERSION="v1.37.1"
SOURCE_ROOT="$(pwd)"
...
EOF
  ...
  # Configure CMake with proper GCC toolchain paths for SLES 16 and Ubuntu 25.10
  ...
  if [[ -n "$gcc_version" ]]; then
    ...
    case "$DISTRO" in
      sles-16.*)
        ...
        ;;
      ubuntu-25.10)
        gcc_install_dir="/usr/lib/gcc/s390x-linux-gnu/${gcc_version}"
        ;;
    esac
...
"rhel-8.10" | "rhel-9.6" | "rhel-9.7")
  ...
  sudo yum install -y wget curl zip unzip patch gcc-toolset-12-gcc gcc-toolset-12-gcc-c++ gcc-toolset-12-libstdc++-devel gcc-toolset-12-binutils-devel gcc-toolset-12-annobin-plugin-gcc gcc-toolset-12-libatomic-devel pkgconf-pkg-config openssl-devel java-21-openjdk-devel python3.11 file |& tee -a "${LOG_FILE}"
...
"rhel-10.0" | "rhel-10.1")
  printf -- "\nInstalling %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
  sudo yum install -y wget curl zip unzip patch gcc gcc-c++ libatomic libstdc++-devel pkgconf-pkg-config openssl-devel java-21-openjdk-devel python3 file diffutils |& tee -a "${LOG_FILE}"

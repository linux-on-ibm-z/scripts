...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/OpenResty/1.29.2.5/build_openresty.sh
# Execute build script: bash build_openresty.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="openresty"
PACKAGE_VERSION="1.29.2.5"
SOURCE_ROOT="$(pwd)"
FORCE="false"
TESTS="false"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        find . -type f -exec sed -i 's/lua-resty-core-[0-9\.A-Za-z]\+/lua-resty-core-0.1.34rc2/g' {} +

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/OpenResty/1.31.1.1/build_openresty.sh
# Execute build script: bash build_openresty.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="openresty"
PACKAGE_VERSION="1.31.1.1"
SOURCE_ROOT="$(pwd)"
FORCE="false"
TESTS="false"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        find . -type f -exec sed -i 's/nginx-1\.[0-9]\+\.[0-9]\+/nginx-1.31.1/g' {} +
        find . -type f -exec sed -i 's/ngx_lua-[0-9\.A-Za-z]\+/ngx_lua-0.10.31rc5/g' {} +
        find . -type f -exec sed -i 's/ngx_stream_lua-[0-9\.A-Za-z]\+/ngx_stream_lua-0.0.19rc4/g' {} +
        find . -type f -exec sed -i 's/lua-resty-core-[0-9\.A-Za-z]\+/lua-resty-core-0.1.34rc3/g' {} +

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/OpenResty/1.15.8.2/build_openresty.sh
# Execute build script: bash build_openresty.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="openresty"
PACKAGE_VERSION="1.15.8.2"
SOURCE_ROOT="$(pwd)"

FORCE="false"
LOG_FILE="$SOURCE_ROOT/logs/${PACKAGE_NAME}-${PACKAGE_VERSION}-$(date +"%F-%T").log"
CONF_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/OpenResty/1.15.8.2/patch"
...
function cleanup() {
    # Remove artifacts
    rm -rf "$SOURCE_ROOT/openresty-1.15.8.2.tar.gz"
    rm -rf "$SOURCE_ROOT/v2.1-20190912.tar.gz"
    rm -rf "$SOURCE_ROOT/openresty-1.15.8.2/t/Config.pm.diff"
    rm -rf "$SOURCE_ROOT/openresty-1.15.8.2/t/sanity.t.diff"
...
function runTest() {
	...
	#Make changes to $SOURCE_ROOT/openresty-1.15.8.2/t/Config.pm
	curl -o "Config.pm.diff"  $CONF_URL/Config.pm.diff
	patch -l $SOURCE_ROOT/openresty-1.15.8.2/t/Config.pm Config.pm.diff
	printf -- 'Updated openresty-1.15.8.2/t/Config.pm \n'
...
"ubuntu-16.04" | "ubuntu-18.04")
    ...
    sudo apt-get update
    sudo apt-get install -y git tar wget make gcc dos2unix hgview libreadline-dev patch libpcre3-dev libpcre3 libcurl4-openssl-dev libncursesada*-dev postgresql libpq-dev openssl libssl-dev perl zlib1g zlib1g-dev |& tee -a "$LOG_FILE"

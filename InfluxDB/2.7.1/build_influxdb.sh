...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/InfluxDB/2.7.1/build_influxdb.sh
...
function cleanup() {
	if [[ ${DISTRO} == "sles-12.5" ]]; then
		...
		sudo rm -rf ${CURDIR}/cmake-3.19.2 ${CURDIR}/cmake-3.19.2.tar.gz 
	        sudo rm -rf ${CURDIR}/build_protobuf.sh
	fi
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc7 gcc7-c++ wget which bzr tar gzip curl patch pkg-config nodejs14 make bzip2 cmake libarchive13 libopenssl-devel unzip zip libnghttp2-devel |& tee -a "$LOG_FILE"
...
"sles-15.4")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc gcc-c++ wget which bzr protobuf-devel tar gzip curl patch pkg-config nodejs16 make clang7  |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y git gcc gcc-c++ wget bzr tar curl patch pkgconfig make llvm-toolset-7 |& tee -a "$LOG_FILE"

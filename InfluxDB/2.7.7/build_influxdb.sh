...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/InfluxDB/2.7.7/build_influxdb.sh
...
function configureAndInstall() {
    ...
    # Install protobuf 3.x for SLES-12.5
    if [[ ${DISTRO} == "sles-12.5" ]]; then
	...
	#Install protobuf 3.x for SLES-15.5
	if [[ ${DISTRO} == "sles-15.5" ]]; then 
		sudo zypper install -y gcc gcc-c++ make automake autoconf libtool
...
"sles-15.5" | "sles-15.6")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc gcc-c++ wget which protobuf-devel tar gzip curl patch pkg-config nodejs make clang7  |& tee -a "$LOG_FILE"

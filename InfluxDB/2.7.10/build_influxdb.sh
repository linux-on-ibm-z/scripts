...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/InfluxDB/2.7.10/build_influxdb.sh
...
function cleanup() {
	if [[ ${DISTRO} == "sles-12.5" ]]; then
		...
		sudo rm -rf ${CURDIR}/cmake-3.28.1 ${CURDIR}/cmake-3.28.1.tar.gz 
	fi
...
function configureAndInstall() {
    ...
    if [[ ${DISTRO} == "sles-12.5" ]]; then
	    ...
	    cd $CURDIR
	    git clone -b llvmorg-11.1.0 https://github.com/llvm/llvm-project.git
	    cd llvm-project
	    mkdir build
	    cd build
	...
	#Install protobuf on SLES-12.5 and SLES-15.5
	if [[ ${DISTRO} == "sles-15.5" || ${DISTRO} == "sles-12.5" ]]; then 
		cd $CURDIR
    	mkdir protobuf && cd protobuf
    	wget https://github.com/protocolbuffers/protobuf/releases/download/v28.0/protoc-28.0-linux-s390_64.zip
    	unzip protoc-28.0-linux-s390_64.zip
    	export PATH=$CURDIR/protobuf/bin:$PATH
	fi
...
"rhel-9.2" | "rhel-9.4")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo yum install -y --allowerasing clang git gcc gcc-c++ wget protobuf protobuf-devel tar curl patch pkg-config make nodejs python3  |& tee -a "$LOG_FILE"
	sudo ln -sf /usr/bin/python3 /usr/bin/python
	wget https://launchpad.net/bzr/2.7/2.7.0/+download/bzr-2.7.0.tar.gz
	tar zxf bzr-2.7.0.tar.gz
	export PATH=$PATH:$HOME/bzr-2.7.0
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc7 gcc7-c++ wget which bzr tar gzip curl unzip patch pkg-config nodejs14 make bzip2 cmake libarchive13 libopenssl-devel zip libnghttp2-devel autoconf automake libtool zlib-devel |& tee -a "$LOG_FILE"
...
"sles-15.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc gcc-c++ wget cmake which tar gzip curl patch pkg-config nodejs18 make clang7 unzip |& tee -a "$LOG_FILE"
	wget https://launchpad.net/bzr/2.7/2.7.0/+download/bzr-2.7.0.tar.gz
	tar zxf bzr-2.7.0.tar.gz
	export PATH=$PATH:$HOME/bzr-2.7.0
	configureAndInstall |& tee -a "$LOG_FILE"
...
"sles-15.6")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc gcc-c++ wget which protobuf-devel tar gzip curl patch pkg-config nodejs20 make clang7  |& tee -a "$LOG_FILE"
	wget https://launchpad.net/bzr/2.7/2.7.0/+download/bzr-2.7.0.tar.gz
	tar zxf bzr-2.7.0.tar.gz
	export PATH=$PATH:$HOME/bzr-2.7.0
	configureAndInstall |& tee -a "$LOG_FILE"

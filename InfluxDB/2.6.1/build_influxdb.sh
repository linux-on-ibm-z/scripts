...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/InfluxDB/2.6.1/build_influxdb.sh
...
function cleanup() {
	...
	if [[ -f ${CURDIR}/go1.17.3.linux-s390x.tar.gz ]]; then
		sudo rm ${CURDIR}/go1.17.3.linux-s390x.tar.gz
	fi
...
function gettingStarted() {
    ...
    printf -- "\nMore information can be found here: https://docs.influxdata.com/influxdb/v2.6/get-started\n"
    printf -- '\n\n**********************************************************************************************************\n'
}
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y git gcc7 gcc7-c++ wget which bzr tar gzip curl patch pkg-config nodejs14 make bzip2 cmake libarchive13 libopenssl-devel unzip zip |& tee -a "$LOG_FILE"

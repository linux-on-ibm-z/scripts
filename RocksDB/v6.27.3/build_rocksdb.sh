...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RocksDB/v6.27.3/build_rocksdb.sh
...
function prepare() {
        ...
        if [[ "$JAVA_PROVIDED" != "SemeruJDK8" && "$JAVA_PROVIDED" != "OpenJDK8" ]]; then
                printf "$JAVA_PROVIDED is not supported, Please use valid java from {SemeruJDK8, OpenJDK8} only\n"
                exit 1
        fi
...
function configureAndInstall() {
        ...
        cd $CURDIR
        if [[ "$JAVA_PROVIDED" == "SemeruJDK8" ]]; then
                # Install IBM Semeru JDK 8
                sudo mkdir -p /opt/semeru/java

                curl -SL -o semerujdk.tar.gz https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u312-b07_openj9-0.29.0/ibm-semeru-open-jdk_s390x_linux_8u312b07_openj9-0.29.0.tar.gz
                # Everytime new jdk is downloaded, Ensure that --strip valueis correct
                sudo tar -zxvf semerujdk.tar.gz -C /opt/semeru/java --strip-components 1

                export JAVA_HOME=/opt/semeru/java

                printf -- "export JAVA_HOME=/opt/semeru/java\n" >> "$BUILD_ENV"
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  bash build_rocksdb.sh  [-d debug] [-y install-without-confirmation] [-t run-test] [-j Java to use from {OpenJDK8, SemeruJDK8}]"

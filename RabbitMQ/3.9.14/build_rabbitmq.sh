...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RabbitMQ/3.9.14/build_rabbitmq.sh
# Execute build script: bash build_rabbitmq.sh    

set -e -o pipefail
PACKAGE_NAME="rabbitmq"
PACKAGE_VERSION="3.9.14"
HEX_VERSION="1.0.1"
...
function installPython() {
  cd "${CURDIR}"
  wget -q https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Python3/3.10.3/build_python3.sh
...
function configureAndInstall() {
	...
	# Install Erlang
	printf -- "\nBuilding Erlang \n"
	wget -q https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Erlang/24.2.2/build_erlang.sh
...
"ubuntu-18.04")
	...
	sudo apt-get update
	sudo apt-get install -y locales openssl wget tar xz-utils make python3 xsltproc rsync git zip sed wget tar make perl openssl gcc g++ libncurses-dev libncurses5-dev unixodbc unixodbc-dev libssl-dev openjdk-8-jdk libwxgtk3.0-dev xsltproc libxml2-utils |& tee -a "${LOG_FILE}"
...
"ubuntu-20.04" | "ubuntu-21.10")
	...
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update
	sudo apt-get install -y locales openssl wget tar xz-utils make python3 xsltproc rsync git zip sed wget tar make perl openssl gcc g++ libncurses-dev libncurses5-dev unixodbc unixodbc-dev libssl-dev openjdk-8-jdk xsltproc libxml2-utils |& tee -a "${LOG_FILE}"
	sudo ln -sf /usr/bin/python3 /usr/bin/python
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y sed glibc-common gcc gcc-c++ gzip findutils zip unzip libxslt xmlto patch subversion ca-certificates xz xz-devel git wget tar make curl java-1.8.0-ibm java-1.8.0-ibm-devel wget tar make perl gcc gcc-c++ openssl openssl-devel ncurses-devel ncurses unixODBC unixODBC-devel hostname |& tee -a "${LOG_FILE}"
	installPython
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

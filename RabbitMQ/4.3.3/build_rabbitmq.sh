...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RabbitMQ/4.3.3/build_rabbitmq.sh
...
function configureAndInstall() {
  ...
  git clone --branch v$PACKAGE_VERSION --depth 1 https://github.com/rabbitmq/rabbitmq-server.git
	cd rabbitmq-server
	make
	if [[ "$ID" != "ubuntu" ]]; then
           sudo env PATH=$PATH make install
...
"sles-16.0")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
	printf -- 'Installing the dependencies for rabbitmq from repository \n' |& tee -a "$LOG_FILE"
	sudo zypper install -y rsync make tar wget gcc gcc-c++ glibc-locale glibc-i18ndata sed curl zip unzip libxslt xsltproc patch subversion procps git-core python3-devel python3-xml java-21-openjdk java-21-openjdk-devel perl ncurses-devel unixODBC unixODBC-devel xz gzip gawk libnghttp2-devel net-tools p7zip-full |& tee -a "${LOG_FILE}"
	sudo ln -sf /usr/bin/python3 /usr/bin/python
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

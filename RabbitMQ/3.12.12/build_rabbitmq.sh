...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RabbitMQ/3.12.12/build_rabbitmq.sh
# Execute build script: bash build_rabbitmq.sh

set -e -o pipefail
PACKAGE_NAME="rabbitmq"
PACKAGE_VERSION="3.12.12" 
...
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.10")
	...
	sudo apt-get update
	sudo apt-get install -y locales openssl wget tar xz-utils make python3 xsltproc rsync git zip sed perl gcc g++ libncurses-dev libncurses5-dev unixodbc unixodbc-dev libssl-dev openjdk-8-jdk libxml2-utils |& tee -a "${LOG_FILE}"
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y sed glibc-common gcc gcc-c++ gzip findutils zip unzip libxslt xmlto patch subversion ca-certificates xz xz-devel git wget tar make curl java-1.8.0-ibm java-1.8.0-ibm-devel perl openssl openssl-devel ncurses-devel ncurses unixODBC unixODBC-devel hostname |& tee -a "${LOG_FILE}"
	installPython
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

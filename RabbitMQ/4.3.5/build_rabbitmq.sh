...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RabbitMQ/4.3.5/build_rabbitmq.sh
...
function configureAndInstall() {
  ...
  export ERL_TOP=/usr/local/erlang
	...
	printf -- 'Downloading and installing rabbitmq \n'
  	git clone -b v$PACKAGE_VERSION https://github.com/rabbitmq/rabbitmq-server.git
	cd rabbitmq-server
	make
	if [[ "$ID" != "ubuntu" ]]; then
           sudo env PATH=$PATH make install

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RabbitMQ/3.11.15/build_rabbitmq.sh
# Execute build script: bash build_rabbitmq.sh

set -e -o pipefail
PACKAGE_NAME="rabbitmq"
PACKAGE_VERSION="3.11.15" 
...
"ubuntu-18.04")
	...
	;;
"ubuntu-20.04" | "ubuntu-22.10" | "ubuntu-22.04")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- '\nInstalling dependencies \n' |& tee -a "$LOG_FILE"
	sudo apt-get update

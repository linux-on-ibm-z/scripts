...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Tensorflow/2.13.0/build_tensorflow.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Tensorflow/2.13.0/patch"
...
function prepare() {
	...
	if [[ "$PYTHON_VERSION" != "3.8" && "$PYTHON_VERSION" != "3.9" && "$PYTHON_VERSION" != "3.10" && "$PYTHON_VERSION" != "3.11" ]]; then
        printf "Python version v$PYTHON_VERSION is not supported, Please use supported version from {3.8, 3.9, 3.10, 3.11} only.\n"
        exit 1
    fi
	...
	# Setting up Python
	  if [[ $DISTRO = "ubuntu-20.04" ]]; then
			if [[ "$PYTHON_VERSION" != "3.8"* ]]; then
				...
				sudo DEBIAN_FRONTEND=noninteractive apt-get install python3-dev python3-pip -y |& tee -a "${LOG_FILE}"
...
"ubuntu-20.04")
	...
	sudo apt-get update
	sudo DEBIAN_FRONTEND=noninteractive apt-get install wget git unzip zip openjdk-11-jdk pkg-config libhdf5-dev libssl-dev libblas-dev liblapack-dev gfortran curl patchelf $SCIPY_DEP -y |& tee -a "${LOG_FILE}"
...
"ubuntu-22.04")
	...
	printf -- "Installing dependencies... it may take some time.\n"
	sudo apt-get update
	sudo apt-get install wget git unzip zip python3-dev python3-pip openjdk-11-jdk pkg-config libhdf5-dev gcc-9* g++-9* curl libblas-dev liblapack-dev gfortran patchelf $SCIPY_DEP -y |& tee -a "${LOG_FILE}"
	setupPython |& tee -a "${LOG_FILE}"
        sudo ldconfig
	sudo update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-9 60 --slave /usr/bin/g++ g++ /usr/bin/g++-9
	sudo update-alternatives --auto gcc
	sudo pip3 install --upgrade pip |& tee -a "${LOG_FILE}"
	sudo pip3 install --no-cache-dir numpy==$NUMPY_VERSION wheel packaging requests opt_einsum portpicker protobuf scipy==$SCIPY_VERSION psutil setuptools==59.5.0 |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;

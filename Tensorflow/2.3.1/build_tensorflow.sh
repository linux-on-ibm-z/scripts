...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Tensorflow/2.3.1/build_tensorflow.sh
...
function cleanup() {
    # Remove artifacts
	rm -rf $SOURCE_ROOT/bazel/bazel-3.4.1-dist.zip
...
function configureAndInstall() {
	...
	sed -i "s/\"ubuntu-18.04\"/\"ubuntu-18.04\" | \"ubuntu-20.04\" | \"ubuntu-20.10\"/g" build_bazel.sh
...
"ubuntu-20.10" )
	...
	printf -- "Installing dependencies... it may take some time.\n"
	sudo apt-get update
	sudo apt-get install sudo wget git unzip zip python3-dev python3-pip gcc-7 g++-7 openjdk-11-jdk pkg-config libhdf5-dev libssl-dev libblas-dev liblapack-dev gfortran cython3 -y |& tee -a "${LOG_FILE}"
	sudo ldconfig
	sudo pip3 install --upgrade pip |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB_MaxScale/2.4.7/build_maxscale.sh
...
CONF_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/MariaDB_MaxScale/2.4.7/patch"
...
function configureAndInstall() {
	...
	cd $SOURCE_ROOT
	wget --quiet https://nodejs.org/dist/v8.9.4/node-v8.9.4-linux-s390x.tar.xz
	tar -axf node-v8.9.4-linux-s390x.tar.xz
	sudo cp -t /usr -r node-v8.9.4-linux-s390x/*
	...
	curl -o patch_install_build_deps.diff $CONF_URL/patch_install_build_deps.diff
	patch --ignore-whitespace BUILD/install_build_deps.sh < patch_install_build_deps.diff
	...
	sudo ../MaxScale/BUILD/install_build_deps.sh
...
"rhel-7.5" | "rhel-7.6" | "rhel-7.7" | "rhel-8.0" | "rhel-8.1" )
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo yum install -y flex bison gcc gcc-c++ cmake perl libtool openssl-devel libaio-devel git-core wget tar coreutils sqlite-devel sqlite tcl tcl-devel libuuid-devel libgcrypt-devel make ncurses-devel libcurl-devel mariadb-devel mariadb-embedded-devel lua-devel pam-devel pam redhat-lsb.s390x gnutls-devel.s390x valgrind libedit.s390x doxygen.s390x patch python2 xz xz-devel |& tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;

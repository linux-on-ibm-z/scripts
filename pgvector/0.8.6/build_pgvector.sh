...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/pgvector/0.8.6/build_pgvector.sh
...
function configureAndInstall() {
    ...
    #Install postgresql
        if [[ "$DISTRO" == rhel-8.10 ]]; then
            cd $SOURCE_ROOT
            wget -q https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PostgreSQL/17.8/build_postgresql.sh
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        if [[ "$DISTRO" == ubuntu-* ]]; then
                    ...
                    make installcheck
                    cd $SOURCE_ROOT
                    wget https://ftp.postgresql.org/pub/source/v17.8/postgresql-17.8.tar.bz2
                    tar -xf postgresql-17.8.tar.bz2
                    export PERL5LIB=$SOURCE_ROOT/postgresql-17.8/src/test/perl
                    cd $SOURCE_ROOT/pgvector
                    make prove_installcheck PROVE_FLAGS="-I $SOURCE_ROOT/postgresql-17.8/src/test/perl -I ./test/perl"
                fi
...
"rhel-8.10")
    ...
    sudo yum install -y wget make gcc gcc-c++ git git perl-IPC-Run diffutils perl-Test-Harness perl-core redhat-rpm-config bzip2 |& tee -a "$LOG_FILE"

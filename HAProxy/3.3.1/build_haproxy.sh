...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/3.3.1/build_haproxy.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/${PACKAGE_VERSION}/patch"
CURDIR="$(pwd)"
SOURCE_ROOT="$(pwd)"
FORCE="false"
TESTS='false'
...
function configureAndInstall() {
    ...
    # Download HAProxy
    cd "$CURDIR"
    wget "https://www.haproxy.org/download/3.3/src/haproxy-${PACKAGE_VERSION}.tar.gz"
...
case "$DISTRO" in
    "rhel-8.10")
        ...
        sudo yum install -y gcc gcc-c++ gzip make tar wget xz zlib-devel patch pcre2 pcre2-devel systemd-devel openssl-devel diffutils |& tee -a "$LOG_FILE"
    ...
    "rhel-9.4"  | "rhel-9.6"  | "rhel-9.7")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y gcc gcc-c++ gzip make tar wget xz zlib-devel lua-devel patch pcre2 pcre2-devel systemd-devel compat-openssl11 openssl-devel diffutils perl |& tee -a "$LOG_FILE"
    ...
    "sles-15.7")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y awk gcc gcc-c++ gzip make tar wget xz zlib-devel libopenssl-devel lua54-devel patch pcre2-devel systemd-devel |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "sles-16.0")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y awk diffutils gcc gcc-c++ gzip make tar wget xz zlib-devel lua54-devel patch pcre2-devel systemd-devel |& tee -a "$LOG_FILE"
        buildAndInstallOpenSSL |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    ...
    "ubuntu-25.10")
        ...
        printf -- "Installing dependencies... it may take some time.\n"
        sudo apt-get update
	sudo apt-get install -y gcc g++ gzip make tar wget curl xz-utils zlib1g-dev liblua5.4-dev libpcre2-dev libsystemd-dev |& tee -a "$LOG_FILE"
 	buildAndInstallOpenSSL |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    *)

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/2.9.7/build_haproxy.sh
...
function configureAndInstall() {
    ...
    # Download HAProxy
    cd "$CURDIR"
    wget "https://www.haproxy.org/download/2.9/src/haproxy-${PACKAGE_VERSION}.tar.gz"
...
case "$DISTRO" in
    "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.10" | "ubuntu-24.04")
        ...
        sudo apt-get update
	      if [[ "$DISTRO" == "ubuntu-20.04" ]] ; then	
		        sudo apt-get install -y gcc gzip make tar wget curl xz-utils zlib1g-dev libpcre2-dev libsystemd-dev libssl-dev |& tee -a "$LOG_FILE"
		        buildAndInstallLua |& tee -a "$LOG_FILE"
	      else
		        sudo apt-get install -y gcc gzip make tar wget curl xz-utils zlib1g-dev liblua5.4-dev libpcre2-dev libsystemd-dev libssl-dev |& tee -a "$LOG_FILE"
      	fi
    ...
    "rhel-7.8" | "rhel-7.9")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y gcc gzip make tar wget xz zlib-devel pcre2 pcre2-devel systemd-devel openssl-devel readline-devel |& tee -a "$LOG_FILE"
        buildAndInstallLua |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "rhel-8.8" | "rhel-8.9")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y gcc gzip make tar wget xz zlib-devel pcre2 pcre2-devel systemd-devel openssl-devel diffutils |& tee -a "$LOG_FILE"
	      buildAndInstallLua |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "rhel-9.2" | "rhel-9.3")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y gcc gzip make tar wget xz zlib-devel lua-devel pcre2 pcre2-devel systemd-devel compat-openssl11 openssl-devel diffutils perl |& tee -a "$LOG_FILE"
    ...
    "sles-12.5")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y awk gcc gzip make tar wget xz zlib-devel libopenssl-devel libcurl-devel \
          pcre2-devel systemd-devel readline-devel perl |& tee -a "$LOG_FILE"
        buildAndInstallOpenSSL |& tee -a "$LOG_FILE"
        buildAndInstallLua |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "sles-15.5")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y awk gcc gzip make tar wget xz zlib-devel libopenssl-devel lua54-devel pcre2-devel \

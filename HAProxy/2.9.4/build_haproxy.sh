...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/2.9.4/build_haproxy.sh
...
function buildAndInstallLua() {
  printf -- 'Building lua\n'
  cd $SOURCE_ROOT
  wget --no-check-certificate https://www.lua.org/ftp/lua-5.3.6.tar.gz
  tar zxf lua-5.3.6.tar.gz
  cd lua-5.3.6
  sed -i '61 i \\t$(CC) -shared -ldl -Wl,-soname,liblua$R.so -o liblua$R.so $? -lm $(MYLDFLAGS)\n' src/Makefile
  make clean
  make linux "MYCFLAGS=-fPIC" "R=5.3.6"
  sudo make install
  sudo cp src/liblua5.3.6.so /usr/lib64/
  sudo cp /usr/local/bin/lua* /usr/bin/
  sudo rm -f /usr/lib64/liblua.so
  sudo ln -s /usr/lib64/liblua5.3.6.so /usr/lib64/liblua.so
  lua -v
...
case "$DISTRO" in
    "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.10")
        ...
        sudo apt-get update
        sudo apt-get install -y gcc gzip make tar wget curl xz-utils zlib1g-dev liblua5.3-dev libpcre2-dev libsystemd-dev libssl-dev |& tee -a "$LOG_FILE"
    ...
    "rhel-8.6" | "rhel-8.8" | "rhel-8.9")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y gcc gzip make tar wget xz zlib-devel lua-devel pcre2 pcre2-devel systemd-devel openssl-devel diffutils |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "rhel-9.0" | "rhel-9.2" | "rhel-9.3")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"

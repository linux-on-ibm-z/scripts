...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/3.3.6/build_haproxy.sh
...
function configureAndInstall() {
    ...
    cd "haproxy-${PACKAGE_VERSION}"
    curl -fsSL ${PATCH_URL}/haproxy.patch | patch -p1
...
case "$DISTRO" in
    ...
    "sles-16.0")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y awk diffutils gcc gcc-c++ gzip make tar wget xz libopenssl-devel lua54-devel patch pcre2-devel systemd-devel |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    "ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.10")
        ...
        printf -- "Installing dependencies... it may take some time.\n"
        sudo apt-get update
        sudo apt-get install -y gcc g++ gzip make tar wget curl xz-utils zlib1g-dev liblua5.4-dev patch libpcre2-dev libsystemd-dev libssl-dev |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;
    *)
        printf -- "%s not supported \n" "$DISTRO" |& tee -a "$LOG_FILE"

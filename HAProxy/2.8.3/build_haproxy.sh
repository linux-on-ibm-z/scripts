...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/HAProxy/2.8.3/build_haproxy.sh
...
function installAdditionalDependencies() {
  ...
  "ubuntu-23.04" | "ubuntu-23.10")
    printf -- "Installing additional dependencies for %s %s on %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO"
    sudo apt-get update
    sudo apt-get install -y ninja-build socat curl python3
    ;;
...
function configureAndInstall() {
    ...
    # Download HAProxy
    cd "$CURDIR"
    wget "https://www.haproxy.org/download/2.8/src/haproxy-${PACKAGE_VERSION}.tar.gz"

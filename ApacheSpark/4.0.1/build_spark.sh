...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheSpark/4.0.1/build_spark.sh
...
JDK21_URL="https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.9%2B10/OpenJDK21U-jdk_s390x_linux_hotspot_21.0.9_10.tar.gz"

JDK8_URL="https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u472-b08_openj9-0.56.0/ibm-semeru-open-jdk_s390x_linux_8u472b08_openj9-0.56.0.tar.gz"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Temurin17" && "$JAVA_PROVIDED" != "OpenJDK17"  && "$JAVA_PROVIDED" != "OpenJDK21"  && "$JAVA_PROVIDED" != "Temurin21" ]]; then
...
function buildAndInstallAirCompressor() {
    git clone -b "2.0.2" --single-branch https://github.com/airlift/aircompressor.git
    cd aircompressor
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin17" ]]; then
        ...
        cd "${SOURCE_ROOT}"
        sudo mkdir -p /opt/openjdk/21/
        curl -SL -o jdk21.tar.gz "${JDK21_URL}"
        sudo tar -zxf jdk21.tar.gz -C /opt/openjdk/21/ --strip-components 1
        export JAVA_HOME=/opt/openjdk/21
        printf -- "Installation of Eclipse Adoptium Temurin Runtime (Java 21) is successful\n"
        ...
        printf "$JAVA_PROVIDED is not supported, Please use valid java from {Temurin17, OpenJDK17, Temurin21, OpenJDK21} only"
        exit 1
    fi

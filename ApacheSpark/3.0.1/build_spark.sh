...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheSpark/3.0.1/build_spark.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheSpark/3.0.1/patch/"
...
function configureAndInstall() {
    ...
    # Install JDK8 (required for LevelDB JNI and Zstd JNI)
    cd "${SOURCE_ROOT}"
...
case "$DISTRO" in
    ...
    "rhel-7.6" | "rhel-7.7" | "rhel-7.8" | "rhel-8.1" | "rhel-8.2")
        ...
        sudo yum install -y wget tar git libtool autoconf make curl  |& tee -a "${LOG_FILE}"
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheSpark/2.4.4/build_spark.sh
...
PATCH_DIR="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/ApacheSpark/2.4.4/patch/"
...
# Patch Platform.java file
    curl -SL -o "Platform.diff"  $PATCH_DIR/Platform.diff
...
# Patch OffHeapColumnVector.diff
    curl -SL -o "OffHeapColumnVector.diff"  $PATCH_DIR/OffHeapColumnVector.diff
...
# Patch OnHeapColumnVector.diff
    curl -SL -o "OnHeapColumnVector.diff"  $PATCH_DIR/OnHeapColumnVector.diff
...
# Patch StatsdSinkSuite.diff file
    curl -SL -o "StatsdSinkSuite.diff"  $PATCH_DIR/StatsdSinkSuite.diff
...
# Patch Murmur3_x86_32Suite.diff
    curl -SL -o "Murmur3_x86_32Suite.diff"  $PATCH_DIR/Murmur3_x86_32Suite.diff
    patch "${CURDIR}/spark/common/unsafe/src/main/java/org/apache/spark/unsafe/hash/Murmur3_x86_32.java" Murmur3_x86_32Suite.diff
    printf -- 'Patched Murmur3_x86_32Suite \n'  >> "$LOG_FILE"

# Patch UnsafeMapSuite.diff
    curl -SL -o "UnsafeMapSuite.diff"  $PATCH_DIR/UnsafeMapSuite.diff
    patch "${CURDIR}/spark/sql/catalyst/src/test/scala/org/apache/spark/sql/catalyst/util/UnsafeMapSuite.scala" UnsafeMapSuite.diff
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "AdoptJDK" ]]; then
        ...
        curl -SL -o adoptjdk.tar.gz https://github.com/AdoptOpenJDK/openjdk8-binaries/releases/download/jdk8u202-b08_openj9-0.12.1/OpenJDK8U-jdk_s390x_linux_openj9_8u202b08_openj9-0.12.1.tar.gz
...
function printHelp() {
    echo
    echo "Usage: "
    echo " build_spark.sh  [-d debug] [-y install-without-confirmation] [-j Java to use from {AdoptJDK, IBM}]"
...
case "$DISTRO" in
    "ubuntu-16.04" | "ubuntu-18.04" | "ubuntu-19.10")
        ...
        sudo apt-get update
        sudo apt-get install -y wget tar git libtool autoconf build-essential maven curl |& tee -a "$LOG_FILE"
    ...
    "sles-12.4" | "sles-15.1")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y wget tar git libtool autoconf curl gcc make  gcc-c++ zip unzip gzip gawk  patch |& tee -a "$LOG_FILE"
        configureAndInstall |& tee -a "$LOG_FILE"
        ;;

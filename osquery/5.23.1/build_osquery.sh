...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/osquery/5.23.1/build_osquery.sh
# Execute build script: bash build_osquery.sh

set -e -o pipefail

PACKAGE_NAME="osquery"
PACKAGE_VERSION="5.23.1"
TOOLCHAIN_VERSION="1.3.0"
FORCE="false"
SOURCE_ROOT="$(pwd)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/osquery/5.23.1/patch"
...
function configureAndInstall() {
        ...
        if [[ $DISTRO == "ubuntu-24.04" ]]; then
        	python3 -m venv "$SOURCE_ROOT/osquery-venv"
	        source "$SOURCE_ROOT/osquery-venv/bin/activate"

	        python3 -m pip install \
                        psutil timeout_decorator thrift==0.11.0 osquery pexpect==3.3 docker
        else
...
"ubuntu-22.04" | "ubuntu-24.04")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "${LOG_FILE}"
        sudo apt-get update
        sudo DEBIAN_FRONTEND=noninteractive apt-get install -y git python3 python3-pip python3-venv python3-setuptools python3-psutil \
        python3-six python3-wheel g++ gcc automake autoconf gettext bison flex unzip help2man libtool-bin libncurses-dev \

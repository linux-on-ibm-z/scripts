...
"ubuntu-18.04" )
  ...
  sudo apt-get update >/dev/null
  sudo apt-get install -y --no-install-recommends gcc-10 g++-10  | tee -a "${LOG_FILE}"
  sudo apt-get install -y openjdk-8-jdk libaio-dev \
    ...
    pigz ragel rapidjson-dev stow patch locales valgrind |& tee -a "$LOG_FILE"
...
# C/C++ environment settings
  ...
  pip3 install --user --upgrade pip
  pip3 install --user pystache cassandra-driver colorama boto3 \
    pyudev urwid pyparsing python-magic psutil pytest distro

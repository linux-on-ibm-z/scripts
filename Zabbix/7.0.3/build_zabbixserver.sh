...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/7.0.3/build_zabbixserver.sh
...
EOF
  ...
  sudo yum install -y autoconf libtool cmake openssl-devel libcurl libcurl-devel libxml2 libxml2-devel readline readline-devel \
    ...
    recode-devel readline-devel gdbm-devel libdb-devel automake patch pkgconfig \
...
"ubuntu-20.04")
  ...
  sudo apt-get -y install wget curl perl vim make pkg-config snmp snmptrapd ceph locales libmariadbd-dev libxml2-dev \
        ...
        libevent-dev libpcre3-dev automake pkg-config libcmocka-dev \

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/5.0.17/build_zabbixserver.sh
...
function configureAndInstall() {
  ...
  if [[ "$ID" == "rhel" ]]; then
    ...
    if [[ "$VERSION_ID" == "8.2" || "$VERSION_ID" == "8.4" ]]; then
  ...
  if [[ "$ID" == "ubuntu" ]]; then
    ...
    if [[ "$VERSION_ID" == "20.04" || "$VERSION_ID" == "21.04" ]]; then
  ...
  sed -i '120s/is_uint64/is_uint32/' tests/mocks/valuecache/valuecache_mock.c
...
function gettingStarted() {
  ...
  printf -- "\n\nReference: \n"
  printf -- " More information can be found here : https://www.zabbix.com/documentation/5.0/manual/installation\n"
  printf -- '\n'
  printf -- ""
}

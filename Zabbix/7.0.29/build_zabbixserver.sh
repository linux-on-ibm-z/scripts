...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/7.0.29/build_zabbixserver.sh
...
URL_NAME="zabbix"
PACKAGE_VERSION="7.0.29"
SOURCE_DIR="${URL_NAME}-${PACKAGE_VERSION}"
PHP_VERSION="8.4.6"
CURDIR="$(pwd)"
BUILD_DIR="$(pwd)"
PREFIX="/usr/local"
...
function configureAndInstall() {
  ...
  if [[ ! -d "$SOURCE_DIR/.git" ]]; then
    rm -rf "$SOURCE_DIR"
    git clone --branch "$PACKAGE_VERSION" --depth 1 https://github.com/zabbix/zabbix.git "$SOURCE_DIR"
  fi
  cd "$SOURCE_DIR"
...
function gettingStarted() {
  printf -- "\n* Getting Started * \n"
  printf -- " Follow the official guide given [here](https://www.zabbix.com/documentation/7.0/en/manual/concepts/server) to verify the installation. \n"
  printf -- "\n\nReference: \n"
...
"rhel-9.6" | "rhel-9.7" | "rhel-9.8")
  ...
  sudo yum-config-manager --enable codeready-builder-for-rhel-9-s390x-rpms |& tee -a "$LOG_FILE"
...
"rhel-10.0" | "rhel-10.1" | "rhel-10.2")
  ...
  printf -- 'Installing the dependencies for Zabbix server from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y yum-utils |& tee -a "$LOG_FILE"
  sudo yum-config-manager --enable codeready-builder-for-rhel-10-s390x-rpms |& tee -a "$LOG_FILE"
  sudo dnf install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-10.noarch.rpm |& tee -a "$LOG_FILE"
  sudo dnf install -y initscripts httpd tar wget curl vim gcc make net-snmp net-snmp-devel php-mysqlnd git \
        ...
        libpath_utils-devel perl-IPC-Run3 perl-Path-Tiny mariadb mariadb-server mysql8.4-devel mysql8.4-libs perl-Time-HiRes --allowerasing --nobest --skip-broken |& tee -a "$LOG_FILE"
  sudo yum groupinstall -y 'Development Tools' |& tee -a "$LOG_FILE"
  configureAndInstall |& tee -a "$LOG_FILE"
  ;;
...
"sles-16.0")
  ...
  sudo zypper install -y wget tar curl vim gcc make net-snmp net-snmp-devel net-tools git apache2 apache2-devel mariadb \
        libmariadbd-devel apache2-mod_php8 php8 php8-mysql php8-xmlreader php8-xmlwriter php8-gd php8-bcmath php8-mbstring \
        php8-ctype php8-sockets php8-gettext libcurl-devel libxml2-2 libxml2-devel openldap2_6-devel php8-ldap unixODBC-devel \
        libevent-devel pcre-devel pcre2-devel awk gzip automake cmake libyaml-devel perl-YAML-LibYAML perl-Path-Tiny perl-IPC-Run3 \
        glibc-locale glibc-gconv-modules-extra |& tee -a "$LOG_FILE"

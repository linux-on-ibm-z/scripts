...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/7.0.10/build_zabbixserver.sh
...
"ubuntu-20.04")
  ...
  sudo DEBIAN_FRONTEND=noninteractive apt-get -y install wget curl perl vim make pkg-config snmp snmptrapd ceph locales libmariadbd-dev libxml2-dev \
        libsnmp-dev libcurl4 libcurl4-openssl-dev git apache2 apache2-dev mysql-server \
        libevent-dev libpcre3-dev libpcre2-dev automake pkg-config libcmocka-dev \
        libyaml-dev libyaml-libyaml-perl libpng-dev libpath-tiny-perl libipc-run3-perl build-essential |& tee -a "$LOG_FILE"
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y locales language-pack-de \
        autoconf libtool libssl-dev libjpeg-dev libxpm-dev \
        libreadline8 libreadline-dev libzip-dev libxslt1-dev \
        nginx openssl zlib1g-dev libfreetype6-dev\
        libsqlite3-dev libonig-dev libpq-dev unixodbc-dev \
        gcc-10 g++-10 tar bzip2 |& tee -a "$LOG_FILE"

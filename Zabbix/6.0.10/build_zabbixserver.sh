...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/6.0.10/build_zabbixserver.sh
...
function configureAndInstall() {
  ...
  if [[ "$ID" == "rhel" ]]; then
    ...
    if [[ "$VERSION_ID" == "8.4" || "$VERSION_ID" == "8.6" || "$VERSION_ID" == "9.0" ]]; then
  ...
  if [[ "$ID" == "sles" ]]; then
    ...
    if [[ "$VERSION_ID" == "15.3" || "$VERSION_ID" == "15.4" ]]; then
...
"ubuntu-18.04")
  ...
  sudo apt-get -y install wget curl vim gcc make pkg-config snmp snmptrapd ceph libxml2-dev libsnmp-dev libcurl4 \
        libcurl4-openssl-dev git apache2 php php-mysql libapache2-mod-php php7.2-xml php7.2-gd php-bcmath php-mbstring \
        php7.2-ldap libevent-dev libpcre3-dev automake pkg-config libcmocka-dev libyaml-dev libyaml-libyaml-perl \
        libpath-tiny-perl libipc-run3-perl build-essential libmysqld-dev libmysqlclient-dev |& tee -a "$LOG_FILE"
...
"rhel-7.8" | "rhel-7.9")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Zabbix server from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y initscripts httpd tar wget curl vim gcc pcre pcre-devel make net-snmp net-snmp-devel httpd-devel \
        ...
        gcc-c++ bison openssl aspell mariadb-devel mariadb-libs |& tee -a "$LOG_FILE"
...
"sles-12.5")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Zabbix server from repository \n' |& tee -a "$LOG_FILE"
  sudo zypper install -y wget tar curl vim gcc make net-snmp net-snmp-devel net-tools git apache2 apache2-devel \
        ...
        libevent-devel pcre-devel automake libyaml-devel perl-YAML-LibYAML perl-IPC-Run3 cmake glibc-locale libmysqld-devel |& tee -a "$LOG_FILE"
  curl -L http://cpanmin.us | sudo perl - --self-upgrade |& tee -a "$LOG_FILE"
...
"sles-15.3")
  ...
  sudo zypper install -y wget tar curl vim gcc make net-snmp net-snmp-devel net-tools git apache2 apache2-devel mariadb \
        libmysqld-devel apache2-mod_php7 php7 php7-mysql php7-xmlreader php7-xmlwriter php7-gd php7-bcmath php7-mbstring \
        php7-ctype php7-sockets php7-gettext libcurl-devel libxml2 libxml2-devel openldap2-devel openldap2 php7-ldap \
...
"sles-15.4")
  ...
  sudo zypper install -y wget tar curl vim gcc make net-snmp net-snmp-devel net-tools git apache2 apache2-devel mariadb \
        libmysqld-devel apache2-mod_php7 php7 php7-mysql php7-xmlreader php7-xmlwriter php7-gd php7-bcmath php7-mbstring \
        php7-ctype php7-sockets php7-gettext libcurl-devel libxml2-2 libxml2-devel openldap2-devel openldap2 php7-ldap \
        libevent-devel pcre-devel awk gzip automake cmake libyaml-devel perl-YAML-LibYAML perl-Path-Tiny perl-IPC-Run3 \
        glibc-locale |& tee -a "$LOG_FILE"
  export LC_CTYPE="en_US.UTF-8"

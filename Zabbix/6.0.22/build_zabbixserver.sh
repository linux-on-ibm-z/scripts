...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Zabbix/6.0.22/build_zabbixserver.sh
...
function configureAndInstall() {
  ...
  if [[ "$ID" == "ubuntu" ]]; then
    ...
    if [[ "$VERSION_ID" == "22.04" || "$VERSION_ID" == "23.04" ]]; then
...
"rhel-7.8" | "rhel-7.9")
  ...
  LD_LIBRARY_PATH=${PREFIX}/lib:${PREFIX}/lib64:/usr/lib64${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}
  export LD_LIBRARY_PATH
  LD_RUN_PATH=${PREFIX}/lib:${PREFIX}/lib64:/usr/lib64${LD_RUN_PATH:+:${LD_RUN_PATH}}
  export LD_RUN_PATH

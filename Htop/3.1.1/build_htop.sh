...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Htop/3.1.1/build_htop.sh
...
function configureAndInstall() {
	...
	# Download and unpack the htop 3.1.1 source code
	cd /"$CURDIR"/
	git clone https://github.com/htop-dev/htop.git
 	cd htop
  git checkout 3.1.1

	# Configure and build htop-3.1.1
	cd /"$CURDIR"/htop
	./autogen.sh
	./configure
	make
...
"sles-12.5" | "sles-15.2" | "sles-15.3")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for Htop from repository \n' |& tee -a "$LOG_FILE"
	sudo zypper install -y ncurses ncurses-devel gcc make wget tar awk git autoconf libtool xz |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Htop/3.1.0/build_htop.sh
...
function configureAndInstall() {
	...
	# Download and unpack the htop 3.1.0 source code
	cd /"$CURDIR"/
	git clone https://github.com/htop-dev/htop.git
 	cd htop
  	git checkout 3.1.0

	# Configure and build htop-3.1.0
	cd /"$CURDIR"/htop
	./autogen.sh
	./configure
	make
...
"ubuntu-20.04" | "ubuntu-21.04")
	...
	printf -- 'Installing the dependencies for Htop from repository \n' |& tee -a "$LOG_FILE"
    	sudo apt-get update -y >/dev/null
	sudo apt-get install -y gcc make wget tar libncursesw5 libcunit1-ncurses libncursesw5-dev automake |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"rhel-7.8" | "rhel-7.9")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for Htop from repository \n' |& tee -a "$LOG_FILE"
	sudo yum install -y ncurses ncurses-devel gcc make wget tar xz autoconf libtool |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"sles-12.5" | "sles-15.2" | "sles-15.3")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for Htop from repository \n' |& tee -a "$LOG_FILE"
	sudo zypper install -y ncurses ncurses-devel gcc make wget tar awk autoconf libtool xz |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

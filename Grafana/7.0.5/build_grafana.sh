...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/7.0.5/build_grafana.sh
...
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/7.0.5/patch/datasource.test.ts.patch"

GO_VERSION="1.14.4"
NODE_JS_VERSION="12.18.2"
IMAGE_RENDERER_VERSION="2.0.0"
...
function prepare() {
	...
	if [[ "$FORCE" == "true" ]]; then
		...
		printf -- "\n\nAs part of the installation , Go "${GO_VERSION}", NodeJS "${NODE_JS_VERSION}", Grafana-image-renderer "${IMAGE_RENDERER_VERSION}" will be installed, \n"
		while true; do
...
function configureAndInstall() {
	...
	# Check if Grafana-image-renderer directory exists
	cd "$GOPATH/src/github.com/grafana/"
	if [ -d "$GOPATH/src/github.com/grafana/grafana-image-renderer" ]; then
		rm -rf "$GOPATH/src/github.com/grafana/grafana-image-renderer"
		printf -- "Removing Existing Grafana-image-renderer Directory at GOPATH \n"
	fi

	# Build Grafana-image-renderer
	git clone -b v"${IMAGE_RENDERER_VERSION}" https://github.com/grafana/grafana-image-renderer.git
	cd grafana-image-renderer
	make deps
	make build

	printf -- 'Grafana-image-renderer build success \n'
	...
	sudo sed -i "s/^server_url.*/server_url\ =\ http:\/\/localhost:8081\/render/" /etc/grafana/grafana.ini
	sudo sed -i "s/^callback_url.*/callback_url\ =\ http:\/\/localhost:3000/" /etc/grafana/grafana.ini
	...
	echo "alias grafana-image-renderer='/usr/local/node-v${NODE_JS_VERSION}-linux-s390x/bin/node $GOPATH/src/github.com/grafana/grafana-image-renderer/build/app.js'" >> ~/.bashrc
...
function gettingStarted() {
	...
	printf -- "To run grafana-image-renderer, run the following command : \n"
	printf -- "    source ~/.bashrc  \n"
	printf -- "    grafana-image-renderer server --port=8081 &  (Run in background)  \n"

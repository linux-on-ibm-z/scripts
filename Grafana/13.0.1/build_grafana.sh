...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/13.0.1/build_grafana.sh
...
function configureAndInstall() {
	...
	rm grafana-dist/bin/*
	cp grafana/bin/linux/s390x/grafana grafana-dist/bin/grafana
	cp grafana/bin/linux/s390x/grafana grafana-dist/bin/grafana-server
	cp grafana/bin/linux/s390x/grafana grafana-dist/bin/grafana-cli
...
"rhel-8.10" | "rhel-9.6" | "rhel-9.7" | "rhel-10.0" | "rhel-10.1")
	...
	sudo yum install -y ${ALLOWERASING} make gcc gcc-c++ tar wget git git-core patch xz curl zlib-devel python3 procps |& tee -a "$LOG_FILE"
...
"ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.10")
	sudo apt-get update
	sudo apt-get install -y gcc g++ libc6-dev tar wget git make xz-utils patch curl python3 |& tee -a "$LOG_FILE"

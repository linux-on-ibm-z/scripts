...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.1/build_grafana.sh
...
GRAFANA_CONFIG_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.1/conf/grafana.ini"
...
function prepare() {
	...
	if [[ "$FORCE" == "true" ]]; then
		...
		printf -- "\n\nAs part of the installation , Go 1.11.5 and PhantomJS 2.1.1 will be installed, \n"
		while true; do
...
function configureAndInstall() {
	...
	curl $GO_INSTALL_URL | bash -s -- -v 1.11.5
	#rm build_go.sh

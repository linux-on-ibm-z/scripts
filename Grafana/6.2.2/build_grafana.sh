...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.2/build_grafana.sh
...
GRAFANA_CONFIG_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.2/conf/grafana.ini"
...
function cleanup() {
	...
	if [ -f /usr/local/node-v10.15.3-linux-s390x/bin/yarn ]; then
		sudo rm /usr/local/node-v10.15.3-linux-s390x/bin/yarn
	fi

	if [ -f "$BUILD_DIR/node-v10.15.3-linux-s390x.tar.xz" ]; then
		sudo rm "$BUILD_DIR/node-v10.15.3-linux-s390x.tar.xz"
	fi
...
function configureAndInstall() {
	...
	#Install NodeJS
	cd "$BUILD_DIR"
	sudo wget https://nodejs.org/dist/v10.15.3/node-v10.15.3-linux-s390x.tar.xz
	sudo chmod ugo+r node-v10.15.3-linux-s390x.tar.xz
	sudo tar -C "$BUILD_DIR" -xf node-v10.15.3-linux-s390x.tar.xz
	...
	if [[ "$ID" == "rhel" ]]; then
	    ...
	    sudo sed -i'' -r 's/^( +, uidSupport = ).+$/\1false/' /usr/local/node-v10.15.3-linux-s390x/lib/node_modules/npm/node_modules/uid-number/uid-number.js

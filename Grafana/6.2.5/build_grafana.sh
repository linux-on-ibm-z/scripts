...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.5/build_grafana.sh
...
GRAFANA_CONFIG_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/6.2.5/conf/grafana.ini"
...
function cleanup() {
	...
	if [ -f /usr/local/node-v10.16.0-linux-s390x/bin/yarn ]; then
		sudo rm /usr/local/node-v10.16.0-linux-s390x/bin/yarn
	fi

	if [ -f "$BUILD_DIR/node-v10.16.0-linux-s390x.tar.xz" ]; then
		sudo rm "$BUILD_DIR/node-v10.16.0-linux-s390x.tar.xz"
	fi
...
function configureAndInstall() {
	...
	#Install NodeJS
	cd "$BUILD_DIR"
	sudo wget https://nodejs.org/dist/v10.16.0/node-v10.16.0-linux-s390x.tar.xz
	sudo chmod ugo+r node-v10.16.0-linux-s390x.tar.xz
	sudo tar -C "$BUILD_DIR" -xf node-v10.16.0-linux-s390x.tar.xz
	export PATH=$PATH:/usr/local/node-v10.16.0-linux-s390x/bin
	...
	if [[ "$ID" == "rhel" ]]; then
	    ...
	    sudo sed -i'' -r 's/^( +, uidSupport = ).+$/\1false/' /usr/local/node-v10.16.0-linux-s390x/lib/node_modules/npm/node_modules/uid-number/uid-number.js
...
"sles-12.4" | "sles-15")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	sudo zypper install -y make gcc wget tar git unzip curl xz |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

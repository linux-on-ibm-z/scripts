...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/12.0.0/build_grafana.sh
...
function configureAndInstall() {
	...
	cd grafana
	# Adding a replace directive to enforce Go to use the specified version and not upgrade it.
	sed -i "7i github.com/grafana/grafana/apps/dashboard => ../../../../apps/dashboard" pkg/storage/unified/resource/go.mod
	sed -i "7i github.com/grafana/grafana/apps/dashboard => ../../../../apps/dashboard" pkg/storage/unified/apistore/go.mod
	go mod download
	make build-go
...
function runTest() {
	...
	if [[ "$TESTS" == "true" ]]; then
        ...
        printf -- '    make test-go\n\n'
        printf -- 'Failed frontend tests can be rerun with the commands:\n'

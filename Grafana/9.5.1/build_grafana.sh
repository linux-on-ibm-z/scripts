...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/9.5.1/build_grafana.sh
...
CURDIR="$(pwd)"
export PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Grafana/9.5.1/patch"
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	sudo zypper install -y make gcc7 gcc7-c++ wget tar git xz gzip curl libnghttp2-devel patch python36 |& tee -a "$LOG_FILE"

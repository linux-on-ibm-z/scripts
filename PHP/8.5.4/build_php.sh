...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PHP/8.5.4/build_php.sh
...
runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        rm -f ./ext/opcache/tests/log_verbosity_bug.phpt
        rm -f ./ext/standard/tests/strings/chunk_split_variation3.phpt
        sed -i 's/run-tests.php -n -c/run-tests.php -q -n -c/' Makefile
        export TEST_PHP_ARGS="-d memory_limit=2G"

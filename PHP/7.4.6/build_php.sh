...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PHP/7.4.6/build_php.sh
...
#----------------------------------------------------------
  ...
  curl -sSL ${url}/nan.diff | patch -p1 || error "${url}/nan.diff"
  curl -sSL ${url}/infinity.diff | patch -p1 || error "${url}/infinity.diff"

  # fix reflection
  curl -sSL ${url}/reflection.diff | patch -p1 || error "${url}/reflection.diff"
...
patchTest()
{
  ...
  case "$DISTRO" in
    "rhel-7."*) url=${PATCH_URL}/rhel7-test.diff ;;
    "rhel-8."*) url=${PATCH_URL}/rhel8-test.diff ;;
    "sles"*) url=${PATCH_URL}/sles-test.diff ;;
  esac

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/PHP/8.5.10/build_php.sh
...
buildPHP() {
    ...
    cd "$SOURCE_ROOT" || error "Cannot cd to $SOURCE_ROOT"

    if ! wget -q -O "php-$ver.tar.gz" "$PHP_URL"; then
        error "Failed to download $PHP_URL"
    fi

    tar -xzf "php-$ver.tar.gz" || error "Failed to extract php-$ver.tar.gz"

    cd php-$ver || error "Source dir php-$ver not found"
...
installPHP() {
    ...
    if [ "$rc" -ne "0" ]; then
        error "make install failed (exit code $rc). Please check the logs."
    else
...
# For the failing OpenSSL and Phar test modules, RHEL systems require the following crypto policy setting before executing the tests:
...
runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd "$SOURCE_ROOT/php-$ver" || error "php source dir missing for tests"
        sed -i 's/run-tests.php -n -c/run-tests.php -q -n -c/' Makefile
        export TEST_PHP_ARGS="-j$(nproc)"
        make TEST_PHP_ARGS="$TEST_PHP_ARGS" test
...
gettingStarted() {
    ...
    ***********************************************************************
	  ...
	  For more information, refer to the Build Instructions: 
...
"rhel-9.6" | "rhel-9.7" | "rhel-9.8" | "rhel-10.0" | "rhel-10.1" | "rhel-10.2")
    ...
    sudo yum install -y \
        ...
        wget glibc-gconv-extra glibc-locale-source |& tee -a "$LOG_FILE"
...
"sles-15.7" | "sles-16.0")
    ...
    sudo zypper install -y \
        autoconf curl libtool \
        libxml2-devel readline-devel \
        libcurl4 libcurl-devel openssl \
        libzip-devel pkg-config oniguruma-devel git \
        tar sqlite3-devel zlib-devel gcc13 gcc-c++ \
        libzip5 postgresql17 nginx postgresql17-server-devel bzip2 \
        make gzip gawk gmp-devel mpfr-devel mpc-devel wget |& tee -a "$LOG_FILE"

    if [[ "$DISTRO" == "sles-15.7" ]]; then
        sudo zypper install -y libreadline7 |& tee -a "$LOG_FILE"
    fi

    if [[ "$DISTRO" == "sles-16.0" ]]; then
        sudo zypper install -y glibc-gconv-modules-extra |& tee -a "$LOG_FILE"
    fi
    ...
    [ -f /usr/bin/pg_config ] || sudo ln -s /usr/lib/postgresql17/bin/pg_config /usr/bin/pg_config
...
"ubuntu-22.04" | "ubuntu-24.04")
    ...
    sudo apt-get install -y locales language-pack-de \
        ...
        gcc g++ git tar gcc make bzip2 wget \
        |& tee -a "$LOG_FILE"
    ;;
*)

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CruiseControl/2.5.70/build_cruise_control.sh
# Execute build script: bash build_cruise_control.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="cruise-control"
PACKAGE_VERSION="2.5.70"
CURDIR="$(pwd)"
SOURCE_ROOT="$(pwd)"
...
JAVA_PROVIDED="AdoptJDK11_openj9"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "AdoptJDK11_openj9" ]]; then
        # Install AdoptOpenJDK 11 (With OpenJ9 Large heap)
        cd "$CURDIR"
        sudo rm -rf /opt/adopt/java
        sudo mkdir -p /opt/adopt/java

        curl -SL -o adoptjdk.tar.gz https://github.com/AdoptOpenJDK/openjdk11-binaries/releases/download/jdk-11.0.10%2B9_openj9-0.24.0/OpenJDK11U-jdk_s390x_linux_openj9_linuxXL_11.0.10_9_openj9-0.24.0.tar.gz
        ...
        printf -- "Installation of AdoptOpenJDK 11 (With OpenJ9 Large heap) is successful\n" >> "$LOG_FILE"
    ...
    curl -sSL https://github.com/linkedin/cruise-control/commit/917b78a193546cce59e56e3c83b9893aa125b116.diff | git apply
    ./gradlew jar
...
function printHelp() {
    echo " build_cruise_control.sh [-d debug][-y install-without-confirmation][-t run-test][-j Java to use from {AdoptJDK11_openj9, AdoptJDK11_hotspot, OpenJDK11}] "
...
function gettingStarted() {
    ...
    printf -- "\n To start Cruise Control server refer: https://github.com/linkedin/cruise-control/tree/2.5.70#quick-start  \n\n"
    printf -- '**********************************************************************************************************\n'
}

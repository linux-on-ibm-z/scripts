...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru17" && "$DISTRO" == "ubuntu-25.04" ]]; then
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru17" ]]; then   
        ...
        curl -SL -o adoptjdk.tar.gz https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.16%2B8_openj9-0.53.0/ibm-semeru-open-jdk_s390x_linux_17.0.16_8_openj9-0.53.0.tar.gz
        ...
        sudo rm -rf /opt/adopt/java
        sudo mkdir -p /opt/adopt/java
        curl -SL -o adoptjdk.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.16%2B8/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.16_8.tar.gz
        # Everytime new jdk is downloaded, Ensure that --strip value is correct
        sudo tar -zxvf adoptjdk.tar.gz -C /opt/adopt/java --strip-components 1
        export JAVA_HOME=/opt/adopt/java
        printf -- "export JAVA_HOME=/opt/adopt/java\n" >> "$BUILD_ENV"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CruiseControl/2.5.145/build_cruise_control.sh
# Execute build script: bash build_cruise_control.sh    (provide -h for help)
set -e -o pipefail
PACKAGE_NAME="cruise-control"
PACKAGE_VERSION="2.5.146"
CURDIR="$(pwd)"
FORCE="false"
TESTS="false"
LOG_FILE="$CURDIR/logs/${PACKAGE_NAME}-${PACKAGE_VERSION}-$(date +"%F-%T").log"
BUILD_ENV="$HOME/setenv.sh"
JAVA_PROVIDED="Semeru17"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Semeru17" && "$JAVA_PROVIDED" != "Temurin17" && "$JAVA_PROVIDED" != "OpenJDK17" ]]; then
        printf "$JAVA_PROVIDED is not supported, Please use valid java from {Semeru17, Temurin17, OpenJDK17} only\n"
        exit 1
    fi
    if [[ "$JAVA_PROVIDED" == "OpenJDK17" && "$DISTRO" == "rhel-10.0" ]]; then
        printf "OpenJDK17 is not supported on $DISTRO\n"
        exit 1
    fi
    if [[ "$JAVA_PROVIDED" == "Semeru17" && "$DISTRO" == "ubuntu-25.04" && "$DISTRO" == "ubuntu-25.10" ]]; then
        printf "IBM Semeru Runtime (Semeru17) is not supported on $DISTRO\n"
        exit 1
    fi
    if [[ "$FORCE" == "true" ]]; then
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru17" ]]; then   
        ...
        curl -SL -o adoptjdk.tar.gz https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.17%2B10_openj9-0.56.0/ibm-semeru-open-jdk_s390x_linux_17.0.17_10_openj9-0.56.0.tar.gz
        ...
        sudo rm -rf /opt/adopt/java
        sudo mkdir -p /opt/adopt/java
        curl -SL -o adoptjdk.tar.gz https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.17%2B10/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.17_10.tar.gz
        # Everytime new jdk is downloaded, Ensure that --strip value is correct
        sudo tar -zxvf adoptjdk.tar.gz -C /opt/adopt/java --strip-components 1
        export JAVA_HOME=/opt/adopt/java
        printf -- "export JAVA_HOME=/opt/adopt/java\n" >> "$BUILD_ENV"
    ...
    cd "$CURDIR"
    git clone -b ${PACKAGE_VERSION} https://github.com/linkedin/cruise-control.git
    cd cruise-control
    ./gradlew jar
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        if [[ "$DISTRO" != "rhel-10.0" || "$DISTRO" != "ubuntu-25.10" ]]; then
           wget https://raw.githubusercontent.com/linux-on-ibm-z/dockerfile-examples/b30616065fd8c88d38fad8e74680f743b54ce761/ApacheKafka/Dockerfile
           sed -i 's/3\.9\.0/3\.9\.1/g' Dockerfile
           docker build -t apache/kafka:3.9.1 .
        fi
...
function printHelp() {
    echo " bash build_cruise_control.sh [-d debug][-y install-without-confirmation][-t run-test][-j Java to use from {Semeru17, Temurin17, OpenJDK17}] "
    ...
    echo "  - OpenJDK17 is NOT supported on RHEL 10.0"
    echo "  - Semeru17 is NOT supported on Ubuntu 25.04"
}
...
function gettingStarted() {
    ...
    printf -- "\n To start Cruise Control server refer: https://github.com/linkedin/cruise-control/tree/2.5.145#quick-start  \n\n"
    printf -- '**********************************************************************************************************\n'
}
logDetails
...
"sles-15.6" | "sles-15.7")
    ...
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
"ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.04" | "ubuntu-25.10")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update

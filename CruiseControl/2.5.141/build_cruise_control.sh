...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CruiseControl/2.5.141/build_cruise_control.sh
# Execute build script: bash build_cruise_control.sh    (provide -h for help)
set -e -o pipefail
PACKAGE_NAME="cruise-control"
PACKAGE_VERSION="2.5.141"
CURDIR="$(pwd)"
FORCE="false"
TESTS="false"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
        ...
        curl -SL -o adoptjdk.tar.gz https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.23%2B9_openj9-0.44.0/ibm-semeru-open-jdk_s390x_linux_11.0.23_9_openj9-0.44.0.tar.gz
        ...
        sudo rm -rf /opt/adopt/java
        sudo mkdir -p /opt/adopt/java
        curl -SL -o adoptjdk.tar.gz https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.23%2B9/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.23_9.tar.gz
        # Everytime new jdk is downloaded, Ensure that --strip value is correct
        sudo tar -zxvf adoptjdk.tar.gz -C /opt/adopt/java --strip-components 1
        export JAVA_HOME=/opt/adopt/java
        printf -- "export JAVA_HOME=/opt/adopt/java\n" >> "$BUILD_ENV"
...
function gettingStarted() {
    ...
    printf -- "\n To start Cruise Control server refer: https://github.com/linkedin/cruise-control/tree/2.5.141#quick-start  \n\n"
    printf -- '**********************************************************************************************************\n'
}
logDetails

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CruiseControl/2.5.80/build_cruise_control.sh
# Execute build script: bash build_cruise_control.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="cruise-control"
PACKAGE_VERSION="2.5.80"
CURDIR="$(pwd)"
SOURCE_ROOT="$(pwd)"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
        # Apply patch for IBM Semeru Runtime (previously known as AdoptOpenJDK openj9)
        sed -i 's/io.github.classgraph:classgraph:4.8.117/io.github.classgraph:classgraph:4.8.115/g' ./build.gradle
    fi
    ./gradlew jar
...
function gettingStarted() {
    ...
    printf -- "\n To start Cruise Control server refer: https://github.com/linkedin/cruise-control/tree/2.5.80#quick-start  \n\n"
    printf -- '**********************************************************************************************************\n'
}
...
"sles-12.5" | "sles-15.3")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y wget tar git curl gzip gawk
    configureAndInstall |& tee -a "$LOG_FILE"
    ;;
*)

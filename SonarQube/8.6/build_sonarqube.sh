...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/8.6/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="8.6.0.39681"
SCANNER_VERSION="4.5.0.2216"
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "AdoptJDK11" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget https://github.com/AdoptOpenJDK/openjdk11-binaries/releases/download/jdk-11.0.9%2B11_openj9-0.23.0/OpenJDK11U-jdk_s390x_linux_openj9_11.0.9_11_openj9-0.23.0.tar.gz
        sudo tar -C /usr/local -xzf OpenJDK11U-jdk_s390x_linux_openj9_11.0.9_11_openj9-0.23.0.tar.gz
        export JAVA_HOME=/usr/local/jdk-11.0.9+11

        printf -- 'export JAVA_HOME=/usr/local/jdk-11.0.9+11\n'  >> "$BUILD_ENV"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd "$SOURCE_ROOT"
        wget https://nodejs.org/dist/latest-v10.x/node-v10.23.1-linux-s390x.tar.xz
        chmod ugo+r node-v10.23.1-linux-s390x.tar.xz
        sudo tar -C /usr/local -xf node-v10.23.1-linux-s390x.tar.xz
		export PATH=$PATH:/usr/local/node-v10.23.1-linux-s390x/bin
	    node -v

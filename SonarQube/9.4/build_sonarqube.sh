...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/9.4/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="9.4.0.54424"
	...
	cd Semeru_11_openj9
	 ...
	 cd Adoptium_11_hotspot
        ...
        if [[ "$ID" == "rhel" ]]; then
            ...
            if [[ "$VERSION_ID" == "20.04" || "$VERSION_ID" == "18.04" || "$VERSION_ID" == "21.10" ]]; then

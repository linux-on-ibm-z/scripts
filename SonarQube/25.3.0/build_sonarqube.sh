...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/25.3.0/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="25.3.0.104237"
SONAR_VERSION="25.3.0"
SCANNER_VERSION="6.2.0.4584"
NODEJS_VERSION="v20.12.2"
ES_VERSION="8.16.3" #https://github.com/SonarSource/sonarqube/blob/25.3.0.104237/gradle.properties#L14
ZSTD_VERSION=1.5.5
SOURCE_ROOT="$(pwd)"
BUILD_DIR="/usr/local"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" != "Temurin_17" && "$JAVA_PROVIDED" != "Temurin_21" && "$JAVA_PROVIDED" != "OpenJDK_17" && "$JAVA_PROVIDED" != "OpenJDK_21" ]]
    then
        printf --  "$JAVA_PROVIDED is not supported, Please use valid java from {Temurin_17, Temurin_21, OpenJDK_17, OpenJDK_21} only." |& tee -a "$LOG_FILE"
        exit 1
    fi
...
function cleanup() {
    ...
    elif [[ "$JAVA_PROVIDED" == "Temurin_21" ]]; then
	      sudo rm -rf temurin17.tar.gz
    fi
...
function installJava() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin_17" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget -O temurin17.tar.gz -q https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.13%2B11/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.13_11.tar.gz        
        ...
        sudo mkdir -p /opt/java
        cd "$SOURCE_ROOT"
        sudo wget -O temurin21.tar.gz -q https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.6%2B7/OpenJDK21U-jdk_s390x_linux_hotspot_21.0.6_7.tar.gz        
	      sudo tar -C /opt/java -xzf temurin21.tar.gz --strip 1
        printf -- 'export JAVA_HOME=/opt/java\n'  >> "$BUILD_ENV"
        printf -- 'Eclipse Adoptium Temurin Runtime 21 installed\n'	
    
    elif [[ "$JAVA_PROVIDED" == "OpenJDK_17" ]]; then
        if [[ "$ID" == "rhel" ]]; then
        ...
        printf --  '$JAVA_PROVIDED is not supported, Please use valid java from {Temurin_17, Temurin_21, OpenJDK_17, OpenJDK_21} only' >> "$LOG_FILE"
        exit 1
    fi
...
function runTest() {
	...
	SONAR_TOKEN=$(curl -u admin:admin -X POST "http://localhost:9000/api/user_tokens/generate" -d "name=my-token" | jq -r .token)
        ...
        ./gradlew --no-daemon -Dsonar.host.url=http://localhost:9000 -Dsonar.token=$SONAR_TOKEN sonar
        ...
        ./gradlew --no-daemon clean build codeCoverageReport -Dsonar.host.url=http://localhost:9000 -Dsonar.token=$SONAR_TOKEN sonar	  
...
function printHelp() {
    echo
    echo "Usage: "
    echo " bash build_sonarqube.sh  [-d debug] [-y install-without-confirmation] [-t install and run tests] [-j Java to use from {Temurin_17, OpenJDK_17, Temurin_21, OpenJDK_21}]"
...
"rhel-8.8" | "rhel-8.10" | "rhel-9.2" | "rhel-9.4" | "rhel-9.5")
    ...
    sudo yum install -y wget git unzip tar which net-tools curl gzip patch make gcc gcc-c++ xz jq procps --allowerasing |& tee -a "$LOG_FILE"
    ;;
"sles-15.6")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo zypper install -y git wget unzip tar which gzip xz net-tools curl patch make gcc gcc-c++ jq procps |& tee -a "$LOG_FILE"
    ;;
"ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-24.10")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing dependencies... it may take some time.\n"
    sudo apt-get update
    sudo apt-get install -y wget git unzip tar net-tools xz-utils curl gzip patch locales make gcc g++ jq procps |& tee -a "$LOG_FILE"

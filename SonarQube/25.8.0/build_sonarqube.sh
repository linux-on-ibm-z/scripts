...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/25.8.0/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="25.8.0.112029"
SONAR_VERSION="25.8.0"
SCANNER_VERSION="7.2.0.5079"
NODEJS_VERSION="v24.2.0"
ES_VERSION="8.16.3" #https://github.com/SonarSource/sonarqube/blob/25.8.0.112029/gradle.properties#ll15
ZSTD_VERSION=1.5.5
SOURCE_ROOT="$(pwd)"
BUILD_DIR="/usr/local"
...
function prepare() {
    ...
    if [[ "$JAVA_PROVIDED" == "OpenJDK_17" && "$VERSION_ID" == "10.0" ]]
    then
        printf --  "$JAVA_PROVIDED is not supported, Please use valid java from {Temurin_17, Temurin_21, OpenJDK_21} only." |& tee -a "$LOG_FILE"
        exit 1
    fi
...
function installJava() {
    ...
    if [[ "$JAVA_PROVIDED" == "Temurin_17" ]]; then
        ...
        cd "$SOURCE_ROOT"
        wget -O temurin17.tar.gz -q https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.14%2B7/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.14_7.tar.gz
        ...
        printf --  '$JAVA_PROVIDED is not supported, Please use valid java from {Temurin_17, Temurin_21, OpenJDK_17, OpenJDK_21} only'
        exit 1
    fi
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        #Run Java Scanner for Gradle - only for JDK 17
        #https://docs.sonarsource.com/sonarqube-server/latest/analyzing-source-code/scanners/sonarscanner-for-gradle/#prerequisites

        if [[ "$JAVA_PROVIDED" == "OpenJDK_17" || "$JAVA_PROVIDED" == "Temurin_17" ]]; then
            printf -- 'Running Java Scanner for Gradle tests... \n\n'

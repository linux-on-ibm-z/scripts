...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/8.7.0/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="8.7.0.41497"
SCANNER_VERSION="4.6.0.2311"
    ...
    then
        ...
        then
            ...
            while true; do
                ...
                [Yy]*)
                    JAVA_PROVIDED="AdoptJDK11"
                    printf -- 'SonarQube will be built with AdoptOpenJDK11 openj9. \n' >>"$LOG_FILE" |& tee -a "$LOG_FILE"
                    break
                    ;;
                [Nn]*) exit ;;
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "AdoptJDK11" ]]; then
        ...
        cd "$SOURCE_ROOT"
        sudo wget https://github.com/AdoptOpenJDK/openjdk11-binaries/releases/download/jdk-11.0.10%2B9_openj9-0.24.0/OpenJDK11U-jdk_s390x_linux_openj9_11.0.10_9_openj9-0.24.0.tar.gz
        sudo tar -C /usr/local -xzf OpenJDK11U-jdk_s390x_linux_openj9_11.0.10_9_openj9-0.24.0.tar.gz
        export JAVA_HOME=/usr/local/jdk-11.0.10+9

        printf -- 'export JAVA_HOME=/usr/local/jdk-11.0.10+9\n'  >> "$BUILD_ENV"
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd "$SOURCE_ROOT"
        wget https://nodejs.org/dist/latest-v10.x/node-v10.24.0-linux-s390x.tar.xz
        chmod ugo+r node-v10.24.0-linux-s390x.tar.xz
        sudo tar -C /usr/local -xf node-v10.24.0-linux-s390x.tar.xz
		export PATH=$PATH:/usr/local/node-v10.24.0-linux-s390x/bin
	    node -v

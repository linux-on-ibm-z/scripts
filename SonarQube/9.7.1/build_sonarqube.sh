...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/9.7.1/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="9.7.1.62043"
...
function cleanup() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru_11_openj9" ]]; then
        ...
        sudo rm -rf OpenJDK11U-jdk*.tar.gz
    fi
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru_11_openj9" ]]; then
        ...
        sudo tar -xzf ibm-semeru-open-jdk_s390x_linux_11.0.17_8_openj9-0.35.0.tar.gz
        export JAVA_HOME=$SOURCE_ROOT/Semeru_11_openj9/jdk-11.0.17+8

        printf -- 'export JAVA_HOME=$SOURCE_ROOT/Semeru_11_openj9/jdk-11.0.17+8\n'  >> "$BUILD_ENV"
        ...
        sudo tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.17_8.tar.gz
        export JAVA_HOME=$SOURCE_ROOT/Adoptium_11_hotspot/jdk-11.0.17+8

        printf -- 'export JAVA_HOME=$SOURCE_ROOT/Adoptium_11_hotspot/jdk-11.0.17+8\n'  >> "$BUILD_ENV"

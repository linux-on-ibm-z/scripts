...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/SonarQube/9.6.1/build_sonarqube.sh
# Execute build script: bash build_sonarqube.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="sonarqube"
PACKAGE_VERSION="9.6.1.59531"
...
function cleanup() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru_11_openj9" ]]; then
        ...
        sudo rm -rf ibm-semeru-open-jdk_s390x_linux_11.0.16.1_1_openj9-0.33.1.tar.gz
	
    elif [[ "$JAVA_PROVIDED" == "Adoptium_11_hotspot" ]]; then
        cd "$SOURCE_ROOT"/Adoptium_11_hotspot
        sudo rm -rf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.16.1_1.tar.gz
    fi
...
function configureAndInstall() {
    ...
    if [[ "$JAVA_PROVIDED" == "Semeru_11_openj9" ]]; then
	    ...
	    cd Semeru_11_openj9
        sudo wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.16.1%2B1_openj9-0.33.1/ibm-semeru-open-jdk_s390x_linux_11.0.16.1_1_openj9-0.33.1.tar.gz
        sudo tar -xzf ibm-semeru-open-jdk_s390x_linux_11.0.16.1_1_openj9-0.33.1.tar.gz
        export JAVA_HOME=$SOURCE_ROOT/Semeru_11_openj9/jdk-11.0.16.1+1

        printf -- 'export JAVA_HOME=$SOURCE_ROOT/Semeru_11_openj9/jdk-11.0.16.1+1\n'  >> "$BUILD_ENV"
        ...
        sudo wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.16.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.16.1_1.tar.gz
        sudo tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.16.1_1.tar.gz
        export JAVA_HOME=$SOURCE_ROOT/Adoptium_11_hotspot/jdk-11.0.16.1+1

        printf -- 'export JAVA_HOME=$SOURCE_ROOT/Adoptium_11_hotspot/jdk-11.0.16.1+1\n'  >> "$BUILD_ENV"
        ...
        if [[ "$ID" == "rhel" ]]; then
            ...
            if [[ "$VERSION_ID" == "20.04" || "$VERSION_ID" == "18.04" || "$VERSION_ID" == "22.04" ]]; then
...
function runTest() {
    ...
    if [[ "$TESTS" == "true" ]]; then
        ...
        cd "$SOURCE_ROOT"
        wget https://nodejs.org/dist/v16.17.0/node-v16.17.0-linux-s390x.tar.xz
        chmod ugo+r node-v16.17.0-linux-s390x.tar.xz
        sudo tar -C /usr/local -xf node-v16.17.0-linux-s390x.tar.xz
	    export PATH=$PATH:/usr/local/node-v16.17.0-linux-s390x/bin
	    node -v
...
"ubuntu-18.04" | "ubuntu-20.04" | "ubuntu-22.04")
    ...
    sudo apt-get update
    sudo apt-get install -y wget git unzip tar curl net-tools xz-utils |& tee -a "$LOG_FILE"

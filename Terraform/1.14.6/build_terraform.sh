...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/1.14.6/build_terraform.sh
...
GO_VERSION="1.25.0"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/1.14.6/patch"
...
function configureAndInstall() {
  ...
  if [ "$(readlink -f ${GOPATH}/bin/terraform)" != "$(readlink -f /usr/bin/terraform 2>/dev/null)" ]; then
...
function run_e2e_tests() {
  ...
  mkdir -p $BIN_PATH
  cp $GOPATH/src/github.com/hashicorp/terraform/internal/command/e2etest/testdata/local-only-provider/terraform.d/plugins/example.com/awesomecorp/happycloud/1.2.0/os_arch/terraform-provider-happycloud_v1.2.0 $BIN_PATH/
...
"ubuntu-22.04" | "ubuntu-24.04" | "ubuntu-25.10")
  ...
  sudo apt-get install -y git wget tar build-essential gcc zip unzip |& tee -a "${LOG_FILE}"
  configureAndInstall |& tee -a "${LOG_FILE}"
  ;;

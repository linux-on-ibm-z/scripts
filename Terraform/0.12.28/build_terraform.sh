...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/0.12.28/build_terraform.sh
...
function installGo()
{
  ...
  cd "${GOPATH}"
  wget "https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Go/1.14.4/build_go.sh"
  bash build_go.sh -y -v 1.14.4
...
function configureAndInstall()
{
  ...
  set +e
  make 2>&1 | tee -a $GOPATH/test_logs
  cat $GOPATH/test_logs | grep "FAIL" | grep github.com | awk '{print $2}' > $GOPATH/failed.txt
  echo 'github.com/hashicorp/terraform/internal/getproviders' > $GOPATH/expected.txt

  if [ $(comm -23 $GOPATH/failed.txt $GOPATH/expected.txt | wc -l) -ne 0 ]; then
        printf -- '**********************************************************************************************************\n'
...
"rhel-8.1" | "rhel-8.2")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Terraform from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y git wget make zip which diffutils |& tee -a "${LOG_FILE}"
  installGo
  configureAndInstall |& tee -a "${LOG_FILE}"
  ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/0.14.2/build_terraform.sh
...
function cleanup()
{
  rm -rf $GOPATH/go1.15.3.linux-s390x.tar.gz
...
function configureAndInstall()
{
  ...
  printf -- "Gopath is set to  %s \n" "$GOPATH"
   ...
   cd ${GOPATH}
   wget https://golang.org/dl/go1.15.3.linux-s390x.tar.gz
   chmod ugo+r go1.15.3.linux-s390x.tar.gz
   sudo tar -C /usr/local -xvzf go1.15.3.linux-s390x.tar.gz
  ...
  sudo mv ${GOPATH}/bin/terraform /usr/bin/
...
function runTests() {
   ...
   if [[ "$TESTS" == "true" ]]; then
	...
	go test ./...
	printf -- '**********************************************************************************************************\n'
	printf -- '\nIn case of unexpected test failures try running the test individually using command go test ./<failed_test_name>\n'
	printf -- '**********************************************************************************************************\n'
   fi
  set -e
}  
...
"sles-12.5" |  "sles-15.1" |  "sles-15.2")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Terraform from repository \n' |& tee -a "$LOG_FILE"
  sudo zypper install -y git wget tar gcc gzip |& tee -a "${LOG_FILE}"
  configureAndInstall |& tee -a "${LOG_FILE}"
  ;;

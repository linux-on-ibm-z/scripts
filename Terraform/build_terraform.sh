...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/build_terraform.sh
...
function printHelp() {
  echo
  echo "Usage: "
  echo "bash build_terraform.sh [-d debug] [-v package-version] [-y install-without-confirmation] [-t run-test-cases]"
...
"rhel-7.8" | "rhel-7.9")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Terraform from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y git wget tar gcc diffutils rh-git227-git.s390x |& tee -a "${LOG_FILE}"

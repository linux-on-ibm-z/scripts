...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/0.14.3/build_terraform.sh
...
function runTests() {
   ...
   if [[ "$TESTS" == "true" ]]; then
	...
	go test ./...
	printf -- '**********************************************************************************************************\n'
	printf -- '\nIn case of unexpected test failures try running the test individually using command go test ./<test_name>\n'
	printf -- '**********************************************************************************************************\n'
   fi
  set -e
}  

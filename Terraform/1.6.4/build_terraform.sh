...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Terraform/1.6.4/build_terraform.sh
...
function run_e2e_tests() {
  ...
  git checkout v3.2.1
  go build
  BIN_PATH="${PROVIDER_PLUGIN_LOCAL_MIRROR_PATH}/registry.terraform.io/hashicorp/null/3.2.1/linux_s390x"
  mkdir -p $BIN_PATH
  mv terraform-provider-null $BIN_PATH/terraform-provider-null_v3.2.1
  git checkout v3.1.0
  go build
...
"rhel-7.8" | "rhel-7.9")
  printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
  printf -- 'Installing the dependencies for Terraform from repository \n' |& tee -a "$LOG_FILE"
  sudo yum install -y git wget tar gcc diffutils rh-git227-git.s390x zip unzip |& tee -a "${LOG_FILE}"

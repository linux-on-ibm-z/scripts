...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/etcd/3.4.15/build_etcd.sh
...
function cleanup() {
    # Remove artifacts
    if [[ -f "$HOME/go"${GO_VERSION}".linux-s390x.tar.gz" ]]; then
       sudo rm -rf  "$HOME/go"${GO_VERSION}".linux-s390x.tar.gz"
    fi
...
function configureAndInstall() {
    ...
    sudo cp ${GOPATH}/src/github.com/etcd-io/etcd/etcd.conf.yml.sample  /etc/etcd/etcd.conf.yml
    printf -- "Added etcd.conf.yml in /etc/etcd \n"

    # Add etcd to /usr/bin
    sudo cp ${GOPATH}/src/github.com/etcd-io/etcd/bin/etcd* /usr/bin/
...
function printHelp() {
    echo
    echo "Usage: "
    echo "bash build_etcd.sh  [-d debug] [-y install-without-confirmation] [-t install and run tests]"
    echo
}

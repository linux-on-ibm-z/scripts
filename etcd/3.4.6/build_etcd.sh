...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/etcd/3.4.6/build_etcd.sh
...
case "$DISTRO" in
    "ubuntu-16.04" | "ubuntu-18.04" | "ubuntu-19.10")
        ...
        ;;
    "rhel-7.7" | "rhel-7.5" | "rhel-7.6" | "rhel-8.0" | "rhel-8.1")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"

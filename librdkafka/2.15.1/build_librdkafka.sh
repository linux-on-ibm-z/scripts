...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/librdkafka/2.15.1/build_librdkafka.sh
# Execute build script: bash build_librdkafka.sh (provide -h for help)

set -e  -o pipefail

PACKAGE_NAME="librdkafka"
PACKAGE_VERSION="v2.15.1"
SOURCE_ROOT="$(pwd)"
USER="$(whoami)"
FORCE="false"
...
function configureAndInstall() {
	...
	if [ $ID$VERSION_ID == rhel8.10 ]; then
...
"sles-15.7" | "sles-16.0")
	printf -- "Installing %s %s for %s\n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n" |& tee -a "$LOG_FILE"
	sudo zypper install -y binutils gcc gcc-c++ make git libz1 zlib-devel openssl-devel coreutils diffutils findutils util-linux python3 which gawk |& tee -a "$LOG_FILE"
...
"ubuntu-22.04" | "ubuntu-24.04")
	...
	sudo apt-get install -y git build-essential make zlib1g-dev libpthread-stubs0-dev libssl-dev libsasl2-dev libzstd-dev libcurl4-openssl-dev python3 |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;; 
*)

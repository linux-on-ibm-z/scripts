...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/librdkafka/2.12.1/build_librdkafka.sh
...
"rhel-8.10" | "rhel-9.4" | "rhel-9.6" | "rhel-10.0")
	...
	if [ $ID$VERSION_ID == rhel8.10 ]; then
        ...
        if [ $ID$VERSION_ID == rhel9.4 ] || [ $ID$VERSION_ID == rhel9.6 ] || [ $ID$VERSION_ID == rhel10.0 ]; then
            rm ~/.bash_profile
            touch ~/.bash_profile
        fi

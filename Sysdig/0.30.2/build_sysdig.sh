...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Sysdig/0.30.2/build_sysdig.sh
...
"ubuntu-18.04" | "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-22.10")
    ...
    sudo apt-get install -y git g++ linux-headers-$(uname -r) cmake libelf-dev pkg-config kmod |& tee -a "$LOG_FILE"
...
"rhel-7.8" | "rhel-7.9")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" | tee -a "$LOG_FILE"
    printf -- '\nInstalling dependencies \n' | tee -a "$LOG_FILE"
    sudo yum install -y devtoolset-7 devtoolset-7-elfutils-libelf-devel rh-git227-git.s390x pkgconfig kernel-devel-$(uname -r) kmod perl |& tee -a "$LOG_FILE"
    #switch to GCC 7

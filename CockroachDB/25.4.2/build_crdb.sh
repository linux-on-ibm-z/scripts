...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CockroachDB/25.4.2/build_crdb.sh
...
PACKAGE_VERSION="25.4.2"
GO_VERSION="1.25.4"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CockroachDB/25.4.2/patch"
FORCE="false"
TEST="false"
...
EOF
    ...
    ./dev doctor
    ./dev build
	if [[ "$DISTRO" == "rhel-10.0" || "$DISTRO" == "ubuntu-24.04" || "$DISTRO" == "ubuntu-25.10" ]]; then
...
"ubuntu-25.10")
    ...
    printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
    sudo apt-get update >/dev/null
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y zip unzip autoconf automake wget make libssl-dev libncurses5-dev bison xz-utils patch g++-14 gcc-14 curl git python3 cmake netbase libresolv-wrapper libkeyutils-dev openjdk-21-jdk bzip2 libarchive-tools |& tee -a "$LOG_FILE"
    export CC=gcc-14
    export CXX=g++-14
    fixESTimeZone |&tee -a "$LOG_FILE"
    configureAndInstall |& tee -a "$LOG_FILE"

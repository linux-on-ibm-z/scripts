...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CockroachDB/21.2.6/build_crdb.sh
...
function cleanup() {
	sudo rm -rf ${CURDIR}/go1.16.6.linux-s390x.tar.gz
...
function configureAndInstall() {
	...
	# for rhel 7.x, sles 13, ubuntu 18.04 and ubuntu 20.04
	if [[ ${DISTRO} =~ rhel-7\.* || ${DISTRO} =~ sles-12\.* || ${DISTRO} =~ ubuntu-18\.* || ${DISTRO} =~ ubuntu-20\.* ]]; then
		# Install cmake
	...
	chmod ugo+r go1.16.6.linux-s390x.tar.gz
...
"ubuntu-20.04" | "ubuntu-18.04")
	...
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo apt-get update >/dev/null
	sudo apt-get install -y autoconf automake wget make libssl-dev libncurses5-dev bison xz-utils patch g++ curl git ccache python |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y libopenssl-devel autoconf automake wget ncurses-devel bison patch tar gzip cmake gawk xz python gcc7-c++ zlib-devel bzip2 curl git ccache which |& tee -a "$LOG_FILE"
...
"sles-15.3")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y libopenssl-devel autoconf automake wget ncurses-devel bison patch tar gzip cmake gawk xz gcc-c++ git python curl ccache |& tee -a "$LOG_FILE"
	sudo ln -f -s /usr/bin/gcc /usr/bin/s390x-linux-gnu-gcc
	configureAndInstall |& tee -a "$LOG_FILE"
	;;
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y git autoconf automake openssl-devel wget ncurses-devel bison patch tar gzip xz make bzip2 zlib-devel devtoolset-7-gcc-c++ devtoolset-7-gcc curl diffutils gettext-devel python |& tee -a "$LOG_FILE"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/CockroachDB/21.1.7/build_crdb.sh
...
function cleanup() {
	sudo rm -rf ${CURDIR}/go1.15.14.linux-s390x.tar.gz
...
function configureAndInstall() {
	...
	cd ${CURDIR}
	wget https://storage.googleapis.com/golang/go1.15.14.linux-s390x.tar.gz
	chmod ugo+r go1.15.14.linux-s390x.tar.gz
	sudo tar -C /usr/local -xzf go1.15.14.linux-s390x.tar.gz
...
"ubuntu-18.04" | "ubuntu-20.04" | "ubuntu-21.04")
	...
	sudo apt-get install -y autoconf automake cmake wget libncurses5-dev bison xz-utils patch g++ curl git ccache |& tee -a "$LOG_FILE"
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
	sudo zypper install -y autoconf automake wget ncurses-devel bison patch tar gzip cmake gawk xz python gcc7-c++ zlib-devel bzip2 curl git ccache |& tee -a "$LOG_FILE"
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y git autoconf automake wget ncurses-devel bison patch tar gzip xz make bzip2 zlib-devel devtoolset-7-gcc-c++ devtoolset-7-gcc curl diffutils gettext-devel |& tee -a "$LOG_FILE"
...
"rhel-8.2" | "rhel-8.3" | "rhel-8.4")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
    sudo yum install -y gcc-c++ autoconf libarchive cmake git wget make ncurses-devel curl xz diffutils bison ccache |& tee -a "$LOG_FILE"
    sudo ln -f -s /usr/bin/gcc /usr/bin/s390x-linux-gnu-gcc
    configureAndInstall |& tee -a "$LOG_FILE"
  ;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/helm/3.0.2/build_helm.sh
...
CONF_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Helm/3.0.2/patch"
FORCE="false"
TEST="false"
GOPATH="${CURDIR}"
...
function runTests() {
	...
	if [[ "$TESTS" == "true" ]]; then
		...
		patch -l $GOPATH/src/helm.sh/helm/Makefile Makefile.diff

		wget -O plugin_test.go.diff $CONF_URL/plugin_test.go.diff
		patch -l $GOPATH/src/helm.sh/helm/pkg/plugin/plugin_test.go plugin_test.go.diff

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.35/build_netty.sh
...
function configureAndInstall() {
	...
	if [[ "$VERSION_ID" == "7.8" || "$VERSION_ID" == "7.9" || "$ID" == "sles" ]]  ;then
		printf -- "\nInstalling maven . . . \n"
		cd $SOURCE_ROOT
	...
	printf -- "\nApplying  patch . . . \n"
	# Apply patch
	sed -i '59,59 s/chromium-stable/patch-s390x-Aug2019/g'    pom.xml
	sed -i '100,100 s/boringssl.googlesource.com/github.com\/linux-on-ibm-z/g'  boringssl-static/pom.xml
	mvn install
...
"sles-12.5")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo zypper install -y cmake perl libopenssl-devel libapr1-devel autoconf automake libtool make tar git java-1_8_0-openjdk-devel gcc7 gcc7-c++ wget  which patch |& tee -a "${LOG_FILE}"

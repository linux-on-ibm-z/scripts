...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.71/build_netty.sh
# Execute build script: bash build_netty.sh    (provide -h for help)
#
set -e  -o pipefail
PACKAGE_NAME="netty-tcnative"
PACKAGE_VERSION="2.0.71"
...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
        ...
        sed -i '88,88 s/main/patch-s390x-Jan2021/g' pom.xml 
        sed -i '92,92 s/cccf8525db8a57153d3cb3e22efed2db4b71a8ab/d83fd4af80af244ac623b99d8152c2e53287b9ad/g' pom.xml   
        ...
        sed -i '1475,1475 s/{boringsslHome}/{boringsslHome}\/ssl -L${boringsslHome}\/crypto/g' boringssl-static/pom.xml

        if [[ "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-24.04" || "${DISTRO}" == "ubuntu-24.10" || "${DISTRO}" == "ubuntu-25.04" || "${DISTRO}" == rhel-9*  || "${DISTRO}" == sles* ]] ;then

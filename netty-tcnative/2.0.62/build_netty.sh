...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.62/build_netty.sh
...
function cleanup() {
    # Remove artifacts
        ...
        if [[ $JAVA_PROVIDED == *11 ]]; then
                rm -rf  ibm-semeru-open-jdk_s390x_linux_11.0.20.1_1_openj9-0.40.0.tar.gz
                rm -rf  OpenJDK11U-jdk_s390x_linux_hotspot_11.0.20.1_1.tar.gz
        elif [[ $JAVA_PROVIDED == *17 ]]; then
                rm -rf   ibm-semeru-open-jdk_s390x_linux_17.0.8.1_1_openj9-0.40.0.tar.gz
                rm -rf  OpenJDK17U-jdk_s390x_linux_hotspot_17.0.8.1_1.tar.gz
        else
                rm -rf  ibm-semeru-open-jdk_s390x_linux_8u382b05_openj9-0.40.0.tar.gz
                rm -rf  jdk.tar.gz
        fi
...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.20.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_11.0.20.1_1_openj9-0.40.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_11.0.20.1_1_openj9-0.40.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.20.1+1
                ...
                cd $SOURCE_ROOT
                wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.20.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.20.1_1.tar.gz
                tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.20.1_1.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.20.1+1
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.8.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_17.0.8.1_1_openj9-0.40.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_17.0.8.1_1_openj9-0.40.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.8.1+1
                ...
                cd $SOURCE_ROOT
                        wget https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.8.1%2B1/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.8.1_1.tar.gz
                tar -xzf OpenJDK17U-jdk_s390x_linux_hotspot_17.0.8.1_1.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.8.1+1
			...
			if [[ "${DISTRO}" == rhel-8* ]] ; then
                                export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-17.0.9.0.9-2.el8.s390x
                        fi
                        if [[ "${DISTRO}" == rhel-9* ]] ; then
				export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-17.0.9.0.9-2.el9.s390x
                        fi
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u382-b05_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_8u382b05_openj9-0.40.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_8u382b05_openj9-0.40.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk8u382-b05
...
# Build netty-tcnative
        ...
        sed -i '87,87 s/chromium-stable/patch-s390x-Jan2021/g' pom.xml
        sed -i '91,91 s/dd5219451c3ce26221762a15d867edf43b463bb2/d83fd4af80af244ac623b99d8152c2e53287b9ad/g' pom.xml
        ...
        if [[ "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-23.04" || "${DISTRO}" == "ubuntu-23.10" || "${DISTRO}" == "rhel-9.0" || "${DISTRO}" == "rhel-9.2" ]]  ;then
        curl -o gcc_patch.diff $PATCH_URL/gcc_patch.diff
        cp gcc_patch.diff /tmp/gcc_patch.diff

        sed -i "456i <exec executable=\"git\" failonerror=\"true\" dir=\"\${boringsslSourceDir}\" resolveexecutable=\"true\">"  boringssl-static/pom.xml
        sed -i "457i      <arg value=\"apply\" /> "  boringssl-static/pom.xml
        sed -i "458i   <arg value=\"/tmp/gcc_patch.diff\" />"  boringssl-static/pom.xml
        sed -i "459i   </exec>"  boringssl-static/pom.xml
...
"ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.04" | "ubuntu-23.10")
        ...
        sudo apt-get install -y ninja-build cmake perl golang libssl-dev libapr1-dev autoconf automake libtool make tar git wget maven curl libtool-bin xz-utils gzip python3 |& tee -a "${LOG_FILE}"
...
"rhel-7.8" | "rhel-7.9")
        ...
        sudo yum install -y perl devtoolset-7-gcc-c++ devtoolset-7-gcc openssl-devel apr-devel autoconf automake libtool make tar git wget curl golang xz-devel gzip libcryptui-devel python3-devel patch perl-IPC-Cmd libaio-devel |& tee -a "${LOG_FILE}"
...
"sles-12.5")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo zypper install -y perl libopenssl-devel libapr1-devel autoconf automake libtool make tar git wget curl gcc7 gcc7-c++ gmake xz-devel gzip python3-devel patch |& tee -a "${LOG_FILE}"

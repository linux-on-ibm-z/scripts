...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.57/build_netty.sh
# Execute build script: bash build_netty.sh    (provide -h for help)
#
set -e  -o pipefail
PACKAGE_NAME="netty-tcnative"
PACKAGE_VERSION="2.0.57"
...
if [[ "$JAVA_PROVIDED" != "Semeru8" && "$JAVA_PROVIDED" != "Temurin8" && "$JAVA_PROVIDED" != "OpenJDK8" && "$JAVA_PROVIDED" != "Semeru11" && "$JAVA_PROVIDED" != "Temurin11" && "$JAVA_PROVIDED" != "OpenJDK11" && "$JAVA_PROVIDED" != "Semeru17" && "$JAVA_PROVIDED" != "Temurin17" && "$JAVA_PROVIDED" != "OpenJDK17" ]]; then
                printf "$JAVA_PROVIDED is not supported, Please use valid java from {Semeru8, Temurin8, OpenJDK8, Semeru11, Temurin11, OpenJDK11, Semeru17, Temurin17, OpenJDK17} only"
                exit 1
        fi
if [[ "$FORCE" == "true" ]]; then
...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                if [[ "${ID}" == "ubuntu" ]]; then
                        ...
                        if [[ "${DISTRO}" == rhel-8* ]] ; then
                                export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-17.0.6.0.10-3.el8_7.s390x/
                        fi
                        if [[ "${DISTRO}" == rhel-9* ]] ; then
                                export JAVA_HOME= /usr/lib/jvm/java-17-openjdk-17.0.6.0.10-3.el9_1.s390x
                        fi
                ...
                printf -- "\nInstalling AdoptOpenJDK 8 (With Hotspot) . . . \n"
                cd $SOURCE_ROOT
                export JAVA_HOME=$SOURCE_ROOT
...
cd $SOURCE_ROOT/netty-tcnative
        ...
        if [[ "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-22.10" || "${DISTRO}" == "rhel-9.0" || "${DISTRO}" == "rhel-9.1" ]]  ;then
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  bash build_netty.sh  [-d debug] [-y install-without-confirmation] [-j Java to be used from {Semeru8, Temurin8, OpenJDK8, Semeru11, Temurin11, OpenJDK11, Semeru17, Temurin17, OpenJDK17}]"
        echo
}

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.25/build_netty.sh
...
function configureAndInstall() {
	...
	#Install ninja (for SLES 12 SP4 , RHEL 7.5 and RHEL 7.6 only)
	if [[ "$VERSION_ID" == "7.5" || "$VERSION_ID" == "7.6" || "$VERSION_ID" == "12.4" ]]  ;then
	...
	#Install maven (for SLES , RHEL 7.5 and RHEL 7.6 only)
	if [[ "$VERSION_ID" == "7.5" || "$VERSION_ID" == "7.6" || "$ID" == "sles" ]]  ;then
		printf -- "\nInstalling maven . . . \n"
		cd $SOURCE_ROOT
	...
	#Install GO (RHEL 7.5 and RHEL 7.6 only)
	if [[ "$VERSION_ID" == "7.5" || "$VERSION_ID" == "7.6" ]]  ;then
		cd $SOURCE_ROOT
	...
	#Install gcc 7.4.0 (RHEL 7.5 and RHEL 7.6 only)
	if [[ "$VERSION_ID" == "7.5" || "$VERSION_ID" == "7.6" ]]  ;then
		cd $SOURCE_ROOT
		mkdir gcc
		cd gcc
	...
	cd netty-tcnative
	git checkout netty-tcnative-parent-2.0.25.Final
	
	cd $SOURCE_ROOT/netty-tcnative
	printf -- "\nApplying  patch . . . \n"
	# Apply patch
	curl -o "boringssl.patch"  $PATCH_URL/boringssl.patch
	git apply boringssl.patch
	mvn install 
...
"rhel-7.5" | "rhel-7.6")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo yum install -y perl gcc gcc-c++ openssl-devel apr-devel autoconf automake libtool make tar git java-1.8.0-openjdk-devel wget bzip2 zlib zlib-devel golang patch |& tee -a "${LOG_FILE}"
    configureAndInstall |& tee -a "${LOG_FILE}"
	;;
"rhel-8.0")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo yum install -y cmake perl gcc gcc-c++ openssl-devel apr-devel autoconf automake libtool make tar git java-1.8.0-openjdk-devel wget python2 bzip2 zlib zlib-devel git xz diffutils ninja-build maven golang patch |& tee -a "${LOG_FILE}"
	configureAndInstall |& tee -a "${LOG_FILE}"
	;;
"sles-12.4")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
...
"sles-15")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- "Installing dependencies... it may take some time.\n"
	sudo zypper install -y ninja cmake perl  libopenssl-devel autoconf automake libtool make tar git java-1_8_0-openjdk-devel wget apr-devel zlib-devel gcc gcc-c++ patch |& tee -a "${LOG_FILE}"

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.83/build_netty.sh
# Execute build script: bash build_netty.sh    (provide -h for help)
#
set -e  -o pipefail
PACKAGE_NAME="netty-tcnative"
PACKAGE_VERSION="2.0.83"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/${PACKAGE_VERSION}/patch"
SOURCE_ROOT="$(pwd)"
USER="$(whoami)"
FORCE="false"
...
function installPatchelf() {
        # patchelf is not available in SLES repos — build from source
        # Version 0.14.5 is used as it compiles with GCC 7 (C++11)
        printf -- "Building patchelf from source...\n" |& tee -a "$LOG_FILE"
        cd /tmp
        wget -q https://github.com/NixOS/patchelf/releases/download/0.14.5/patchelf-0.14.5.tar.gz
        ...
        sudo make install
        cd $SOURCE_ROOT
        printf -- "Installation of patchelf is successful\n" >> "$LOG_FILE"
}
function cleanup() {
        # Remove artifacts
        cd $SOURCE_ROOT
        if [[ $JAVA_PROVIDED == *11 ]]; then
                rm -rf  ibm-semeru-open-jdk_s390x_linux_11.0.31.0.tar.gz
                rm -rf  OpenJDK11U-jdk_s390x_linux_hotspot_11.0.31_11.tar.gz
        elif [[ $JAVA_PROVIDED == *17 ]]; then
                rm -rf  ibm-semeru-open-jdk_s390x_linux_17.0.19.0.tar.gz
                rm -rf  OpenJDK17U-jdk_s390x_linux_hotspot_17.0.19_10.tar.gz
        
        else
                rm -rf  OpenJDK8U-jdk_s390x_linux_hotspot_8u292b10.tar.gz 
                rm -rf  ibm-semeru-open-jdk_s390x_linux_8.0.492.0.tar.gz
        fi
...
function configureAndInstall() {
        ...
        if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.31.0/ibm-semeru-open-jdk_s390x_linux_11.0.31.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_11.0.31.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.31+11
                ...
                cd $SOURCE_ROOT
                wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.31%2B11/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.31_11.tar.gz
                tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.31_11.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.31+11
                ...
                if [[ "${ID}" == "ubuntu" ]]; then
                        ...
                        if [[ "${VERSION_ID}" == "10.0" || "${VERSION_ID}" == "10.1" || "${VERSION_ID}" == "10.2" ]]; then 
                                printf -- "$JAVA_PROVIDED is not supported"
                                exit 1
                        fi
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.19.0/ibm-semeru-open-jdk_s390x_linux_17.0.19.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_17.0.19.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.19+10
                ...
                cd $SOURCE_ROOT
                wget https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.19%2B10/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.19_10.tar.gz
                tar -xzf OpenJDK17U-jdk_s390x_linux_hotspot_17.0.19_10.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.19+10
			...
			export JAVA_HOME=/usr/lib/jvm/`echo $java_version | cut -d' ' -f1`
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk-8.0.492.0/ibm-semeru-open-jdk_s390x_linux_8.0.492.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_8.0.492.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk8u492-b09
                ...
                if [[ "${DISTRO}" == ubuntu* || "${DISTRO}" == rhel-9* || "${DISTRO}" == rhel-10*  || "${DISTRO}" == sles* ]] ;then
                        cd $SOURCE_ROOT/
                ...
                sudo tar xzf OpenJDK8U-jdk_s390x_linux_hotspot_8u292b10.tar.gz -C /opt/
        ...
        ./mvnw install -DmuslCheck.skip=true
        #Cleanup
...
case "$DISTRO" in
"rhel-8.10" | "rhel-10.0" | "rhel-10.1" | "rhel-10.2")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y ninja-build cmake perl gcc gcc-c++ libarchive  openssl-devel apr-devel autoconf automake libtool make tar git wget curl golang libstdc++-static.s390x xz-devel gzip python3-devel patch patchelf || true
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;
"rhel-9.6" | "rhel-9.7" | "rhel-9.8")
        printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
        printf -- "Installing dependencies... it may take some time.\n"
        sudo yum install -y ninja-build cmake perl gcc gcc-c++ libarchive  openssl-devel apr-devel autoconf automake libtool make tar git wget golang libstdc++-static.s390x xz-devel gzip python3-devel patch patchelf || true
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;
"sles-15.7" | "sles-16.0")
    printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
    ...
    installPatchelf |& tee -a "${LOG_FILE}"
    configureAndInstall |& tee -a "${LOG_FILE}"
    ;;
"ubuntu-22.04" | "ubuntu-24.04")
        ...
        sudo apt-get install -y ninja-build cmake patch perl golang libssl-dev libapr1-dev autoconf automake libtool make tar git wget curl libtool-bin xz-utils gzip python3 patchelf |& tee -a "${LOG_FILE}"
        configureAndInstall |& tee -a "${LOG_FILE}"
        ;;
*)

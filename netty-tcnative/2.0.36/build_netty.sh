...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.36/build_netty.sh
...
function cleanup() {
	...
	rm -rf  OpenJDK11U-jdk_s390x_linux_openj9_11.0.9_11_openj9-0.23.0.tar.gz
	rm -rf  OpenJDK11U-jdk_s390x_linux_hotspot_11.0.9.1_1.tar.gz
...
function configureAndInstall() {
	...
	if [[ "$JAVA_PROVIDED" == "AdoptJDK11_openj9" ]]; then
		...
		export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.9+11
		...
		cd $SOURCE_ROOT
		wget https://github.com/AdoptOpenJDK/openjdk11-binaries/releases/download/jdk-11.0.9.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.9.1_1.tar.gz
		tar -xf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.9.1_1.tar.gz
		export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.9.1+1
	...
	printf -- "\nApplying  patch . . . \n"
	# Apply patch
	sed -i '62,62 s/chromium-stable/patch-s390x-Aug2019/g' pom.xml
	sed -i '66,66 s/3743aafdacff2f7b083615a043a37101f740fa53/fded602587cc553802e5d73541734c7be2dbfe98/g' pom.xml
...
"rhel-7.8" | "rhel-7.9")
	...
	sudo yum install -y perl devtoolset-7-gcc-c++ devtoolset-7-gcc openssl-devel apr-devel autoconf automake libtool make tar git wget golang maven |& tee -a "${LOG_FILE}"

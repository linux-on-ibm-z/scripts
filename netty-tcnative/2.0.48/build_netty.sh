...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.48/build_netty.sh
...
function cleanup() {
	...
	rm -rf  ibm-semeru-open-jdk_s390x_linux_11.0.13_8_openj9-0.29.0.tar.gz
	rm -rf  OpenJDK11U-jdk_s390x_linux_hotspot_11.0.13_8.tar.gz
...
function configureAndInstall() {
	...
	if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
		...
		export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.13+8
	...
	printf -- "\nApplying  patch . . . \n"
	# Apply patch
	sed -i '70,70 s/chromium-stable/patch-s390x-Jan2021/g' pom.xml
	sed -i '74,74 s/1607f54fed72c6589d560254626909a64124f091/d83fd4af80af244ac623b99d8152c2e53287b9ad/g' pom.xml
	sed -i '55,55 s/boringssl.googlesource.com/github.com\/linux-on-ibm-z/g' boringssl-static/pom.xml
	sed -i '56,56 s/chromium-stable/patch-s390x-Jan2021/g' boringssl-static/pom.xml
	
	if [[ "${DISTRO}" == "ubuntu-21.10" ]]  ;then 
	curl -o gcc_patch.diff $PATCH_URL/gcc_patch.diff 
	cp gcc_patch.diff /tmp/gcc_patch.diff
	
	sed -i "185i <exec executable=\"git\" failonerror=\"true\" dir=\"\${boringsslSourceDir}\" resolveexecutable=\"true\">"  boringssl-static/pom.xml
	sed -i "186i      <arg value=\"apply\" /> "  boringssl-static/pom.xml
	sed -i "187i   <arg value=\"/tmp/gcc_patch.diff\" />"  boringssl-static/pom.xml
	sed -i "188i   </exec>"  boringssl-static/pom.xml

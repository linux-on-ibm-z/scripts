...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/netty-tcnative/2.0.65/build_netty.sh
# Execute build script: bash build_netty.sh    (provide -h for help)
#
set -e  -o pipefail
PACKAGE_NAME="netty-tcnative"
PACKAGE_VERSION="2.0.65"
...
function cleanup() {
    # Remove artifacts
        ...
        if [[ $JAVA_PROVIDED == *11 ]]; then
                rm -rf  ibm-semeru-open-jdk_s390x_linux_11.0.22_7_openj9-0.43.0.tar.gz
                rm -rf  OpenJDK11U-jdk_s390x_linux_hotspot_11.0.22_7.tar.gz
        elif [[ $JAVA_PROVIDED == *17 ]]; then
                rm -rf   ibm-semeru-open-jdk_s390x_linux_17.0.10_7_openj9-0.43.0.tar.gz
                rm -rf  OpenJDK17U-jdk_s390x_linux_hotspot_17.0.10_7.tar.gz
        else
                rm -rf  ibm-semeru-open-jdk_s390x_linux_8u402b06_openj9-0.43.0.tar.gz
                rm -rf  jdk.tar.gz
        fi
...
if [[ "$JAVA_PROVIDED" == "Semeru11" ]]; then
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.22%2B7_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_11.0.22_7_openj9-0.43.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_11.0.22_7_openj9-0.43.0.tar.gz
                ...
                cd $SOURCE_ROOT
                wget https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.22%2B7/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.22_7.tar.gz
                tar -xzf OpenJDK11U-jdk_s390x_linux_hotspot_11.0.22_7.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-11.0.22+7
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.10%2B7_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_17.0.10_7_openj9-0.43.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_17.0.10_7_openj9-0.43.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.10+7
                ...
                cd $SOURCE_ROOT
                        wget https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.10%2B7/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.10_7.tar.gz
                tar -xzf OpenJDK17U-jdk_s390x_linux_hotspot_17.0.10_7.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk-17.0.10+7
			...
			if [[ "${DISTRO}" == rhel-8* ]] ; then
                ...
                cd $SOURCE_ROOT
                wget https://github.com/ibmruntimes/semeru8-binaries/releases/download/jdk8u402-b06_openj9-0.43.0/ibm-semeru-open-jdk_s390x_linux_8u402b06_openj9-0.43.0.tar.gz
                tar -xzf ibm-semeru-open-jdk_s390x_linux_8u402b06_openj9-0.43.0.tar.gz
                export JAVA_HOME=$SOURCE_ROOT/jdk8u402-b06
...
# Build netty-tcnative
        ...
        sed -i '91,91 s/1b7fdbd9101dedc3e0aa3fcf4ff74eacddb34ecc/d83fd4af80af244ac623b99d8152c2e53287b9ad/g' pom.xml
        ...
        if [[ "${DISTRO}" == "ubuntu-22.04" || "${DISTRO}" == "ubuntu-23.10" || "${DISTRO}" == "rhel-9.0" || "${DISTRO}" == "rhel-9.2" || "${DISTRO}" == "rhel-9.3" ]]  ;then
...
"rhel-7.8" | "rhel-7.9")
        ...
        sudo yum install -y perl devtoolset-7-gcc-c++ devtoolset-7-gcc openssl-devel apr-devel autoconf automake libtool make tar git wget curl golang xz-devel gzip libcryptui-devel python3-devel patch perl-IPC-Cmd libaio-devel ca-certificates|& tee -a "${LOG_FILE}"

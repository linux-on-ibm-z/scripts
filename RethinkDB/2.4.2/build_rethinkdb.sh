...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/RethinkDB/2.4.2/build_rethinkdb.sh
# Execute build script: bash build_rethinkdb.sh    (provide -h for help)

set -e -o pipefail

PACKAGE_NAME="rethinkdb"
PACKAGE_VERSION="v2.4.2"
PRE_VERSION="v2.4.1"
CURDIR="$(pwd)"
FORCE="false"
TESTS="false"
...
function gettingStarted() {
	...
	printf -- "\n\nUsage: \n"
	printf -- "  RethinkDB v2.4.2 installed successfully \n"
...
"sles-15.3")
	printf -- "Installing %s %s for %s \n" "$PACKAGE_NAME" "$PACKAGE_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
	printf -- 'Installing the dependencies for RethinkDB from repository \n' |& tee -a "$LOG_FILE"
	sudo zypper install -y gcc gcc-c++ make libopenssl-devel zlib-devel wget tar patch unzip autoconf automake m4 libtool python python-xml python-curses libicu-devel protobuf-devel libprotobuf-c-devel boost-devel termcap curl libcurl-devel git bzip2 awk gzip xz readline-devel libncurses5 ncurses-devel netcfg libbz2-devel glibc-locale |& tee -a "$LOG_FILE"
	configureAndInstall |& tee -a "$LOG_FILE"
	;;

...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/TensorflowServing/2.16.1/build_tensorflow_serving.sh
...
SOURCE_ROOT="$(pwd)"
USER="$(whoami)"
PATCH_URL="https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/TensorflowServing/2.16.1/patch"
...
function configureAndInstall() {
	...
	sed -i '162i patch -p1 < tf.patch' build_tensorflow.sh
 	...
 	if [[ $DISTRO = "ubuntu-24.04" ]]; then
 	       curl -o tfs_libevent.patch $PATCH_URL/tfs_libevent.patch
	       patch -p1 < tfs_libevent.patch               
        fi

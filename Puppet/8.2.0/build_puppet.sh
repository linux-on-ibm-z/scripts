...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Puppet/8.2.0/build_puppet.sh
...
TEMURIN_JDK11_URL="https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.20.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.20.1_1.tar.gz"
TEMURIN_JDK17_URL="https://github.com/adoptium/temurin17-binaries/releases/download/jdk-17.0.8.1%2B1/OpenJDK17U-jdk_s390x_linux_hotspot_17.0.8.1_1.tar.gz"
SEMERU_JDK11_URL="https://github.com/ibmruntimes/semeru11-binaries/releases/download/jdk-11.0.20.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_11.0.20.1_1_openj9-0.40.0.tar.gz"
SEMERU_JDK17_URL="https://github.com/ibmruntimes/semeru17-binaries/releases/download/jdk-17.0.8.1%2B1_openj9-0.40.0/ibm-semeru-open-jdk_s390x_linux_17.0.8.1_1_openj9-0.40.0.tar.gz"
...
function cleanup() {
        ...
        sudo rm -rf $SOURCE_ROOT/ruby-3.1.4
...
function buildAgent() {
        ...
        cd "$SOURCE_ROOT"
           ...
           if [[ ${DISTRO} == rhel-7\.[8-9] ]] || [[ "${ID}" == "ubuntu" ]]; then
...
if [[ "$USEAS" == "server" ]]; then
        ...
        "ubuntu-20.04" | "ubuntu-22.04" | "ubuntu-23.04" | "ubuntu-23.10")
                ...
                sudo apt-get install -y g++ tar git make wget locales locales-all unzip zip gzip gawk zlib1g zlib1g-dev openssl libssl-dev bison flex libdb-dev libgdbm-dev libreadline-dev libyaml-dev |& tee -a "$LOG_FILE"
        ...
        "rhel-7.8" | "rhel-7.9")
                ...
                sudo yum install -y devtoolset-7-gcc-c++ devtoolset-7-gcc bison flex openssl-devel readline-devel gdbm-devel gcc make wget tar libyaml-devel unzip git zip gzip gawk zlib-devel |& tee -a "$LOG_FILE" 
        ...
        "rhel-8.6" | "rhel-8.8" | "rhel-9.0" | "rhel-9.2" | "rhel-9.3")
                printf -- "Installing %s Server %s for %s \n" "$PACKAGE_NAME" "$SERVER_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo yum install -y gcc-c++ tar unzip openssl-devel make git wget zip gzip gawk zlib-devel bison flex readline-devel gdbm-devel libyaml-devel |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;
        ...
        "sles-15.4" | "sles-15.5")
                printf -- "Installing %s Server %s for %s \n" "$PACKAGE_NAME" "$SERVER_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo zypper install -y gcc-c++ tar unzip libopenssl-devel make git wget zip gzip gawk zlib-devel bison flex readline-devel gdbm-devel libyaml-devel |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;
        ...
        *)
        ...
        "ubuntu-20.04" | "ubuntu-22.04")
                ...
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo apt-get update >/dev/null
                sudo apt-get install -y g++ tar make wget gzip gawk zlib1g zlib1g-dev openssl libssl-dev bison flex libdb-dev libgdbm-dev libreadline-dev libyaml-dev |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;
        ...
        "rhel-8.6" | "rhel-8.8" | "rhel-9.0" | "rhel-9.2" | "rhel-9.3")
                printf -- "Installing %s Agent %s for %s \n" "$PACKAGE_NAME" "$AGENT_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo yum install -y gcc-c++ tar make wget openssl-devel gzip gawk zlib-devel bison flex readline-devel gdbm-devel libyaml-devel |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;
        ...
        "sles-12.5")
                printf -- "Installing %s Agent %s for %s \n" "$PACKAGE_NAME" "$AGENT_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo zypper install -y gcc7-c++ tar libopenssl-devel make wget gzip gawk zlib-devel libnghttp2-devel bison flex readline-devel gdbm-devel libyaml-devel |& tee -a "$LOG_FILE"                 
                sudo ln -sf /usr/bin/gcc-7 /usr/bin/gcc
                sudo ln -sf /usr/bin/gcc /usr/bin/cc    
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;
        ...
        "sles-15.4" | "sles-15.5")
                printf -- "Installing %s Agent %s for %s \n" "$PACKAGE_NAME" "$AGENT_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo zypper install -y gcc-c++ tar make wget libopenssl-devel gzip gawk zlib-devel bison flex readline-devel gdbm-devel libyaml-devel |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;

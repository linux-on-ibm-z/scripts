...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Puppet/7.13.1/build_puppet.sh
...
SOURCE_ROOT="$(pwd)"
PACKAGE_NAME="Puppet"
PACKAGE_VERSION="7.13.1"
SERVER_VERSION="7.5.0"
AGENT_VERSION="7.13.1"
RUBY_VERSION="2.7"
RUBY_FULL_VERSION="2.7.2"
JFFI_VERSION="1.3.1"
...
JDK11_URL="https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.13%2B8/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.13_8.tar.gz"
...
function checkPrequisites() {
        ...
        if [[ "$JAVA_PROVIDED" != "Eclipse_Adoptium_Temurin_runtime_11" && "$JAVA_PROVIDED" != "OpenJDK11" ]]; then
...
function cleanup() {
        ...
        if [[ -f apache-ant-1.10.9-bin.tar.gz ]]; then
                sudo rm apache-ant-1.10.9-bin.tar.gz
        fi
...
function buildAgent() {
        ...
        if [[ "$DISTRO" == "sles-15.2" || "$DISTRO" == "sles-15.3" ]]; then
...
function buildServer() {
        ...
        if [[ "$JAVA_PROVIDED" == "Eclipse_Adoptium_Temurin_runtime_11" ]]; then
                ...
                cd $SOURCE_ROOT
                wget -O adoptium.tar.gz ${JDK11_URL}
                mkdir -p adoptium11
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  build_puppet.sh [-s server/agent] [-j Java to be used from {Eclipse_Adoptium_Temurin_runtime_11, OpenJDK11}] "
        echo
}
...
if [[ "$USEAS" == "server" ]]; then
        ...
        "ubuntu-18.04"| "ubuntu-20.04")
                ...
                sudo apt-get install -y g++ tar git make wget locales locales-all unzip ruby-dev ant zip libjffi-jni |& tee -a "$LOG_FILE"
        ...
        "rhel-8.2" | "rhel-8.4" | "rhel-8.5")
                printf -- "Installing %s Server %s for %s \n" "$PACKAGE_NAME" "$SERVER_VERSION" "$DISTRO" |& tee -a "$LOG_FILE"
                printf -- "Installing the dependencies for $PACKAGE_NAME from repository \n" |& tee -a "$LOG_FILE"
                sudo yum install -y gcc-c++ tar unzip openssl-devel make git wget zip ant ruby |& tee -a "$LOG_FILE"
                configureAndInstall |& tee -a "$LOG_FILE"
                ;;

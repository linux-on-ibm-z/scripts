...
#
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/Puppet/7.16.0/build_puppet.sh
...
SERVER_VERSION="7.7.0"
AGENT_VERSION="7.16.0"
RUBY_VERSION="2.7"
...
JDK11_URL="https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.14.1%2B1/OpenJDK11U-jdk_s390x_linux_hotspot_11.0.14.1_1.tar.gz"
...
function cleanup() {
        ...
        if [[ -f apache-ant-1.10.12-bin.tar.gz ]]; then
                sudo rm apache-ant-1.10.12-bin.tar.gz
        fi
...
function buildServer() {
        ...
        if [[ "$DISTRO" == "rhel-7.8" || "$DISTRO" == "rhel-7.9" ]]; then
                ...
                export ANT_HOME=$SOURCE_ROOT/apache-ant-1.10.12
...
function printHelp() {
        echo
        echo "Usage: "
        echo "  bash build_puppet.sh [-s server/agent] [-j Java to be used from {Eclipse_Adoptium_Temurin_runtime_11, OpenJDK11}] "
        echo
}
